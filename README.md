...
### 可視化・分析
- **ダッシュボード** -- 10 種類のページ、57 種類のチャートコンポーネント
...
└── app/                      # アプリケーション本体
    ...
    └── src/
        ...
        │   ├── models/       #   29 モデルファイル（型定義・値オブジェクト）
        │   ├── calculations/ #   19 計算モジュール（粗利・予算・予測・要因分解）
        ...
        │   ├── pages/        #   10 ページ（Dashboard, Daily, Category, Analysis, Forecast, etc.）
        │   ├── components/   #   UI コンポーネント
        │   │   ├── charts/   #     57 チャートコンポーネント
        │   │   ├── common/   #     共通 UI 部品
...
| **Application** | 状態管理（Zustand ストア）、46 カスタムフック、ユースケース、Web Worker |
| **Infrastructure** | IndexedDB 永続化、ファイル読み込み/書き出し、データ変換、DuckDB-WASM、i18n、PWA |
| **Presentation** | React コンポーネント、10 ページ、57 チャート、テーマ（ダーク/ライト対応）。描画のみを担当 |
