...
| 指標 | 数値 |
|---|---|
| ページ | 11 アクティブ（Dashboard, Daily, Insight, Category, CostDetail, StoreAnalysis, PurchaseAnalysis, Reports, Admin, CustomPage, Mobile）+ 6 リダイレクト（Analysis→Insight, Forecast→Insight, Summary→Insight, Transfer→CostDetail, Consumable→CostDetail） |
| チャート | 73 |
| ダッシュボードウィジェット | 124 ファイル |
| ドメインモデル | 45 |
| 計算モジュール | 43（全て純粋関数） |
| DuckDB クエリモジュール | 16 |
| カスタムフック | 109 |
