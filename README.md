...
| 指標 | 数値 |
|---|---|
| ページ | 11 アクティブ（Dashboard, Daily, Insight, Category, CostDetail, StoreAnalysis, PurchaseAnalysis, Reports, Admin, CustomPage, Mobile）+ 6 リダイレクト |
| チャート | 79 |
| ダッシュボードウィジェット | 117 ファイル |
| ドメインモデル | 47 |
| 計算モジュール | 48（全て純粋関数） |
| DuckDB クエリモジュール | 18 |
| カスタムフック | 110 |
| Storybook ストーリー | 21 |
| WASM モジュール | 5（factor-decomposition, gross-profit, budget-analysis, forecast, time-slot） |
| テスト | 4,659+（250ファイル） |
...
| 粗利推移チャート | **ECharts 移行完了** | GrossProfitRateChart / GrossProfitAmountChart |
| 要因分解チャート | **ECharts 移行完了** | FactorDecompositionPanel |
| 相関分析チャート | **ECharts 移行完了** | WeatherCorrelationChart |

**移行方針:** 全チャートが ECharts に移行済み。designSystemGuard で `MAX_RECHARTS_FILES = 0` を強制。
...
- **設計原則 A1-G7（7カテゴリ・28原則）** — 詳細は [design-principles.md](./references/01-principles/design-principles.md)、管理は architecture ロール
- **ガードテスト 7ファイル** — 層境界・純粋性・サイズ・temporal等を機械的に強制
- **UI/UX 4 原則** — [uiux-principles.md](./references/01-principles/uiux-principles.md)

## 参照資料（references/）

**設計原則・制約**（01-principles/）
- [設計原則 A1-G7](./references/01-principles/design-principles.md)
