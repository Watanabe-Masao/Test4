...
> **R0 (境界定義先行) で articulate**: 本 repo は 3 tree (= references / aag / projects) で構造化。各 tree は **reader 別 + 役割別** に articulate されており、AI / userが物理配置を見て「読む / 読まない」を即判断可能。

| tree | 役割 | reader |
|---|---|---|
| **`references/`** | 主アプリ (粗利管理ツール) 改修 AI / userの **knowledge interface** (= 業務 domain knowledge + 主アプリ実装ガイド + 動的 state observation + 主アプリ改修 userが使う AAG public interface) | 主アプリ改修 user |
| **`aag/`** | **AAG framework 本体** (= 内部 articulation + framework 実装 skeleton)。主アプリ改修 userは通常 not read | AAG framework 改修者のみ |
...
詳細: `references/README.md` + `aag/README.md` + projects/ root の README.md。進行中 structural reorganization は `projects/active/aag-self-hosting-completion/plan.md` 参照。
...
> 生成: 2026-05-03T21:55:35.939Z — 正本: `docs/contracts/test-contract.json` — 6/6 契約満足
...
3. **期間 buffer は anti-ritual** — archive / sunset の trigger は state-based (inbound 0 機械検証 + user deletion approval) のみ (`AAG-REQ-NO-DATE-RITUAL`)
...
| 層 | 担い手 | 責務 |
|---|---|---|
| Authority | user | 何をやるか・やらないか（最終意思決定） |
...
詳細は `references/04-tracking/project-structure.md` を参照（正本）。
...
詳細は `references/03-implementation/coding-conventions.md` を参照。

- **strict mode** / `noUnusedLocals` / `noUnusedParameters` — ビルドで強制
- パスエイリアス: `@/` → `src/`
- `any` 禁止（lint エラー）、`readonly` 推奨
- `authoritative` 単独使用禁止 — 必ず `business-authoritative` / `analytic-authoritative` / `candidate-authoritative` で修飾する（`references/01-foundation/semantic-classification-policy.md`）
...
references/04-tracking/generated/
  ...
  architecture-health.md   �� ビュー（user-readableレポート）
    ↓ 埋め込み
...
詳細・適用例は `references/01-foundation/design-principles.md` を参照。管理責任: architecture。
H カテゴリの詳細は `references/01-foundation/safe-performance-principles.md` を参照。
Safety Tier 分類は `references/01-foundation/critical-path-safety-map.md` を参照。
安全設計改善計画は `references/03-implementation/safety-first-architecture-plan.md` を参照。
モジュラーモノリス進化は `references/01-foundation/modular-monolith-evolution.md` を参照。
...
`references/01-foundation/design-principles.md` を参照。
...
- 詳細: `aag/_internal/strategy.md` (戦略 + 文化論 + AAG の本質 + AI 対話、Project A Phase 1〜2 で `adaptive-architecture-governance.md` から Split + Rewrite、Phase 5.8 で旧 doc archive)
- 進化方針: `aag/_internal/evolution.md` (Project A Phase 1 で `adaptive-governance-evolution.md` から Rewrite + Relocate + Rename)
- Architecture Rule 運用: `references/03-implementation/architecture-rule-system.md`
...
1. `references/04-tracking/taxonomy-review-journal.md`（Phase 2 で landing）に提案 entry を追加
2. 提案には Why / 対応 Antibody Pair / 必須 T:kind（または R:tag）/ 推定 promotionLevel を併記
3. 四半期 review window でuser 判断 → 承認後にのみ registry / Constitution / Interlock を改変
...
- 詳細マップ: `references/03-implementation/responsibility-v1-to-v2-migration-map.md` §3.4 / `references/03-implementation/test-tsig-to-v2-migration-map.md` §3.5
- AR-G3-SUPPRESS-RATIONALE は **撤退対象外**（scope 違いで恒久維持）

詳細:

- Constitution 本体: `references/01-foundation/taxonomy-constitution.md`
- Interlock マトリクス: `references/01-foundation/taxonomy-interlock.md`
- Origin Journal: `references/01-foundation/taxonomy-origin-journal.md`
- 親 plan: `projects/active/taxonomy-v2/plan.md` §Operational Control System / §AR-TAXONOMY-*
...
詳細は `references/01-foundation/uiux-principles.md` を参照。
...
データは4段階を経てUIに到達する。詳細は `references/01-foundation/data-flow.md` を参照。
パイプライン整合性の設計思想は `references/01-foundation/data-pipeline-integrity.md`、
期間スコープの分離ルールは `references/01-foundation/temporal-scope-semantics.md` を参照。
...
詳細は `references/01-foundation/engine-boundary-policy.md`、`references/01-foundation/engine-responsibility.md` を参照。
意味分類ポリシーは `references/01-foundation/semantic-classification-policy.md` を参照。
...
シャープリー恒等式・不変条件の詳細は `references/03-implementation/invariant-catalog.md` を参照。
**合計値は実際の売上差に完全一致。** カテゴリデータからの再計算は禁止（D1/D2）。

## 許可リスト・ガード運用

詳細は `references/03-implementation/allowlist-management.md` を参照。
即差し戻し条件は `roles/staff/review-gate/SKILL.md` を参照。

## 仕入原価の正本ルール

詳細は `references/01-foundation/purchase-cost-definition.md` を参照（manifest.discovery.byTopic 経由）。
実施計画は `references/03-implementation/purchase-cost-unification-plan.md` を参照。
実装関数 / 定義書 / ガードのペアは「## 正本化体系（readModels）」セクションを参照。

## 実行時データ経路（実装の主経路）

詳細は `references/03-implementation/runtime-data-path.md` を参照。
...
> 生成: 2026-05-03T21:55:35.932Z — 正本: `references/04-tracking/generated/architecture-health.json`
<!-- GENERATED:END architecture-health-summary -->

## 正本化体系（readModels）

主要な業務値は `application/readModels/` に正本化されている。
定義書は `references/01-foundation/` を参照。
...
- **正本化原則:** `references/01-foundation/canonicalization-principles.md`
- **正本化マップ:** `references/01-foundation/calculation-canonicalization-map.md` — domain/calculations/ 全ファイルの分類
...
詳細は `references/03-implementation/explanation-architecture.md` を参照。
