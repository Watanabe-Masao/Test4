...
| review-gate | 品質の出口。成果物を受けて PASS/FAIL を自律判定 | 13禁止事項・ガードテスト・CI 6段階ゲート |
...
| 10 | 率（rate）をパイプラインで持ち回す・SQL/VMで直接計算する | 加重平均が崩壊し、全店合計・経過日集約が不正確になる。率は domain/calculations で算出する |
| 11 | presentation/ から外部 API・ETRN・fetch を直接呼び出す | レイヤー境界が崩壊し、データ取得ロジックが分散する。外部データ取得は application/（hooks, usecases）または infrastructure/ で行い、presentation/ は hook 経由で結果を受け取るだけにする |
| 12 | 取得対象データの契約（何を取得すべきか）を application/ や presentation/ で定義する | 取得対象の契約は domain/ の型・インターフェースで定義する。application/ は取得・調停、infrastructure/ は実装、presentation/ は描画に専念する |
| 13 | `number \| null` 型の欠損判定に truthiness（`!value` / `if (value)`）を使う | `0` が有効値のフィールド（WMO天気コード、数量、インデックス等）で正当な値が欠損扱いされる。欠損判定は必ず `== null` / `!= null` で行う |
...
- `architectureGuard.test.ts` で `_prototypes/` 外に `Demo` / `Prototype` を含むファイル名がないことを検証する
...
| R12 | Presentation コンポーネント .tsx は 600行以下。Tier 2（600行超）は除外リストで個別管理 | ガードテスト |
...
- Presentation コンポーネント .tsx が 600行を超えている（R12）
