...
- **Phase 5 三段構造確立** `chart-rendering-three-stage-pattern.md` 新設、`YoYChartOptionBuilder.ts` に option builder 抽出 (YoYChart.tsx 260 → 162 行)、`chartRenderingStructureGuard` 追加 (baseline 3 件 ratchet-down)（本 commit）**描画側 Phase 5 pattern 正式化**
...
- **Phase 5 閉じ込み (描画側)**: `chartRenderingStructureGuard` の baseline 3 件 (`DiscountTrendChart` / `GrossProfitAmountChart` / `PrevYearComparisonChart`) を `*Logic.ts` に抽出して 3 → 0 に縮退させる。YoYChart が見本実装として確定しているのでそのままパターンを横展開
