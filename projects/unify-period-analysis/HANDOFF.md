...
- **Phase 6.5 / Step B (readModel 次元拡張)**: 設計 + Phase 6.5-1 (型契約 + pre-work guard) + Phase 6.5-2 (projection 意味境界の凍結、33 truth-table tests) + Phase 6.5-4 (bundle + ctx 配布、AR-003 49 → 51) は完了済み。次の一手は **Phase 6.5-5** = `SalesPurchaseComparisonChart` / `YoYWaterfallChart` を `ctx.storeDailyLane.bundle.currentSeries` / `ctx.categoryDailyLane.bundle.currentSeries` 経由に載せ替え、両 surface guard baseline を全 0 到達させる。Step C の `ee11528` (chart 載せ替え) と同形の widget-level refactor
