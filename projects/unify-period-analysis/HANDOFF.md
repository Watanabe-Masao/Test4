...
- **Phase 6a→6b 移行保証** `useComparisonModuleLegacyCallerGuard.test.ts` を新設。`useComparisonModule` の legacy 3 引数 caller を ratchet-down で固定 (baseline 2 件: `MobileDashboardPage` / `useInsightData`)。Phase 6b で順次 0 に縮退させる計画（本 commit）
