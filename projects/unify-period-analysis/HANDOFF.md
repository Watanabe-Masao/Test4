...
- **Phase 6.5 / Step B (readModel 次元拡張)**: 設計 + Phase 6.5-1 (型契約 + pre-work guard) + Phase 6.5-2 (projection 意味境界の凍結、33 truth-table tests) は完了済み。次の一手は **Phase 6.5-4** = `useStoreDailyBundle` / `useCategoryDailyBundle` hook を新設し `useUnifiedWidgetContext` に `ctx.storeDailyLane` / `ctx.categoryDailyLane` を配布する (Step C の `useTimeSlotBundle` と同形、AR-003 fieldMax 49 → 51 ratchet-up を伴う)。Phase 6.5-3 は 6.5-2 に統合完了
