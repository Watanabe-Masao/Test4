...
- **Phase 6.5 / Step B (readModel 次元拡張)**: 設計 + Phase 6.5-1 (型契約 + guard) + 6.5-2 (projection 意味境界) + 6.5-4 (bundle + ctx) + 6.5-5a (SalesPurchaseComparisonChart 載せ替え、baseline 2 → 1 intentional floor) は完了済み。次の一手は **Phase 6.5-5b** = YoYWaterfall ecosystem (`builders.ts` / `data.ts` / `logic.ts` / `vm.ts`) の `CategoryTimeSalesRecord` 参照 13 箇所を `ctx.categoryDailyLane.bundle.currentSeries` 経由に切替、`categoryDailyLaneSurfaceGuard` baseline 3/5/3/2 → 0/0/0/0 到達させる。widget 側で並び替え / totals 再計算は行わず、6.5-2 の凍結通りに消費する
