...
- **Phase 5 横展開 第 1 バッチ** 5 chart を共通 builder `buildBaseQueryInput` 経由に移行、chartInputBuilderGuard baseline 8 → 3（本 commit）
...
- **Phase 5 横展開 第 2 バッチ**: 残る 3 chart を移行
  - `FactorDecompositionPanel.tsx` — `PairedInput` (当期 + 比較期) の 2 range 用に専用 builder が必要
  - `useCategoryHierarchyData.ts` / `useDeptHourlyChartData.ts` — hook 形式で複数 `dateRangeToKeys` 呼び出し、構造確認の上 builder 化
