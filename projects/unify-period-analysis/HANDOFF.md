...
- **Phase 6a (scope 二重構築解消)** `useComparisonModule` に optional `externalScope` パラメータを追加、`useComparisonSlice` が `frame.comparison` を pass-through することで scope の二重構築を解消。`useComparisonModuleExternalScope.test.ts` (8 tests) で解決ロジックを locked（本 commit）
...
- **Phase 6b**: 残る 2 caller (`MobileDashboardPage` / `useInsightData`) も frame ベースに移行し、`useComparisonModule` から periodSelection 依存を除去。最終的には `useComparisonModule(scope, {period1, period2, ...})` のように scope を primary 入力、残りを minimal な補助 input として受け取る形へ
