...
- **Phase 6.5 / Step B (readModel 次元拡張)**: 設計 + Phase 6.5-1 (型契約 + guard) + 6.5-2 (projection 意味境界) + 6.5-4 (bundle + ctx) + 6.5-5a (SalesPurchaseComparisonChart、baseline 2 → 1 intentional floor) + 6.5-5b (YoYWaterfall ecosystem、baseline 13 → 6 intentional floor、Shapley 5-factor は leaf-grain 必須) は完了済み。次の一手は **Phase 6.5-6** = inventory/05 の HIGH リスク 2 件を Done 記録 + Phase 6 全体クローズ (HANDOFF の Step B block を完了済みに移す)
