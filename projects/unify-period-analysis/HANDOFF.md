...
> 後続が「Phase 1 で比較内部ロジックも frame-native 化済み」と誤読しないよう
> 注意。

### 高優先（次に着手するもの）

- **Phase 2**: `ComparisonScope` resolver への比較先解決集約。主対象は `inventory/01-comparison-math-in-presentation.md` の 4 箇所:
  - `presentation/pages/Dashboard/widgets/YoYWaterfallChart.logic.ts` L18-L50, L36-L37（主犯）
  - `presentation/components/charts/DailySalesChartBodyLogic.ts` L130-L138
  - `presentation/pages/Dashboard/widgets/types.ts` L46
