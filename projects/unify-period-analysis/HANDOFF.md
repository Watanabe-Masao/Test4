...
- **Phase 6.5 / Step B 設計固定** `phase-6-5-step-b-design.md` を新設。`FreePeriodReadModel` を触らず sibling lane (`ctx.storeDailyLane` / `ctx.categoryDailyLane`) 2 本で店舗別日次 + category 次元を配布する方針を固定。6 phase 実装順 (型契約 → 真理表 → pure projection → bundle → widget 載せ替え → クローズ) と AR-003 fieldMax 49 → 51 の ratchet-up 承認を明文化 (本 commit)
...
- **Phase 6.5 / Step B (readModel 次元拡張)**: 設計は `phase-6-5-step-b-design.md` に固定済み。次の一手は Phase 6.5-1 (型契約 + pre-work guard) を 1 commit で追加すること。方針は `FreePeriodReadModel` を touch せず sibling lane 2 本 (`ctx.storeDailyLane` / `ctx.categoryDailyLane`) で店舗別日次 + category 日次を配布する。Step C と同じ成功パターン (方針 → 型 → 真理表 → 実装 → 載せ替え) で進める
