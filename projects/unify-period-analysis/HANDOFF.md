...
- **Phase 5 見本実装** `YoYChart.tsx` を pure builder 経由に移行、allowlist 1 → 0（本 commit）
...
### Phase 5 chart 薄化テンプレート（見本実装から抽出した共通手順）

新規 chart を作るとき、および既存 chart を薄化するときに以下を適用する:
...
3. **widget は builder を呼ぶだけにする** — useMemo で builder を呼び、
   結果を plan hook に渡す。scope.X を直接読まない
4. **存在判定は scope の null チェックに限定する** — empty state 判定など、
   scope 全体が存在するかどうかだけを見る
5. **guard を更新する** — `comparisonResolvedRangeSurfaceGuard` の allowlist に
   古い widget が載っていれば削除。パターンとして baseline 0 を維持

見本実装: `YoYChart.tsx` + `buildYoyDailyInput.ts` (commit 本 PR)

### 高優先（次に着手するもの）

- **Phase 5 横展開**: 他 chart (例: `UnifiedYoYWidget` 以外の comparison chart) を同じパターンで薄化
- **Phase 5 三段構造**: `FreePeriodReadModel → ViewModel → Chart` の明示的な ViewModel 層確立 (現状は builder 1 段のみ)
