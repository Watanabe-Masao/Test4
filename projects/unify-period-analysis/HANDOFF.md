...
### 完了済み（Phase 0 / 0.5 / 1 / 2 / 2b / 3 / 3 封じ手 / 4）
...
- **Phase 4** 率計算を SQL から pure builder に剥離（本 commit）
...
- **Phase 5**: ViewModel / chart 薄化（`FreePeriodReadModel → ViewModel → Chart` の三段構造確立 + `YoYChart.tsx` の `ComparisonResolvedRange` 移行で Phase 3 封じ手 allowlist baseline を 1 → 0 に縮退）
