...
* [x] Step B 完了後: `SalesPurchaseComparisonChart` の raw data 依存を解消する — 設計変更により `ctx.freePeriodLane.bundle.fact` ではなく `ctx.storeDailyLane` sibling lane 経由で実現 (Phase 6.5-5a、storeDailyLaneSurfaceGuard baseline 2 → 1)
* [x] Step B 完了後: `YoYWaterfallChart` の raw data 依存を解消する — 設計変更により `ctx.freePeriodLane.bundle.{fact,deptKPI}` ではなく `ctx.categoryDailyLane` sibling lane 経由で実現 (Phase 6.5-5b、categoryDailyLaneSurfaceGuard baseline 13 → 6)
