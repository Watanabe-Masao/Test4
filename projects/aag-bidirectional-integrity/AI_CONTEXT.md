...
**active → MVP 完遂 (2026-04-30、Phase 3 hard gate B 確定 = AI 推奨 + ユーザー確認)**。
...
- **2026-04-30 Phase 3 hard gate (AI 推奨段階)**: AI 自主推奨 (deferred-decision-pattern §3.1) で **B 推奨 (Project A〜D 分割)** を articulate (audit report §7.2 rationale: scope = Level 4 寄り、commit `5e6193f`)
- **2026-04-30 Phase 3 hard gate (ユーザー確認段階)**: ✅ **B 確定** — ユーザー articulate「Bでよろしくお願いします」(deferred-decision-pattern §3.2 例外則、高 blast radius のためユーザー確認必須 gate)
- **MVP 完遂状態**: Phase 1 + Phase 3 + cyclic refinement で完了。Phase 4〜10 は Project A〜D に移管確定
- **2026-04-30 Project A〜D bootstrap 全完了**: 4 project の skeleton landing 済 (= `aag-core-doc-refactor` / `aag-rule-schema-meta-guard` / `aag-display-rule-registry` / `aag-legacy-retirement`)、project-health に全 4 project が `derivedStatus = in_progress` で登録済
- **次 session**: 各 project の Phase 1 着手 (依存順: Project A → Project B → Project C / A → D)、本 project は archive 候補に migrate (Project A〜D 全完了後に archive プロセス着手)

canonical 計画 doc は本 project の `plan.md`。parent: なし。後続: 4 project (`aag-core-doc-refactor` / `aag-rule-schema-meta-guard` / `aag-display-rule-registry` / `aag-legacy-retirement`、bootstrap 完了済)。
...
7. **`references/99-archive/adaptive-architecture-governance.md`** (旧 AAG マスター、Phase 4 で Split + Rewrite)
8. **`references/99-archive/aag-5-constitution.md`** (旧 4 層構造定義、Phase 4 で Rewrite + Relocate + Rename)
9. **`references/99-archive/adaptive-governance-evolution.md`** (旧進化動学、Phase 4 で Rewrite + Relocate + Rename)
...
| `references/99-archive/adaptive-architecture-governance.md` | 旧 AAG マスター (Phase 4 で Split + Rewrite + Relocate → aag/strategy.md ほか) |
| `references/99-archive/aag-5-constitution.md` | 旧 4 層構造定義 (Phase 4 で Rewrite + Relocate + Rename → aag/architecture.md) |
| `references/99-archive/adaptive-governance-evolution.md` | 旧進化動学 (Phase 4 で Rewrite + Relocate + Rename → aag/evolution.md) |
