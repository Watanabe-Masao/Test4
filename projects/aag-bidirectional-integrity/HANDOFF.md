...
main 上の `projects/phased-content-specs-rollout/checklist.md` line 158 が `[ ]` のままなら、
**次セッション開始時に Step 0 として archive 8-step を再実行**する必要あり:

1. line 158 を `[x]` flip (人間 review 完了 articulation)
2. mv `projects/phased-content-specs-rollout/` → `projects/completed/phased-content-specs-rollout/`
3. `projects/phased-content-specs-rollout/config/project.json`: `status="archived"`
