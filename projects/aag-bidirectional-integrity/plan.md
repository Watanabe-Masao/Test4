...
10. **破壊的変更前提**: 本 project は AAG の根本的整理を行うため、追加コスト / 変更コストを考慮せず必要な変更を遂行する。既存 AR-NNN rule の振る舞い変更 / 縦スライス境界の reshape / 層構造の調整 / AAG framework 構造変更 等を許容 (本体アプリの機能変更は scope 外)
...
| 4 | **検証** (Verification) | 外部監査 | health-rules.ts / Discovery Review / certificate / meta-guard / aag/meta.md §4 達成判定総括 / PR comment renderer / Constitution 改訂人間 review | 第三者視点の audit、AAG が claim と actual を受動的に照合すること |
...
- 役割が違うため独立 layer として articulate。現状の AAG では両者が混在している (例: meta-guard は実装としても検証としても機能) が、本 project で responsibility 分離を進める
...
### 8.4. Layer 3 と Layer 4 の境界 (Phase 3 audit + Phase 4 refactor の入力)
...
| §8.4 Layer 3-4 境界 | `references/02-status/aag-doc-audit-report.md` (Phase 3) + Phase 4 refactor note |
...
| §8.9 もう一押し候補 | Phase 1 / Phase 8 の implementation 着手前判断 (本 plan に decision 記録) |
