...
これらは AAG の 4 層構造（Constitution / Schema / Execution / Operations）には組み込まれて
...
4. **drill-down 構造は経営階層 metaphor**: 経営理念 → 社訓 → 経営戦略 → 戦術 (4 層、双方向 trace chain)
5. **既存 AAG Core 8 doc は内容を定義し直し、整理し直す必要がある** (sprawl + 重複 + 4 層位置付け不適合)
...
9. **4 層アーキテクチャ + 縦スライス** (本プロジェクト本体側 modular monolith evolution と parallel) を AAG architecture pattern として確立。詳細: §3.1
10. **破壊的変更前提**: 本 project は AAG の根本的整理を行うため、追加コスト / 変更コストを考慮せず必要な変更を遂行する。既存 AR-NNN rule の振る舞い変更 / 縦スライス境界の reshape / 4 層構造の調整 / AAG framework 構造変更 等を許容 (本体アプリの機能変更は scope 外)
...
- **AAG Core 8 doc** (本 project の整理対象、§3.5 で op 別判定):
  ...
  - `aag-5-constitution.md` (4 層構造定義 + 非目的、Rename + Relocate + 内容再定義候補)
...
11. **破壊的変更前提** (§1.2 #10): AAG の根本的整理に必要なら既存 AR-NNN rule の振る舞い変更 / 縦スライス境界 reshape / AAG framework 構造変更 / 5 縦スライス → 別構造 等を許容。Layer 0 を除き、AAG 内のあらゆる構造は変更可能
...
### 3.1. 4 層 drill-down 構造 + 縦スライス (AAG architecture pattern)

AAG は **4 層 (横軸) × 縦スライス (縦軸)** の matrix で articulate される。これはプログラミング
原則の **4 層アーキテクチャ + 縦スライス** (modular monolith evolution / clean architecture /
hexagonal、本プロジェクト本体側で既確立) を AAG 自身に適用した構造。経営階層 metaphor (経営理念
→ 社訓 → 経営戦略 → 戦術) は横軸の概念伝達 image であり、本 plan では正式用語を使用する。

#### 3.1.1. 4 層 drill-down (横軸)
...
| 2 | **設計** (Design) | 経営戦略 | AAG の構造方針 (4 層構造 / 製本-rule binding / 3 層サイクル / now/debt/review 運用区分) | 中期方針、how (structural) |
| 3 | **実装** (Implementation) | 戦術 | 個別 rule / guard / allowlist / health 閾値 / migrationRecipe | 具体施策、how (instance) |

drill-down は **双方向の trace chain**:

- 上位 → 下位 (how chain): 目的が要件として articulate され、要件が設計として realize され、設計が実装として instantiate される
- 下位 → 上位 (why chain): 各実装が「どの設計 / 要件 / 目的を realize しているか」を pointer で示せる
...
| Layer 3 実装 | layerBoundaryGuard / topologyGuard | calculationCanonGuard / canonicalizationSystemGuard / *PathGuard 群 | queryPatternGuard / analysisFrameGuard / comparisonScopeGuard | responsibilitySeparationGuard / responsibilityTagGuard / sizeGuard | architectureRuleGuard / docRegistryGuard / docCodeConsistencyGuard / health-rules.ts |
...
drill-down chain の多段拡張: AR-rule schema に下記 field を追加:

```typescript
canonicalDocRef: Array<{
...
これらは:
- **機械検証 + 維持される field** として確立 (Phase 8 meta-guard が articulation 必須化)
...
拡張に相当 (pointer のみ → pointer + problem + resolution)。
...
### Phase 1: AAG Meta doc (`aag/meta.md`) の新規創出
...
framework + AR-rule binding hub の 3 機能融合 mechanism doc) で確立する。

**deliverable**:
- `references/01-principles/aag/meta.md` 新規 Create (新 path、ディレクトリ階層化、§3.7 命名規則)
  - §1 目的 (Purpose、1-2 段落、Layer 0)
  - §2 要件 (Requirements、不変条件 + 禁則 table、各行に enforcing AR-rule + state-based 達成条件 + 達成 status)
  - §3 AAG Core 構成要素 mapping (Layer 2 doc + Layer 3 実装 を 4 層位置付けで整理)
...
**完了条件**: aag/meta.md skeleton landing + 人間 review 通過 (Constitution 改訂と同等の慎重さ) +
doc-registry 登録 + 既存 guard 全 PASS。**Phase 1 と Phase 3 audit は並行 refine 可能** (Meta が
audit 結果を反映して update する cyclic refinement、最終確定は Phase 3-4 完了後)。
...
drill-down chain の重複と参照の切り分け + 「下位が上位の何を課題として / 何を解決しているか」を
管理対象として確立。
...
- `guardCategoryMap.ts` の対応 field 追加 (または schema 統合)
- 既存全 rule に `canonicalDocRef: []` + `metaRequirementRefs: []` 空 array で初期化 (後続 Phase で順次埋める)
- TypeScript 型定義の整合
- (option) 既存 `principleRefs` の semantic 化検討 (pointer のみ → pointer + problem + resolution)、別 sprint で

**完了条件**: 既存全 rule が空 array で初期化、build / lint / 既存 guard 全て PASS。schema は
semantic articulation を必須 field として表現可能。
...
§3.4.3 semantic management の articulation field (problemAddressed + resolutionContribution) も
検証対象に含める。

**deliverable**:
- `canonicalDocRefIntegrityGuard.test.ts` (reverse): 各 AR rule の `canonicalDocRef` の各 entry に対して:
  - `docPath` 実在
  - 参照先 doc 内に rule ID 出現
  - `problemAddressed` / `resolutionContribution` non-empty (semantic 妥当性は人間 review)
- `canonicalDocBackLinkGuard.test.ts` (forward): canonical doc の `## Mechanism Enforcement` section の各 entry に対して:
  - 対応 AR ID が実在
  - 要件 ID が aag/meta.md §2 に実在
  - architect 寄与 articulation non-empty
- (option) `metaRequirementBindingGuard.test.ts`: 各 rule の `metaRequirementRefs` の各 entry に対して:
  - `requirementId` が aag/meta.md §2 に実在
  - `problemAddressed` / `resolutionContribution` non-empty
- 例外 allowlist の機械管理 (baseline、§3.9 純粋 mechanism rule)
...
**aag/meta.md §2 の双方向 integrity 要件 status が「未達成」→「達成」に flip**。
semantic articulation field が全 binding で non-empty。
