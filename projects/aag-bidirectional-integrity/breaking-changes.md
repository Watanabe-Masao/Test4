...
| 対象 | 変更内容 | 移行方針 | 影響範囲 |
|---|---|---|---|
| **`canonicalDocRef` field 追加** | **`app-domain/gross-profit/rule-catalog/base-rules.ts`** (BaseRule 物理正本) の rule entry に `canonicalDocRef: SemanticTraceBinding<CanonicalDocTraceRef>` (= `{ status: 'pending'\|'not-applicable'\|'bound', justification?, refs: Array<{ docPath, problemAddressed, resolutionContribution }> }`) を追加。型定義は `architectureRules/types.ts` (or `aag-core-types.ts`)。**`defaults.ts` (execution overlay) と `guardCategoryMap.ts` には置かない** (二重正本回避、PR review Review 3 P0 #1 + P1 #4 反映) | 既存全 rule に `{ status: 'pending', refs: [] }` で初期化、Phase 6 で分類 A から順次 `'bound'` に flip or `'not-applicable'` で justify。Phase 2 で schema migration | 既存 100+ AR-NNN rule、`architectureRules/types.ts`、`base-rules.ts`、`merged.ts` (consumer 経路)。**defaults.ts / guardCategoryMap.ts は不変** |
| **`metaRequirementRefs` field 追加** | **`base-rules.ts`** の rule entry に `metaRequirementRefs: SemanticTraceBinding<MetaRequirementTraceRef>` (= `{ status, justification?, refs: Array<{ requirementId, problemAddressed, resolutionContribution }> }`) を追加。`requirementId` は aag/meta.md §2 で landed する stable ID (`AAG-REQ-*`) | 既存全 rule に `{ status: 'pending', refs: [] }` で初期化、Phase 6 で binding 記入。Phase 8 meta-guard で検証 (status 整合性 + semantic 品質基準) | 既存全 rule、aag/meta.md §2、Phase 8 で landing する `canonicalDocRefIntegrityGuard.test.ts` + `statusIntegrityGuard.test.ts` |
| **`RuleBinding` 型への配置** (`architectureRules/types.ts`) | 上記 2 field を `RuleSemantics` でなく `RuleBinding` 側に配置 (PR review Review 3 P1 #3 反映、docPath はリポジトリ固有具体値のため Core 純粋型でなく App Domain 寄り) | 既存 `RuleBinding` 型に optional field 追加 (existing fields 不変) | `RuleBinding` 型 consumer 全数、ただし optional のため backward compat |
| **semantic articulation 品質基準** (Phase 8 で機械検証、hard fail / warning 分離) | hard fail = 禁止 keyword (TBD/N/A/same/see above 等) + 20 文字 minimum + 重複検出 + status 整合性 + path 実在 / warning = 意味的「それっぽい空文」(human review + sample audit + Discovery Review で扱う、機械では判定しない) | Phase 0.5 で品質基準確定、Phase 8 で `semanticArticulationQualityGuard.test.ts` 実装 (hard fail) + Discovery Review で意味品質補完 | semantic articulation を持つ全 rule entry |
