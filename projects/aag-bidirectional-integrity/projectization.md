...
| **2026-04-30** | **PR review Review 3 反映 (P0 3 + P1 4 + P2 1)**: schema 正本境界 + Phase 3 split hard gate + Phase 8 MVP 化 + human deletion approval + observeForDays 切り分け + Phase 0.5 完遂宣言 | (1) **P0 #1 schema 変更対象修正**: `defaults.ts` (execution overlay) + `guardCategoryMap.ts` (二重正本回避) は touch せず、BaseRule 物理正本 `app-domain/gross-profit/rule-catalog/base-rules.ts` + 型定義 `architectureRules/types.ts` / `aag-core-types.ts` + derived consumer `merged.ts` に修正。(2) **P0 #2 schema status object 統一**: 空配列でなく `SemanticTraceBinding<T>` (status: pending/not-applicable/bound + justification + refs)。(3) **P0 #3 Phase 3 split hard gate 化**: Phase 3 完了後の Phase 4〜10 を単一 project 継続 / sub-project / follow-up project 分割の判断を **hard gate decision** に格上げ、**default 推奨は MVP=Phase 1〜3 / Follow-up=Phase 4〜10 別 project 化**。Level 3 のまま開始するが audit findings により follow-up project 分割を default 推奨。(4) **P1 #4 RuleBinding 配置**: `RuleSemantics` でなく `RuleBinding` 側 (docPath は App Domain 寄り)。(5) **P1 #5 Requirement ID namespace**: aag/meta.md §2 で `AAG-REQ-*` stable ID list を articulate。(6) **P1 #6 Phase 8 MVP 化**: Layer 4 sub-audit を 4.2 方向 + 4.4 完備性 のみに絞る、4.1 / 4.3 / 4.5 + selfHostingGuard + metaRequirementBindingGuard は follow-up project 候補。(7) **P1 #7 quality 統合**: hard fail (構文整合) と warning (意味品質 human review) を分離。(8) **P2 #8 物理削除 human approval**: anti-ritual と orthogonal な安全装置として inbound 0 + 人間 deletion approval を必須化。(9) **重大指摘 #6 observeForDays 切り分け**: legacy doc 削除 = inbound 0 only / experimental rule 昇格・撤回 = observeForDays 許容 (supplementary signal)。(10) **改善 4 件**: §8.0 Phase 別 prerequisite 集約 table、Phase 4 sub-phase 4.1〜4.4 事前 articulation、Phase 6.1 初期 batch 別出し (articulation draft 生成 protocol)、Phase 0.5 完遂宣言 (再 open 不可)。 |
