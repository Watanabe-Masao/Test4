...
**Phase 0 Scaffold 完了 / Phase 1 未着手。status: `draft`。**
...
- `checklist.md` を Phase 0-7 + 最終レビュー checkbox で埋めた（Phase 0 は全 [x]）
- `HANDOFF.md` 本ファイルを customize（scaffold 完了）
- `aag/execution-overlay.ts` を draft 初期状態（空 overlay）で landed
- `inquiry/` ディレクトリを `.gitkeep` 付きで作成
- 派生セット（`pr-breakdown.md` / `review-checklist.md` / `test-plan.md` / `inventory/`）を `derived/` に landed

### Phase 1 着手前の残タスク

- なし（Phase 0 scaffold は landed 済み。次は Phase 1 inquiry の事実棚卸し）
...
詳細は `checklist.md` を参照。Phase 0 は landed 済み。次は Phase 1 inquiry kickoff。

### 高優先

- Phase 1 kickoff: `inquiry/01-widget-registries.md` から着手
  - 9 registry × 全 widget の網羅的棚卸し（事実のみ）
  - 各事実に出典（ファイルパス / 行番号 / commit hash）を付記
  - recommendations / 改修案を書かない（不可侵原則 #12）

### 中優先

- `inquiry/02-widget-ctx-dependency.md` 以降を `plan.md` §3 Phase 1 の成果物表に沿って順次着手
- Phase 1 期間中にコード変更を一切行わないことを定期的に `git log` で自己点検
...
| `inquiry/` | Phase 1-5 の成果物置き場（`.gitkeep` のみ。Phase 1 で 01-08 を追加） |
