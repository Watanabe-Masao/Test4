...
> **前提**: `config/project.json` の `status` フィールド（永続的な track 状態）と、
> `CURRENT_PROJECT.md` が指す作業対象（短命な working marker）は別のものとして扱う。
> **`CURRENT_PROJECT.md` は常に 1 project のみを指す**（単一作業対象の強制）。

- `status: "active"` と `CURRENT_PROJECT.md` は**独立した指標**として管理する
...
```jsonc
{
  ...
  "status": "active",  // umbrella の `status: "active"` と共存（CURRENT_PROJECT.md で作業対象を区別）
  ...
}
```
...
| `projects/architecture-debt-recovery/inquiry/*.md` | Phase 1-5 の成果物（21 ファイル想定: 01-21） |
...
`RuleExecutionOverlayEntry` 型（`app/src/test/aag-core-types.ts` の `RuleOperationalState`）が受理するフィールドのみ。

- `fixNow` — `now` / `debt` / `review` の分類
- `executionPlan` — effort / priority
- `reviewPolicy` — owner / lastReviewedAt / reviewCadenceDays
- `lifecyclePolicy` — `introducedAt` / `observeForDays` / `promoteIf` / `withdrawIf`（experimental の出口）

> **補足**: `ruleClass` / `confidence` は App Domain 側の `RuleIdentity`、`sunsetCondition` は App Domain 側の `RuleGovernance` に属する。Project Overlay に置こうとすると `RuleExecutionOverlayEntry` の型定義で拒否される。これら 3 フィールドは Phase 6 の Architecture Rule 正本登録時に App Domain 側で埋める。

Phase 3 `inquiry/12-principle-candidates.md` は、App Domain 側と Project Overlay 側の両フィールドの**候補**を明示的に区別して埋めた状態で提示する。人間承認後に Phase 6 で正本登録する。
