...
- judgementCommit: `<本 commit sha>`
- preJudgementCommit: `21bd4ad` (= R2 完了 + DA-α-003 Lineage update commit)
- judgementTag: `aag-self-hosting-completion/DA-α-004a-judgement`
- rollbackTag: `aag-self-hosting-completion/DA-α-004a-rollback-target` (= `21bd4ad` に landing 済)
- implementationCommits:
  - `<本 commit sha>` — R3a 全実装 (= 5 directory rename + 05-contents → 04-tracking/elements merge + ~580 inbound update + doc-registry / manifest / guard / collector path constants update + DA-α-004a entry landing)
