...
6. `decision-audit.md`（DA-α-000 進行モデル + ADR-SCP-001〜015 mapping、計 15 ADR + 7 GUIDANCE 系列）
...
10. `aag/scp-checkers/README.md`（§A2 boundary protection 4 件の articulate + 実装方式 = declarative YAML + common runner、ADR-SCP-015）
11. 必要に応じて `aag/_internal/strategy.md` / `architecture.md` / `evolution.md`（AAG 戦略 + 5 層構造 + 進化方針）
12. 必要に応じて `references/05-aag-interface/operations/project-checklist-governance.md` / `projectization-policy.md`（AAG Layer 4A 共通運用）
13. 必要に応じて `projects/active/reposteward-ai-ops-platform/AI_CONTEXT.md`（substrate 提供 program、Task Capsule / Parameters / SourceFacts / DetectorResult の入力）
