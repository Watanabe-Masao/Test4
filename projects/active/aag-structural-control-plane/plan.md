...
| sub-PR | 名称 | 内容 |
|---|---|---|
| **Phase 2A** | Structural Skeleton declaration | tree-contracts.yaml を 1 entry 雛形 → top-level 8 root の declared 状態に拡張（元 Phase 3 で実施予定だった skeleton 宣言を前倒し）。**planned 8 以外の既存 top-level directory は declared にしない**（Ideal-first 原則）— `app-domain/` / `fixtures/` / `roles/` / `scripts/` / `workers/` / `docs/` 等は **存在を理由に skeleton へ declared しない**。Phase 2C で out-of-skeleton candidate として surface し、user 判断により promote / move / archive / tolerate / delete-candidate / revise-skeleton に分類（AAG-SCP-MEANING-002 + DESIGN-001/002 = 存在 ≠ 必要性の証明）。例外: dotfile dirs (`.github/` / `.claude/` / `.vscode/`) は external-tool-managed 整合の明確な rationale があるため Phase 2A で unmanaged-but-tolerated として articulate（promotionRationale 必須）。 |
