...
### 4.5. CHANGELOG.md 更新 + バージョン管理 (= AAG version 完全分離 で institute、本 MVP 期間中の user feedback で articulation 修正)
...
- **詳細**: DA-α-013 (= AAG version separation institute)
- **scope 外 (= post-archive 別 program 起票候補、discovery-log P2 articulate)**:
  - `aag-changelog-vertical-obligation-guard` (= AAG-tagged project は `aag/CHANGELOG.md` 更新を mechanical 強制する guard)
