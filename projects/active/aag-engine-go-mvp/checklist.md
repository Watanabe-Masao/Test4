...
* [ ] `fixtures/aag/` 配下 8 fixture の `input.json` を Go 側で読める
* [ ] `expected.json` と actual DetectorResult[] を deep equality で比較できる
* [ ] pass / fail fixture の差分を machine-readable に出力できる
...
* [ ] `project-lifecycle/pass-active` fixture で 0 violation
* [ ] `project-lifecycle/fail-completed-not-archived` fixture で AR-PROJECT-LIFECYCLE-C1 を 1 件 emit
* [ ] active / completed v1 / completed v2 圧縮済 の 3 状態すべてで read 経路が動作 (= readiness inventory §1 + §4.3 articulate を Go で再現)
* [ ] Archive v2 圧縮済 project に対して `plan.md` / `checklist.md` 等の物理存在を前提にした read を行わない (= false positive 0 件)
* [ ] DA-α-007 entry を articulate する

## Phase 8: Generated Metadata Detector (advisory)

* [ ] `generated/fail-stale-metadata` fixture で AR-GENERATED-METADATA-G2 を **advisory severity** (= warn) で 1 件 emit
* [ ] hard gate / block-merge severity では emit しない (= advisory 限定)
* [ ] CI hard fail を引き起こさない
