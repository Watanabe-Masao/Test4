...
* [ ] 各 v2 T:kind frontmatter に親 plan.md §OCS.2 Evidence Level が登録されている
* [ ] 各 v2 T:kind frontmatter に親 plan.md §OCS.5 Promotion Gate（初期 L1 Registered）が登録されている
* [ ] 各 v2 T:kind frontmatter に親 plan.md §OCS.4 Lifecycle status（初期 `proposed` または `active`）が登録されている
