...
| `buildDailyDecomp` | `FactorDecompositionPanel` | `domain/calculations/factorDecomposition` 連携 + DailyDecomp shape |
| `buildSalesVsCostData` | `features/purchase/ui/PurchaseVsSalesChart` | 累計 + 原価率（costToSalesRatio）= 業務指標 + 文字列化 |

#### 2026-04-21 再評価（Phase A Step 3 調査結果）

上記 5 件を詳細 inspect した結果、**分離を保留** が妥当と判断:

- これらは **既存 domain 計算（`calculateShare` / `calculateGrossProfitRate`）の
  オーケストレーション**であり、**新規 authoritative kernel ではない**。日毎ループ
  で accumulator を回す薄い処理でしかない。
- 出力の `ChartRenderModel<Point[]>` 形状は presentation 契約そのもの。
- `pure-calculation-reorg` の鉄則（contract ID + `calculationCanonRegistry` 登録 +
  Zod + invariant tests）を満たす「新規計算」としては scope がない。
- 仮に分離するとしても、作成されるのは「日単位 accumulator の汎用
  utility」であり、registry 登録対象ではない（non-authoritative helper）。

**判断:** これらは presentation に留める。Phase A Step 3 は実施しない。
真に domain 昇格すべき候補は別軸で再検出（例: 計算式自体を持つ関数、
not 既存関数のループ呼び出し）。
