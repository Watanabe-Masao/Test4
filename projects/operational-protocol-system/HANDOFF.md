...
**Phase 0 (Bootstrap) のみ landed**。AAG Platformization Pilot 完遂 (2026-05-02 archive) を trigger に、user articulation で post-Pilot 運用制度として独立 project に bootstrap (本 commit)。
...
- 実装 0 件 → **Phase M1 から articulation 開始**
...
不可侵原則 1 違反。本 project は AAG 上に薄く載せる operational layer のみ。AAG 内部 (`references/01-principles/aag/` + `aag/core/`) は **touch しない**、参照のみ。drawer (`references/03-guides/decision-articulation-patterns.md`) も改変しない、引用のみ。
...
| `references/03-guides/projectization-policy.md` | AAG-COA |
| `references/03-guides/new-project-bootstrap-guide.md` | bootstrap 手順 |
