...
6. **Phase K 実装** (中規模、Option 1+2 推奨) — checklist Phase K 参照
   - Option 1: cadence 廃止 + `lastVerifiedCommit` 強化（commit-pin 自動同期で stale spec 検出）
   - Option 2: reviewed claim に `verificationNote` 必須化（test 化しない理由の rationale 強制）
   - 着手前に plan.md §Phase K として canonical 化が必要

7. **UIC-004/005 ChartCard 単独 story 作成** — baseline 8 → 6 (中 cost、全 chart wrapper drift 防御)
   - state overlay (loading / error / empty / ready) 4 状態を story で固定
   - UIC-004 cover で UIC-005 も同時 cover 可能（ChartCard 内部で ChartLoading 使用）

8. **CALC test 紐付け** (Phase D 残) — **Phase K Option 2 完了後に rationale 化**
   - 残 reviewed claim 5 件 (CALC-013/014/016/018/020 CLM-004) は機械的昇格 anti-pattern
   - Option 2 landing 後、`verificationNote` 列に「なぜ test 化しないか」を 1-2 文記入で完了
...
### 3.9. 90 日 review cadence は儀式 — 既存防御で代替済
...
/ lifecycle / evidence-level) が構造的 drift を網羅し、`lastVerifiedCommit` が既に
concrete signal を提供している。次セッションで Phase K (Option 1+2) を実装し、
cadence を機械的 commit-pin に置換する設計が landed (2026-04-29、commit b872f5f)。

**やってはいけないこと**: 「reviewed claim を機械的に tested に昇格」する作業を
追加し、test 化しにくい claim に無理やり test を書く（anti-pattern、CALC-013/014
CLM-004 等の横断的意味整合 / 表現規約 / 定数固定 claim は性質上 test 単独で表現不能）。

**やるべきこと**: Phase K Option 2 の `verificationNote` 列を導入し、reviewed claim
に「なぜ test 化しないか」の explicit rationale を強制する。それで漏れ・忘却を防ぐ。

### 3.10. pre-push 警告「principles.json 未更新の可能性」は generated section の false positive

`docs:generate` が `references/01-principles/adaptive-architecture-governance.md` の
GENERATED:START/END マーカー内 timestamp を更新するため、本ファイルの diff が出るたびに
pre-push hook が `docs/contracts/principles.json` 更新を提案する。原則定義が変わって
いない限り false positive。次セッションで principles.json 同期 mechanism の精度を
上げる（GENERATED マーカー外の変更だけを trigger 化）改善余地あり。
