...
| 2026-04-29 | **scope reduction (anti-bloat self-test 適用)**: 25 未着手 checkbox → keep 12 + defer 5 + vacuous 1 + cut 6 + human gate 1 に再構成。cut 内訳: 全 chart cover (58 中 5 spec、selection rule 意図的限定で不可侵原則 3 違反 risk) / 全 UIC cover (26 中 5 spec 同様) / state 別 story 管理 (schema 拡張 cost > value) / Promotion Gate L6 (per-tag tracking 過剰) / PR comment bot (1 人 project で過剰)。defer は #2 content graph collector が prerequisite (5 件 auto-resolve)。HANDOFF を Tier 1/2/cut 構成に再整理。Phase Q reduction (integrity-framework-evolution) と同思想を本 project に一貫適用 |
