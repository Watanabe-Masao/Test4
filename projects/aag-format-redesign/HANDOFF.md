...
project ディレクトリを bootstrap し、Phase 0「痛点棚卸し」を完了した段階。
`AI_CONTEXT.md` / `plan.md` / `checklist.md` / `config/project.json` /
`aag/execution-overlay.ts` / `pain-points.md` (P-01〜P-09) /
`strengths.md` (S-01〜S-13) を作成済み。`CURRENT_PROJECT.md` を本 project に
切替済み。
...
次は Phase 1「現状調査」— `pain-points.md` の各痛点に対応する現行コードの
箇所を特定し、`current-state-survey.md` にまとめる。
...
- Phase 1: `current-state-survey.md` を作成し、`pain-points.md` 各痛点
  （P-01〜P-09）に対応する現行コードの箇所（ファイル名・行）を特定する
- Phase 1: `_template/` / `executionOverlayGuard` / `project-resolver.ts` /
  `vite.config.ts` / `vitest.config.ts` / project-health collector の
  解決経路を図解する

### 中優先

- Phase 2: 新フォーマット設計（project 文書側） — 必須セットと派生セット
  の境界を `new-format-design.md` で定義
- Phase 3: 新フォーマット設計（AAG overlay 側） — overlay bootstrap 緩和の
  3 案比較を `overlay-bootstrap-design.md` で
- Phase 4: サブプロジェクト機能設計（P1） — `subproject-design.md`
