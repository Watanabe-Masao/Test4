...
**Phase 1 / 2 実装 + 手動検証完了。Phase 3 β（撤退条件の明文化）/ Phase 4
（ドキュメント更新）完了。** 本 project は archive 可能状態。`selectCtsWithFallback` /
`selectCtsWithFallbackFromPair` の実際の削除は leaf-grain 正本新設（後継 project
`category-leaf-daily-series`）の完了を待つ。
...
## 1.2. Phase 3 β 事前準備 — 撤退条件の明文化

`selectCtsWithFallback` / `selectCtsWithFallbackFromPair` の完全削除は本 project では
完遂できない。理由と撤退条件を以下に固定する。

### 現状（Phase 2 完了時点）

| シンボル | 本番利用 | 備考 |
|---|---|---|
| `selectCtsWithFallback` | 0 件 | 定義とテストのみ。本番 consumer なし |
| `selectCtsWithFallbackFromPair` | 2 箇所（useDayDetailPlan）| `prevDayRecords` / `cumPrevRecords` を合成 |

### `prevDayRecords` / `cumPrevRecords` の consumer

- `DayDetailSalesTab` → `DrilldownWaterfall`（要因分解、Shapley）
- `DayDetailSalesTab` → `CategoryDrilldown`（部門×line×klass ドリルダウン）
- `DayDetailHourlyTab` → `HourlyChart` の `hourDetail`（選択時間帯のカテゴリ別内訳）

これらは **dept\|line\|klass の leaf-grain key** を要求する。
`categoryDailyLane.bundle` は dept 粒度までしか持たず、`timeSlotLane.bundle` は
leaf-grain / 量 / 時間帯 / 店舗の複合粒度を持たないため、現 bundle 契約では代替不能。

### 撤退条件（後継 project の completion 条件）

以下が **全て成立** した時点で `selectCtsWithFallback` / `selectCtsWithFallbackFromPair` を
安全に削除できる。

1. `CategoryLeafDailySeries`（または同等の leaf-grain 正本契約）が新設されている
2. `DrilldownWaterfall` / `CategoryDrilldown` / `HourlyChart.hourDetail` の 3 consumer が
   leaf-grain 正本経由に載せ替わっている
3. `useDayDetailPlan` が `prevDayRecords` / `cumPrevRecords`（raw CTS）を返す必要が
   なくなっている
4. `timeSlotLaneSurfaceGuard` 相当の surface guard（leaf-grain 版）で raw
   CTS の presentation 直接 import を 0 件に ratchet-down 完了している

### 回帰防御（Phase 2 完了時点で整っている機構）

- `timeSlotLaneSurfaceGuard.test.ts` — `StoreAggregationRow` の presentation 直接 import を 0 件で固定
- `HourlyChart.builders.test.ts` — `buildHourlyDataSets` が `TimeSlotSeries` を受け取ることを型 + 実値で固定
- `buildHourlyData` は presentation 内に定義を残すが、HourlyChart の時間帯主経路からの
  呼び出しは無し（builders.ts は bundle 経由のみ）

### 部分完了として残る Phase 3 / 4 の位置付け

- **Phase 3**: 「撤退判定」は本 project の範囲内で明文化完了（本セクション）。
  「削除実行」は後継 leaf-grain project の範囲とする
- **Phase 4**: `runtime-data-path.md` / `data-pipeline-integrity.md` を bundle 経由化
  後の状態に更新（本 commit で実施）
...
| `projects/category-leaf-daily-series/HANDOFF.md` | 後継 project（Phase 3 削除実行の移管先）|
