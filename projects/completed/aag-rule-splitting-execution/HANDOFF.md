...
| `app-domain/gross-profit/rule-catalog/base-rules.ts` | 7 AR-RESP-* ルールの物理正本 |
| `app/src/test/architectureRules/rules.ts` | 後方互換 re-export facade |
| `app/src/test/guards/responsibilitySeparationGuard.test.ts` | G8 の現行実装（ruleId ベース） |
| `app/src/test/allowlists/responsibility.ts` | AR-RESP-MODULE-STATE 例外管理 |

---

Archived: 2026-04-13

7 ルール (AR-RESP-STORE-COUPLING / MODULE-STATE / HOOK-COMPLEXITY / FEATURE-COMPLEXITY
/ EXPORT-DENSITY / NORMALIZATION / FALLBACK-SPREAD) は base-rules.ts に定義済み。
responsibilitySeparationGuard.test.ts は getRuleById ベースで 7 種のパターンを個別
ruleId で集計している。AR-RESP-MODULE-STATE に 4 件の permanent structural 例外
(wasmEngine / calculationWorker / uiPersistenceAdapter / swUpdateSignal) が再帰属済み。
他 6 ルールは baseline 0 で固定。旧 AR-STRUCT-RESP-SEPARATION は削除済み。
