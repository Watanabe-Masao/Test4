...
| DA-α-004d | Phase R3d | guard / collector path + doc-registry / manifest + 新 guard landing 判断 | active |
| DA-α-005 | Phase R4 | per-element directory + dashboard layer + element taxonomy 判断 | active |
...
- 判断根拠:
  ...
  - 事実 3: 不可侵原則 7「起動・archive 判断は user 領域、AI 単独で起動・archive しない」と整合 (= AI 単独進行 + 最終 archive user 承認)
...
| date | event | 対応 |
|---|---|---|
| 2026-05-02 | bootstrap 直後の構造案で `aag/interface/*` (= drawer / protocols / operations) を articulate、ただし「主アプリ改修 user not read」と articulate した aag/ 配下に「主アプリ改修 userも使う」doc を配置 = 境界矛盾 (= 物理配置から AI 誤読 risk) を user 指摘で identify | forward-fix: `aag/interface/*` → `references/05-aag-interface/*` に articulate update、aag/ は framework 本体に純化 (`_internal/` + `_framework/`)。plan.md §3 R2 + §5 関連実装 path table 更新済 (本 commit) |
...
| 2026-05-02 | **doc audit + 4 系統 lens articulation** (= user articulation 反映): R-phase 進行中に発見した doc 改善 candidate を **live tracking artifact** で accumulate、R3c で batch 解消する flow。整理 lens として **doc 4 系統 (ログ / メトリクス / 手順書 / チェックリスト)** articulate (= active project directory (`projects/<active>/` R6 後) は作業 lens + 進行に必要な最低限の状態のみ、メトリクス本体は references/04-tracking/、手順は references/05-aag-interface/ + plan.md、混同禁止) | forward-fix: `projects/completed/aag-self-hosting-completion/doc-improvement-backlog.md` 新設 (= live tracking)、P1 (= 用語統一 + 旧構造前提 doc 最適化 + ハマりポイント refine) を R-phase 内吸収 (R2 + R3c + R6)、P2-P3 (= 構造化 + 重複削減 + reader taxonomy) は post-R7 別 program candidate。R3c deliverable に「doc-improvement-backlog 反映 (= P1 batch 解消)」追加 |
...
- 判断根拠:
  ...
  - 事実 4: R0 が landed すれば R1 以降の物理移動が始まっても AI / userが物理配置を見て「読む / 読まない」即判断可能 (= 後続 phase の判断 base 確立)
...
- 振り返り観測点 (5 点):
  ...
  - R0-2 (subjective、honest skip articulate): 「主アプリ改修 user read OK / not read」が file 名 / directory 名から判断可能 — **subjective で機械検証不能**、R6 selfHostingGuard 拡張で代替検証
...
- 想定リスク:
  - 最大被害: aagBoundaryGuard sub-invariant (b) (= aag/ 配下 主アプリ改修 user向け doc 検出) で false positive。mitigation = ALLOWLIST 経由
...
- 振り返り観測点 (5 点、machine-verifiable):
  ...
  - R2-5 (反証): synthetic で aag/ 配下に主アプリ改修 user向け doc 配置試行 → fail
...
## DA-β-001: pre-push hook 役割分離 (= 軽量 / 変更箇所 + 影響範囲 / negative articulation)
...
R3b commit 後の tag push 時、`check_format_added_files` が `merge-artifact-generator.ts` (= 過去 commit で導入された新規 file) を ERROR 検出して push が止まった。**`PUSH_RANGE = origin/main...HEAD` 展開** によって、本 commit と無関係の過去違反が発火した。

User articulation (= 4 message 連鎖、本 turn):
1. Pre-push hook は変更箇所、影響範囲に絞る (= 全スキャンは CI、AI 不可侵のインフラ ガード)
2. 必要な test のみに動的に絞れたらいい (= 選択と集中)
3. 緩和ではなく選択と集中で精度向上
4. **何をチェックするかではなく、何をチェックしないか** が大事

= **negative articulation** (= exclusion-driven design) が core insight。AAG 5.5 「製本されないものを guard 化しない」の対称形。

### 判断時 (2026-05-03 / β stream / R3b 完了直後)

- 候補:
  1. **A. Skip-rules manifest (= negative)**: `tools/git-hooks/pre-push-skip-rules.json` で changed files path pattern → どの check を skip するか宣言。default = full check (= safe fallback)。skipBaseline で ratchet-down
  2. **B. Impact-scope (= positive)**: 各 check 関数に scope 絞り込み logic (= vitest --related, eslint changed files, tsc incremental, prettier --check changed)
  3. **C. Hybrid**: skip でまず除外 → 残りに impact-scope 適用
- 採用案: **A + 部分的 B** (= skip-rules.json で negative、+ check_format / check_format_added_files は changed-files に絞る = 軽い positive)
- 判断根拠:
  - 事実 1: User articulation の core = **何を check しないか** (negative-driven)
  - 事実 2: skip-rules.json は JSON + shell glob で実装可、複雑度 低 (= 即着手可能、blocking 解消の即時効果)
  - 事実 3: vitest --related は guard test には適用困難 (= guard は global state を読むため依存解析で漏れる)、tsc -b は元々 incremental なので絞り込み済
  - 推論: A を baseline、B は format check のみ追加適用が cost-effective
- 不可侵原則 (= AAG-self-hosting-completion 不可侵原則 8 件) 整合:
  - 1: 主アプリ code touch なし (= tools/ + .git/hooks/ のみ)
  - 2a: AAG framework articulate 内容不変
  - 2b: AAG framework 構造変更なし (= 既存の hook structure 維持、check 関数は不変、wrap で skip 評価追加)
  - 3: drawer 内容不変
  - 4: 既存 functionality 維持 (= skip rules が 0 件なら現状と等価動作)
  - 5: DA-β-001 単独 atomic commit
  - 6: 観測点 5 件 machine-verifiable
  - 8: operational-protocol-system project boundary 維持 (= 本 reform は β stream、α stream に介入しない)
- **「pre-push 通過 = CI 通過」原則の articulation 緩和**:
  - 旧: pre-push 通過は CI 通過を保証 (= AI session が CI 待たずに confidence 持って push)
  - 新: pre-push 通過は **影響範囲内** での失敗予測のみ。Full-scope ガードは CI が単独 authoritative
  - 理由: 過剰防衛による noise + AI session velocity 阻害 (= R3c-R7 の連続 commit/push で累積問題)
  - trade-off: 「CI で初めて落ちる」体験が許容される代わりに、各 commit の push が **影響範囲精度の保証** に焦点化

### 想定リスク

- 最大被害: skip-rule の overmatching で **必要な check が skip され**、本来なら pre-push で止まる違反が CI で初めて落ちる
  - mitigation: skipBaseline guard で skip rules 件数を baseline=3 で固定、増加要求は review window 経由 (= AI が単独追加できない)
  - mitigation: skip-rules は **explicit-only** (= 一致しない場合は skip しない default 動作)
- 二番目: skip-rules.json と pre-push hook の semantic drift (= skip rule 名と check 関数名の不一致)
  - mitigation: skipRulesGuard で skip rule の `skip` 配列値が hook の関数名 registry に存在することを機械検証

### 振り返り観測点 (5 点、machine-verifiable)

- β1 (machine-verifiable): `tools/git-hooks/pre-push-skip-rules.json` 実在 + JSON parseable
- β2 (machine-verifiable): pre-push hook が skip-rules を読み込んで evaluate (= dry-run で skip 動作確認)
- β3 (machine-verifiable): skipBaseline guard が skip rules 件数を baseline 比較 (= `skipBaseline = 3`)
- β4 (machine-verifiable): doc-only commit (= `references/**/*.md` のみ変更) で `check_tsc / check_lint / check_test_guards` skip 動作 (= dry-run check)
- β5 (machine-verifiable): `npm run test:guards` 全 test PASS (= guard 改修後の global health 維持)

### 5 軸 articulation

- **製本** (canonical): `tools/git-hooks/pre-push-skip-rules.json` が canonical (= AI 編集可、skipBaseline guard で機械検証)
- **依存方向**: 上位 = AAG ratchet-down 文化、下位 = R3c-R7 全 commit の push velocity
- **意味**: 「pre-push hook は CI のミニチュアではなく、**影響範囲フォーカス** の即時 feedback layer」
- **責務**: skip-rules manifest 新設 + pre-push hook に skip 評価追加 + skipBaseline guard 新設
- **境界**: 本 DA-β-001 内 = pre-push hook のみ、外 = pre-commit hook (= 別 mechanism)、CI (= AI 不可侵)

### Skip-rules schema (= negative manifest 仕様)
  ...
  "skipRules": [
    {
      ...
      "matchAll": "^(references|projects|aag|docs)/.*\\.md$",
      "skip": ["check_tsc", "check_lint", "check_test_guards"],
      "rationale": "doc-only 変更は code 影響なし → CI に集約"
    },
    {
      "name": "test-additions-only",
      "matchAll": "^app/src/.+\\.test\\.tsx?$",
      "skip": ["check_docs_drift"],
      "rationale": "test 追加は KPI に影響しないため docs:check 不要"
    },
    {
      "name": "format-cleanup-only",
      "matchAllPattern": "format|prettier",
      "skip": ["check_test_guards"],
      "rationale": "format-only commit は test 影響なし"
    }
  ]
}
```

- `matchAll`: 全 changed file が regex 一致した場合のみ skip (= 1 file でも外れたら skip しない、安全側 default)
- `skip`: skip する check 関数名の配列 (= hook の registry と sync 必須、guard で機械検証)
- `rationale`: 設計判断のuser-readable articulation (= why we don't check)
...
- judgementCommit: `58d1577`
- preJudgementCommit: `5d04fe0` (= DA-β-002 Lineage update commit)
- judgementTag: `aag-self-hosting-completion/DA-β-003-judgement` (= `58d1577` に landing 済)
- rollbackTag: `aag-self-hosting-completion/DA-β-003-rollback-target` (= `5d04fe0` に landing 済)
- implementationCommits:
  - `58d1577` — DA-β-003 全実装 (= _template/discovery-log.md schema + 5 active project discovery-log + PZ-14 guard + projectization-policy + project-checklist-governance §3.3 + guard-test-map + regen)
...
## DA-α-004d: R3d guard / collector path + doc-registry / manifest update + decisions/ 新設 + 新 guard 2 件 landing 着手判断

**status**: active

### 判断時 (2026-05-03 / Phase R3d)

- 候補:
  1. R3d を 1 commit で完結 (= 3 active code file + doc-registry + decisions/ 新設 + 2 新 guard を atomic)
  2. R3d を sub-phase 化 (= path update / decisions 新設 / 新 guard を別 commit)
- 採用案: 候補 1
- 判断根拠:
  - 事実 1: 残 active code 3 file (= 既存 R3a で update済みでない部分)、scope 小
  - 事実 2: doc-registry.json 旧 path 3 mention のみ、scope 小
  - 事実 3: decisions/ 新設は user 判断置き場の一般 mechanism (= recent-changes.generated と分離、user 判断 lineage を articulate するための独立 directory、AAG 4 系統 lens の「ログ」系統に属する)
  - 事実 4: 2 新 guard (= generatedFileEditGuard / oldPathReferenceGuard) は ratchet-down baseline=0 で landing、本 program 後半 phase の保護に必須
  - 事実 5: 不可侵原則 5「partial migration 禁止、同 commit で完結」と整合
- 想定リスク:
  - 最大被害: oldPathReferenceGuard の baseline=0 で archive 例外 whitelist 漏れ → archive doc が hard fail。mitigation = `99-archive/` + `projects/completed/` を whitelist で明示除外
  - 二番目: generatedFileEditGuard の検出が test ファイル (= guard test 自身) を誤検出。mitigation = `*.generated.md` suffix をもつ実 file のみ trigger
- 振り返り観測点 (5 点、machine-verifiable):
  - R3d-1: 残 3 active code file 全 path update 後 grep 0 件 (= archive 除く)
  - R3d-2: doc-registry.json 旧 path 0 mention (= archive 除く)
  - R3d-3: `references/01-foundation/decisions/` 存在 + README.md (= 役割 articulate)
  - R3d-4: `generatedFileEditGuard.test.ts` + `oldPathReferenceGuard.test.ts` 両 PASS、baseline=0
  - R3d-5: `npm run test:guards` 全 PASS (= 既存 + 新 2 guard)

### 5 軸 articulation

- **製本** (canonical): 本 R3d で landed する 2 guard が canonical (= 機械検証 active)、`references/01-foundation/decisions/` README.md が canonical (= directory 役割 articulate)
- **依存方向**: 上位 = R3a-R3c 完了、下位 = R4 (= per-element directory) 着手前提
- **意味**: 「旧構造完全撤退の機械化 + user 判断 lineage の独立 articulation」
- **責務**: 残 active code path update + doc-registry update + decisions/ landing + 2 guard landing
- **境界**: 本 R3d 内 = active scope のみ、外 = archive (= projects/completed/ + 99-archive/) は touch なし (= 不可侵原則 5 + drawer 不可逆性)

### Commit Lineage

- judgementCommit: `ee41603`
- preJudgementCommit: `4551b5e` (= DA-β-003 Lineage update commit)
- judgementTag: `aag-self-hosting-completion/DA-α-004d-judgement` (= `ee41603` に landing 済)
- rollbackTag: `aag-self-hosting-completion/DA-α-004d-rollback-target` (= `4551b5e` に landing 済)
- implementationCommits:
  - `ee41603` — R3d 全実装 (= 3 active code path update + decisions/ 新設 + generatedFileEditGuard + oldPathReferenceGuard + guard-test-map 登録 + regen)
...
## DA-α-005: R4 per-element directory + dashboard layer + element taxonomy 着手判断

**status**: active

### 判断時 (2026-05-03 / Phase R4)

- 候補:
  1. R4 を 1 commit で完結 (= taxonomy + index + dashboard 4 件 + per-element pilot を atomic)
  2. R4 を sub-phase 化 (= pilot subset 移行 / dashboard 新設 / taxonomy 制定 を別 commit)
- 採用案: 候補 1
- 判断根拠:
  - 事実 1: pilot subset (= charts/ 5 element) は scope 限定、5 file → 5 directory + 4 doc 各 (= 25 file 新設、scope 中規模)
  - 事実 2: dashboard 4 件は initial skeleton (= 内容 enrichment は段階別、本 commit 範囲は landing のみ)
  - 事実 3: element-taxonomy.md は 5 prefix の正本制定 (= 単一 doc、scope 小)
  - 事実 4: 不可侵原則 5「partial migration 禁止、同 commit で完結」と整合
  - 事実 5: pilot subset で value verify → 後段階 (R6 / R7 / post-archive) で他 category に展開、本 commit は **mechanism landing** が主目的
- 想定リスク:
  - 最大被害: per-element directory 化で既存 `charts/CHART-NNN.md` への inbound link が break。mitigation = git mv で history 保持 + grep で inbound link verify + bulk update
  - 二番目: dashboard.generated.md の collector が未実装で empty 状態のまま (= 機械生成 mechanism 不在)。mitigation = R4 では skeleton landing のみ、collector 実装は post-archive 別 program candidate (= scope 外 articulate)

### 振り返り観測点 (5 点、machine-verifiable)

- R4-1: pilot subset (= charts/ 5 element) の per-element directory 5 件 全完成 + 各 4 doc 存在
- R4-2: dashboard layer 4 doc skeleton (= 全 `.generated.md` suffix) landed
- R4-3: `element-taxonomy.md` で ID prefix 5 種類 (WID/CHART/ENG/PAGE/FLOW) articulated
- R4-4: `element-index.generated.md` skeleton landed (= 機械生成 mechanism 未実装でも file 存在)
- R4-5: `npm run test:guards` 全 PASS (= 既存 + R4 影響範囲)

### 5 軸 articulation

- **製本** (canonical): `element-taxonomy.md` (= ID prefix 正本) + 各 element directory `README.md` (= 手書き正本) + `*.generated.md` (= 機械生成正本)
- **依存方向**: 上位 = R3 完了、下位 = R5 (= operational-protocol-system resume) + R6 (= self-hosting closure) + R7 (= 統合 guard)
- **意味**: 「element の物理境界 + ID prefix 正本 + dashboard layer = 横断的 metrics 集約場の articulation」
- **責務**: pilot subset 移行 (= mechanism verify) + 5 prefix 正本 + 4 dashboard skeleton
- **境界**: 本 R4 内 = pilot subset (= charts/ 5 element) + skeleton + taxonomy、外 = 全 89 element 一括移行 (= drawer Pattern 5 意図的 skip + rationale: 段階適用) + dashboard collector 実装 (= post-archive 別 program candidate)

### Commit Lineage

- judgementCommit: `a1703a8`
- preJudgementCommit: `0f273f4` (= R3d 後 docs:generate 反映 commit)
- judgementTag: `aag-self-hosting-completion/DA-α-005-judgement` (= `a1703a8` に landing 済)
- rollbackTag: `aag-self-hosting-completion/DA-α-005-rollback-target` (= `0f273f4` に landing 済)
- implementationCommits:
  - `a1703a8` — R4 全実装 (= element-taxonomy + element-index skeleton + dashboards/ 4 件 skeleton + charts/ 5 element per-element directory pilot + inbound update + regen)
...
## DA-α-006: R5 operational-protocol-system project resume + plan refinement trigger 着手判断

**status**: active

### 判断時 (2026-05-03 / Phase R5 / scope 縮小済)

- 候補:
  1. **A**. R5 を本 program 内で operational-protocol-system M1-M5 全 deliverable landing まで実施 (= 旧 plan)
  2. **B**. R5 scope 縮小 (= operational-protocol-system project resume articulation + plan refinement trigger のみ、M1-M5 実装は operational-protocol-system project 内で別途) (= 不可侵原則 8 整合)
  3. **C**. R5 を skip して R6 直行
- 採用案: **B** (= self-evaluation 反映で plan §3 R5 articulate 済)
- 判断根拠:
  - 事実 1: operational-protocol-system project は **independent project** (= 別 directory + DA institution 独立、本 project R0 で配置済 + 独自 plan.md / decision-audit.md 持ち)
  - 事実 2: 不可侵原則 8 = 「operational-protocol-system project boundary 維持、本 program 内で M1-M5 実装すると boundary 違反」
  - 事実 3: operational-protocol-system は paused 状態 (= 旧 path `aag/interface/protocols/` 前提で plan articulated、本 R5 で resume 可能 = 新 path `references/05-aag-interface/protocols/` に refinement 必須)
  - 事実 4: 候補 C (= skip) は user articulation の「flow が articulated 整合」を満たさない (= aag-self-hosting-completion 完了 → operational-protocol-system 再 plan + M1-M5、という flow を user 確定)
  - 推論: 候補 B が scope discipline + 不可侵原則 8 + flow 整合 を全て満たす
- 不可侵原則 (= 8 件) 整合:
  - 1: 主アプリ code touch なし
  - 2a: AAG framework articulate 内容不変
  - 2b: AAG framework 構造変更なし
  - 3: drawer 内容不変
  - 4: 既存 functionality 維持
  - 5: R5 単独 atomic commit
  - 6: 観測点 5 件 machine-verifiable
  - **8**: operational-protocol-system project boundary 維持 (= 本 commit で同 project 内 trigger artifact のみ touch、M1-M5 deliverable は scope 外)

### deliverable

- `projects/active/operational-protocol-system/HANDOFF.md` update:
  - **PAUSED** → **READY TO RESUME** articulate (= aag-self-hosting-completion R0-R4 + R5 完了で resume 条件 satisfied)
  - 旧 path 参照 (= `aag/interface/protocols/` / `references/03-implementation/` for protocols) を新 path (= `references/05-aag-interface/protocols/`) に refine
- `projects/active/operational-protocol-system/plan.md` update:
  - 物理配置 path を新構造に refine (= `references/03-implementation/` for protocols → `references/05-aag-interface/protocols/`)
  - skeleton 既 landing 済 articulate (= R2 で `protocols/README.md` landed、M1-M5 で fill のみ残)
- `projects/active/operational-protocol-system/config/project.json` `$comment` update (= 新 path articulate)

### 想定リスク

- 最大被害: operational-protocol-system project resume 後、新 path 前提で M1 着手するが旧 path 残置で混乱
  - mitigation: 本 R5 で全 path refinement 完了、operational-protocol-system 内で grep 0 件 verify
- 二番目: 本 R5 が「trigger only」と articulated だが具体 M1 着手判断が operational-protocol-system DA に丸投げで進行 stuck
  - mitigation: operational-protocol-system project 内 DA-α-001 (= 既存 placeholder) を resume 直後 articulate、M1 着手 trigger は 同 project の責任、本 program R5 完了後 user 判断で起動

### 振り返り観測点 (5 点、machine-verifiable)

- R5-1: `projects/active/operational-protocol-system/HANDOFF.md` で PAUSED articulation 撤去
- R5-2: operational-protocol-system 配下に `aag/interface/protocols/` 文字列 0 件
- R5-3: operational-protocol-system 配下で M1-M5 deliverable target path = `references/05-aag-interface/protocols/` articulated
- R5-4: `references/05-aag-interface/protocols/README.md` skeleton 維持 (= R2 landing 状態保持)
- R5-5: `npm run test:guards` 全 PASS

### 5 軸 articulation

- **製本** (canonical): operational-protocol-system 配下 (= `HANDOFF.md` / `plan.md` / `config/project.json`) が canonical (= trigger artifact)
- **依存方向**: 上位 = R0-R4 完了、下位 = operational-protocol-system project の M1-M5 着手判断 (= 本 program 完了後 user 判断で起動)
- **意味**: 「operational-protocol-system project が新構造前提で resume 可能 articulation + plan refinement trigger」
- **責務**: trigger artifact 3 file の path refinement のみ (= scope 縮小 articulated)
- **境界**: 本 R5 内 = operational-protocol-system project の trigger artifact のみ、外 = M1-M5 実装 (= operational-protocol-system project 内で別途、不可侵原則 8 整合)

### Commit Lineage

- judgementCommit: `4b346a0`
- preJudgementCommit: `5735680` (= R4 Lineage update commit)
- judgementTag: `aag-self-hosting-completion/DA-α-006-judgement` (= `4b346a0` に landing 済)
- rollbackTag: `aag-self-hosting-completion/DA-α-006-rollback-target` (= `5735680` に landing 済)
- implementationCommits:
  - `4b346a0` — R5 全実装 (= operational-protocol-system HANDOFF resume articulate + plan.md M1-M5 path refinement + config/project.json $comment update + regen)
...
## DA-α-007a: R6a AAG self-hosting closure articulate update + selfHostingGuard expansion 着手判断

**status**: active

### 判断時 (2026-05-03 / Phase R6a / R6 sub-divided)

**R6 sub-division 経緯**: 元 R6 plan は「AAG self-hosting closure update + projects/ split + template migrate」3 件を 1 commit。実 scope 観測 = projects/ split 単独で **225 inbound + 6 active project 物理移動 + discovery 関数 update + _template/ migrate** = 大規模。drawer Pattern 1 (Commit-bound Rollback) integrity を保つため R3 と同じ pattern で sub-divide:

- **R6a** (= 本 entry): AAG self-hosting closure articulate update + selfHostingGuard expansion (= 軽量、5 軸 articulation 中心)
- **R6b** (= DA-α-007b): projects/ active+completed split + _template migrate + CURRENT_PROJECT.md guard (= 大規模、別 atomic commit)

- 候補:
  1. R6 を 1 commit で完結 (= 旧 plan)
  2. **R6 を sub-phase 化** (= R6a + R6b、各 atomic commit)
- 採用案: 候補 2
- 判断根拠:
  - 事実 1: projects/ split 単独で 225 inbound update + discovery 関数 (= projectizationPolicyGuard / projectDocConsistencyGuard / project-resolver.ts 等 ~3-5 file) update が必要 = scope 大
  - 事実 2: R3 で同 pattern (= R3a-R3d sub-divide) を採用、各 atomic commit で rollback integrity 維持に成功
  - 事実 3: R6a (= meta.md update + guard expansion) は scope 限定、R6b (= 物理移動) と分離可能
  - 推論: sub-divide で各 commit の scope discipline + rollback granularity を確保
- 不可侵原則 (= 8 件) 整合:
  - 1: 主アプリ code touch なし
  - 2a: AAG framework articulate **内容 update** (= 不可侵原則 2 R6 例外、self-hosting closure 達成 articulation 更新は本 program scope 内)
  - 2b: AAG framework 構造変更なし (= meta.md 内 articulate 更新のみ、構造不変)
  - 3: drawer 内容不変
  - 4: 既存 functionality 維持 (= guard expansion は新 check 追加、既存 check 不変)
  - 5: R6a 単独 atomic commit
  - 6: 観測点 5 件 machine-verifiable
  - 8: operational-protocol-system project boundary 維持

### deliverable

- `aag/_internal/meta.md` §2.1:
  - AAG-REQ-SELF-HOSTING を **「code-level + entry navigation rigor 完全達成」** articulate update
  - self-hosting closure 達成根拠 articulate (= R0-R5 で landing した構造的 mechanism + ratchet-down baseline=0 guard 群)
- `app/src/test/guards/selfHostingGuard.test.ts` 拡張:
  - **entry navigation rigor 検証項目追加** (= aag/ + references/ boundary integrity の集約 check):
    - aag/ 配下に主アプリ改修 user 向け doc が 0 件 (= aagBoundaryGuard 統合 mention)
    - references/ 配下に AAG framework 内部 doc が 0 件 (= aagBoundaryGuard 統合 mention)
    - `*.generated.md` の手編集 0 件 (= generatedFileEditGuard 統合 mention)
    - 旧 path 残置 0 件 (= oldPathReferenceGuard 統合 mention)
  - 各検証項目は既存 guard への delegation で実装 (= 重複検証回避)

### 想定リスク

- 最大被害: meta.md update で AAG-REQ-* / canonicalDocRef integrity が破壊 → selfHostingGuard hard fail
  - mitigation: meta.md update は articulate refinement のみ (= 内容追加、既存項目 untouch)、各 update 後 selfHostingGuard 即時 verify
- 二番目: selfHostingGuard 拡張で baseline=0 を超える違反検出
  - mitigation: 拡張 check は既存 guard へ delegate articulation のみ (= 新 check logic は既存 guard に既 landing 済、selfHostingGuard は articulate 集約のみ)

### 振り返り観測点 (5 点、machine-verifiable)

- R6a-1: meta.md §2.1 で AAG-REQ-SELF-HOSTING articulate update 反映 (= grep で「entry navigation rigor」 / 「完全達成」 articulation 確認)
- R6a-2: selfHostingGuard.test.ts に **entry navigation rigor articulation** が landing (= articulate 内容追記、既存 check 不変)
- R6a-3: `npm run test:guards` 全 PASS (= 既存 + selfHostingGuard articulate update)
- R6a-4: `aagBoundaryGuard` / `generatedFileEditGuard` / `oldPathReferenceGuard` baseline=0 維持 (= 既存 enforcement 不変)
- R6a-5: meta.md changelog 行が **本 commit を articulate** (= 履歴記録)

### 5 軸 articulation

- **製本** (canonical): `aag/_internal/meta.md` §2.1 (= AAG-REQ canonical) + `selfHostingGuard.test.ts` (= self-reference closure 機械検証)
- **依存方向**: 上位 = R0-R5 完了 (= entry navigation rigor 達成基盤)、下位 = R6b (= projects/ split = self-hosting 構造的最終形)
- **意味**: 「self-hosting closure を **code-level (= 既達成 2026-05-01) + entry navigation rigor (= R0-R5 で達成)** の 2 layer で完成 articulation」
- **責務**: meta.md articulate update + selfHostingGuard articulate 集約 (= 既存 guard 群への pointer)
- **境界**: 本 R6a 内 = articulate update + guard articulation 拡張のみ、外 = projects/ 物理移動 (= R6b)、新 logic 追加 (= 既存 guard 群で十分、本 R6a で新規 logic landing なし)

### Commit Lineage

- judgementCommit: `cd76e1d`
- preJudgementCommit: `c68791f` (= R0-R5 + hot fix + CI fix merged to main、branch synced)
- judgementTag: `aag-self-hosting-completion/DA-α-007a-judgement` (= `cd76e1d` に landing 済)
- rollbackTag: `aag-self-hosting-completion/DA-α-007a-rollback-target` (= `a72134a` に landing 済)
- implementationCommits:
  - `cd76e1d` — R6a 全実装 (= meta.md §2.1 update + selfHostingGuard Test 4 拡張 + guard-test-map update + regen)
...
## DA-α-007b: R6b projects/ active+completed split + _template migrate + CURRENT_PROJECT.md guard 着手判断

**status**: active

### 判断時 (2026-05-03 / Phase R6b / R6 sub-divided 後半)

R6a (= self-hosting closure articulate + selfHostingGuard expansion) 完了で **code-level + entry navigation rigor articulation** が完成。R6b では **物理 structural separation の最終形** を実装:

- `projects/` を `active/` + `completed/` に split (= active project の物理位置を articulate で明示)
- 6 active project を `projects/active/<id>/` に git mv
- 75 inbound reference を bulk sed で update
- discovery 関数 (= projectizationPolicyGuard / projectDocConsistencyGuard / project-resolver.ts) を新構造対応に update
- `CURRENT_PROJECT.md` pointer-only guard 機械検証

候補 / 採用案:
- 候補 1: R6b を 1 commit で完結 (= 物理移動 + bulk sed + discovery update + guard を atomic に)
- 候補 2: R6b を sub-phase 化 (= 移動 / inbound update / guard を別 commit)
- **採用**: 候補 1 (= partial migration 禁止、不可侵原則 5 整合)

判断根拠:
- 事実 1: 6 active project + 75 inbound = scope 中規模、bulk sed で機械的に処理可能
- 事実 2: discovery 関数 update を移動より先に実施しないと test:guards が一時的に fail する → 同 commit 内で順序保証
- 事実 3: 不可侵原則 5「partial migration 禁止」+ drawer Pattern 1 (Commit-bound Rollback) integrity を保つため atomic commit が必須
- 推論: 候補 1 が rollback granularity と migration integrity を両立

### 不可侵原則 (8 件) 整合

- 1: 主アプリ code touch なし
- 2a: AAG framework articulate 内容不変 (= 物理移動は metaframework の structural separation 強化、内容追加なし)
- 2b: AAG framework 構造変更なし
- 3: drawer 内容不変
- 4: 既存 functionality 維持 (= test:guards 全 PASS、inbound link は移動と同 commit で update)
- 5: R6b 単独 atomic commit + partial migration 禁止
- 6: 観測点 5 件 machine-verifiable
- 8: operational-protocol-system project boundary 維持 (= 物理位置は移動するが project 内 articulation 不変)

### 観測点 (5 件、machine-verifiable)

- R6b-1: `projects/active/` directory 存在 + 6 active project 全件移動完了
- R6b-2: `projects/<id>/` 直下 (= 旧 location) は 0 件 (`_template` / `completed` / `README.md` / `active` を除く)
- R6b-3: bulk sed inbound update 0 残置 (= `projects/(aag-self-hosting-completion|...)/` grep で archive 例外除き 0 件)
- R6b-4: discovery 関数 (= projectizationPolicyGuard / projectDocConsistencyGuard) PASS (= active project 6 件全 discoverable)
- R6b-5: `npm run test:guards` 全 PASS

### 5 軸 articulation

- **製本** (canonical): `projects/active/<id>/` 物理 layout が canonical (= structural separation 完成形)
- **依存方向**: 上位 = R6a 完了 (= articulate 完成)、下位 = R7 (= 統合 guard + archive)
- **意味**: 「active vs completed の物理 separation で active project boundary を structural articulate」
- **責務**: 6 project 物理移動 + 75 inbound update + discovery 関数 update + CURRENT_PROJECT.md guard
- **境界**: 本 R6b 内 = active project 移動 + 関連 update のみ、外 = archive 移行 (= R7) / new mechanism (= post-R7)
