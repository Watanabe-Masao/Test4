...
- [x] **Project B Phase 1〜5 + archive 完遂** (= `projects/completed/aag-rule-schema-meta-guard/`、2026-05-01): Phase 1〜5 全完遂後ユーザー承認 (本 commit) により 8-step archive プロセス完了。SemanticTraceBinding 型 family 5 件 + RuleBinding 拡張 2 field / 166 rule 全 bound articulation (batch 1+2 人手 + batch 3 synthesizer) / 4 meta-guard MVP (fixed-mode、baseline=0 invariant) / 134 file 901 test PASS / 5 follow-up sub-audit (4.1 境界 / 4.3 波及 / 4.5 機能性 / selfHostingGuard / metaRequirementBindingGuard) を `ar-rule-audit.md §6` で follow-up project candidate に articulate。Project A〜D 4 project 中 3 件目 archive 完遂、**Project C (aag-display-rule-registry) unblock**
...
- [x] **Project C Phase 1〜4 + archive 完遂** (= `projects/completed/aag-display-rule-registry/`、2026-05-01): Project B Phase 4 完了で循環 fail risk 解消後、Phase 1 (DFR registry doc landing = `references/01-principles/aag/display-rule-registry.md`、DFR-001 chart semantic color + DFR-002 axis formatter + DFR-003 percent + DFR-004 currency + DFR-005 icon を Layer 2 articulate) → Phase 2 (5 rule entry を base-rules.ts に bound articulation 装着 + defaults.ts + execution-overlay.ts + guardCategoryMap.ts co-update) → Phase 3 (displayRuleGuard 実装、5 test、baseline ratchet-down = DFR-001=7 / 002=4 / 003=1 / 004=0 fixed / 005=20、135 file 906 test PASS) → Phase 4 (`aag/meta.md §2 / §4` で **5 AAG-REQ status flip** = AAG-REQ-LAYER-SEPARATION + SEMANTIC-ARTICULATION + ANTI-DUPLICATION + NON-PERFORMATIVE + BIDIRECTIONAL-INTEGRITY、達成 6→11 件 / 残 1 件 = SELF-HOSTING)。**AAG bidirectional integrity の最初の concrete instance 成立**。本 archive で **Project A〜D 4 project 全 MVP scope 完了 milestone** 到達 — Project E (DecisionTrace + AI utilization、9 insight 統合) candidate spawn judgment trigger
...
| `references/99-archive/adaptive-architecture-governance.md`  | 旧 AAG マスター (Phase 4 で Split + Rewrite + Relocate)                                       |
