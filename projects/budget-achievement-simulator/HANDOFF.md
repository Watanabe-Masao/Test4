...
- 既存 widget 実装 (`app/src/features/budget/ui/BudgetSimulatorWidget.tsx`) と
...
- **Phase A**: `AI_CONTEXT.md` / `HANDOFF.md` の widget 方針化（本 commit で完了）
- **Phase B**: `mockBudgetSimulatorScenario.ts` 追加と widget 入力契約の再確認
- **Phase C**: `BudgetSimulatorView.tsx` 切り出し + HTML モック準拠レイアウト再整備
- **Phase D**: state 管理の接続見直し（mock scenario 前提で破綻しないこと）
