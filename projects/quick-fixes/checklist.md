...
* [ ] (低) `references/02-status/technical-debt-roadmap.md`: P1〜P9 の改善 project 表の各セクション冒頭に `[履歴]` または `(完了)` バッジを追加する。AAG 5.1 release 後の外部レビューで「冒頭の banner で live task を持たないと宣言したのに本文が live に見える」と指摘された経緯。完全分離 (`technical-debt-history.md` への分割) は破壊的すぎるため軽量な追記で対応する
