...
### Phase D Wave 1 (低結合系、in progress 2026-04-28)
...
| `canonicalizationSystemGuard.test.ts` の inline ディレクトリ走査  | 未着手 (D-W1 残)                                                                 | —                  |
| `testContractGuard.test.ts` の inline JSON.parse + token 走査     | 未着手 (D-W1 残)                                                                 | —                  |
| `scopeJsonGuard.test.ts` の inline JSON.parse + path prefix 走査  | 未着手 (D-W1 残)                                                                 | —                  |

**動作同一性検証 (calc canon)**: 15 既存 test 全件 PASS (動作同一性 = drift 検出強度不変)。
新 primitive `tsRegistry` (Phase D Wave 1 着手 1 番手) を新設、`Record<string, TEntry>` を `Registry<TEntry>` にラップ (副作用なし、入力 record の所有権を奪わない)。

第 2 describe (意味分類ガード Phase 2) は domain primitive で表現できない業務 specific 検証 (semantic class consistency / wasmEngine truth-layer / contractId mapping) のため scope 外。Phase D-E では touch しない。
