...
### Phase D Wave 2 (高結合系、in progress 2026-04-28)
...
| `taxonomyInterlockGuard.test.ts` 等の vocabulary parsing        | 未着手 (D-W2 残)                                                                                                              | —                  |
| `documentConsistency.test.ts` の principles 部分                | 未着手 (D-W2 残)                                                                                                              | —                  |

**動作同一性検証 (smoke guard)**: 9 既存 test 全件 PASS。
新 primitive `cardinality` (Phase D Wave 2 着手 1 番手) を新設、`checkUniqueness` /
`checkUpperBound` / `checkNonEmpty` / `checkSizeEquality` の 4 関数を提供。

`architectureRuleGuard.test.ts` (823 行) は本 wave では touch せず、Phase D Wave 2 後続で扱う。
