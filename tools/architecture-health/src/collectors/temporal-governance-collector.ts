...
export function collectFromTemporalGovernance(repoRoot: string): HealthKpi[] {
  ...
  const withReviewPolicy = (rulesContent.match(/reviewPolicy: \{/g) || []).length
  kpis.push({
    ...
    owner: 'architecture',
    docRefs: [
      { kind: 'definition', path: 'references/03-guides/architecture-rule-system.md' },
    ],
  ...
  const heuristicGateMatches = [
    ...rulesContent.matchAll(/ruleClass: 'heuristic'[\s\S]*?severity: '(\w+)'/g),
  ]
  ...
  for (const file of allowlistFiles) {
    ...
    try {
      ...
      const withCreatedAt = [
        ...content.matchAll(
          /lifecycle: 'active-debt'[\s\S]*?createdAt:/g,
        ),
      ].length
  ...
  kpis.push({
    ...
    owner: 'architecture',
    docRefs: [],
    implRefs: allowlistFiles.map(
      (f) => `app/src/test/allowlists/${f}`,
    ),
  })
