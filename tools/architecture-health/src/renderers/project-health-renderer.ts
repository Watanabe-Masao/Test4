...
export interface ProjectHealthSnapshot {
  ...
  readonly projects: readonly {
    ...
    /**
     * 本 project に紐づく生成文書への参照（ADR-D-005 PR2）。
     * umbrella project のみ remediation.{json,md} を持つ。
     */
...
export function buildProjectHealthSnapshot(
  ...
  const summary = {
    ...
    completedNotArchived: results.filter((r) => r.derivedStatus === "completed")
      .length,
  ...
  const projects = results.map((r) => {
    // ADR-D-005 PR2: umbrella project（現状 architecture-debt-recovery のみ）は
    // 生成された remediation 文書への参照を公開する。
    ...
    return {
      ...
      total: r.total,
      progress:
        r.total === 0 ? 0 : Math.round((r.checked / r.total) * 1000) / 1000,
 ...
 */
export function renderProjectHealthMdContent(
  snapshot: ProjectHealthSnapshot,
): string {
  const lines: string[] = [];
  ...
  lines.push("|---|---|");
  lines.push(
    `| active project 数（archive 未実施を含む） | ${snapshot.summary.active} |`,
  );
  ...
  lines.push(
    `| checkbox 空 (placeholder / 立ち上げ直後) | ${snapshot.summary.empty} |`,
  );
  lines.push(
    `| collection (continuous, 終わらない) | ${snapshot.summary.collection} |`,
  );
  lines.push(
