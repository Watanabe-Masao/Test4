...
const INDICATOR_GROUPS: readonly { name: string; ids: readonly string[] }[] = [
  {
    name: '例外圧',
    ids: ['allowlist.total', 'allowlist.frozen.nonZero', 'allowlist.active.count'],
  },
  {
    name: '後方互換負債',
  ...
  {
    name: '複雑性圧',
    ids: ['complexity.nearLimit.count', 'complexity.hotspot.count', 'complexity.vm.count'],
  },
  {
    name: '境界健全性',
...
export function detectTopRisks(report: HealthReport, maxItems = 3): RiskItem[] {
  ...
  const scored = report.kpis
    ...
    .map((k) => {
      const overBudgetPct = k.budget && k.budget > 0
        ? ((k.value - k.budget) / k.budget) * 100
        : 0
  ...
  return scored.slice(0, maxItems).map(({ kpi }) => {
    const reason = kpi.budget !== undefined
    ...
    const defDoc = kpi.docRefs.find((d) => d.kind === 'definition' || d.kind === 'roadmap')
    const definition = defDoc ? defDoc.path + (defDoc.section ? ` #${defDoc.section}` : '') : '—'
