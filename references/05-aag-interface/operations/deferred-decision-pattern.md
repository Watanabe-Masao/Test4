...
- 「複数選択肢を articulate するが、判断基準が embed されていない」 → 後任 AI / 後任 userが判断軸を見失う
- 「user 判断とラベル付けて先送りする」 → 判断材料の収集が後任に委ねられ、orientation コスト増
...
## 3. AI 自主判断 vs user 判断の分離

deferred decision は基本 AI 自主判断だが、以下 2 種類は **user 判断のまま残す**:
...
### 3.2. user 判断 (例外、安全装置として残す)

- **構造的安全装置** (= AI 誤判断のコストが高い、安全網が必要)
- **規約による必須化** (= project lifecycle 規約等でuser approval が義務)
- 例: 物理削除 trigger (anti-ritual と orthogonal なuser deletion approval)、最終レビュー (archive obligation 発火 gate)

user 判断項目は doc に **「user 判断必須、AI 判断しない」** と明示 articulate。
...
|---|---|---|---|---|---|
| Phase X 着手前 | <項目> | AI session ID / user | A/B/C | <criteria に対して collected material をどう適用したか> | <SHA> |

decision log は **後任 AI / userが trace + revisit 可能** な形で project 内 (例: checklist.md 末尾) に保持。
...
3. **AI 自主判断とuser 判断を曖昧にしない** (= 各項目に「AI 判断 / user 判断」を明示 articulate)
