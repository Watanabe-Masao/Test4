...
| AI 自己レビュー section (= 最終レビュー手前) | — | 任意 | 任意 | ✅ | ✅ |
| `discovery-log.md` (= scope 外発見蓄積) | ❌ | ❌ | ✅ | ✅ | ✅ |
| `decision-audit.md` (= L3 重変更 routing で DA institute 必須、`complexity-policy.md` §3.4) | ❌ | ❌ | 任意 | 推奨 | 推奨 |
| `inquiry/` | ❌ | ❌ | 任意 | ✅ | ✅ |
...
- 推奨 = guard 強制はないが、該当 routing (= L3 重変更 routing 等) が発生したら必要、static articulation は強く推奨
- 任意 = 作っても作らなくてもよい
- — = 該当しない
...
| PZ-14 | Level 2+ project なのに `discovery-log.md` が存在しない、または schema 違反 (= `## priority` / `## 発見済 entry` section 欠落) | implemented (hard fail、DA-β-003 で institute) |
