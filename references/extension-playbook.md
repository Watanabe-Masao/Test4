...
| 対象 | 閾値 | 理由 |
|---|---|---|
| 全体（domain + application + infrastructure） | 55% | 既存ベースライン |
| `domain/calculations/**` | 70% | 数学的不変条件を持つ計算ロジック。回帰リスク最大 |
| `application/usecases/explanation/**` | 60% | StoreResult 参照の正確性が監査対象 |

### 引き上げスケジュール（目安）

| 時期 | 全体 | domain/calculations | explanation |
|---|---|---|---|
| 現在 | 55% | 70% | 60% |
| +1ヶ月 | 55% | 75% | 65% |
| +2ヶ月 | 60% | 80% | 70% |
