...
7. **自動修正しない** — Signal Integrity rule は違反を検知して止めるが、テスト意味論を自動修正しない。修正はuser または 作業 AI が `why` / `protectedHarm` / `steps` を読んだ上で行う
...
- `references/03-implementation/test-signal-integrity-advisory.md` — Advisory レーン運用ガイド (trigger / 文面 / 自己点検 / 昇格条件)
- `references/03-implementation/architecture-rule-system.md` — Architecture Rule の運用手順
