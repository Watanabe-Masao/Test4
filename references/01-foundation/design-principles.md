...
> 本ドキュメントは CLAUDE.md §設計原則（9カテゴリ A〜H + Q）の詳細・適用例を記載する。
> カテゴリ一覧と要約は CLAUDE.md を参照。
> 旧体系との対応は `references/99-archive/principle-migration-map.md` を参照（アーカイブ）。
>
> **カテゴリ体系:**
> A. 層境界（A1-A6） / B. 実行エンジン境界（B1-B3） / C. 純粋性と責務分離（C1-C9） /
> D. 数学的不変条件（D1-D3） / E. 型安全と欠損処理（E1-E4） / F. コード構造規約（F1-F9） /
> G. 機械的防御（G1-G8） / H. Screen Runtime（H1-H6） / Q. Query Access Architecture（Q3-Q4）
>
> 以下の番号付き原則（1〜19）は上記カテゴリの具体的な適用例・詳細解説である。
...
R: タグで責務を宣言し、複数タグ = AND = 分離候補として可視化する。

**カタログ:** `references/03-implementation/responsibility-separation-catalog.md` に 24 パターンを整理。
**ガード:** `responsibilitySeparationGuard.test.ts`（G8）で機械的に検出。

---

### 21. 現実を正しく把握することが最優先（C9）
...
**実装:** `responsibilityTagRegistry.ts` + `responsibilityTagGuard.test.ts`
- 未分類 SNAPSHOT で現在値を固定。新規ファイルで増えたら CI 失敗
- 分類時は中身を確認し、正直にタグを付ける
