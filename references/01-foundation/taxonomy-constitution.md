...
- **どう守るか**: `AR-TAXONOMY-COGNITIVE-LOAD`。registry の R:_ または T:_ が 15 を超過すると fail または review required
...
### 8.1. 原則 6 (Antibody Pair) の enforcement

原則 6「Antibody Pair を持つ」は **AR-TAXONOMY-\* rule 経路を取らない**。理由は次の通り:

- AR-TAXONOMY-\* は **タグ使用箇所の検証**（ファイルが正しくタグ付けされているか）
- Antibody Pair は **registry 構造そのものの検証**（pair 不在の active タグが存在しないか）
- 両者は意味論的に異なる検査軸
...
> **TSIG 観点の note**: 「すべての原則が AR-TAXONOMY-\* rule で守られる」と誤認すると 原則 6 の enforcement gap を見落とす（H1 False Green リスク）。本節 §8.1 の存在によって "原則 6 は別経路" が明示的に文書化される。`constitutionBootstrapGuard.test.ts` B8 はこの mapping の網羅性を双方向で検証する。
...
4. **四半期 review window が 2 回以上記録**
   - `references/04-tracking/taxonomy-review-journal.md` に独立した 2 回の window record
...
| `references/04-tracking/taxonomy-review-journal.md`              | review window 記録（Phase 2 で skeleton landing）                |
| `references/04-tracking/generated/taxonomy-health.json`          | health KPI（Phase 3 + Phase 4 で landing）                       |
