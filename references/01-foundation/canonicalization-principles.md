...
1. **定義書**（`references/01-foundation/xxx-definition.md`）
...
> （詳細は `references/03-implementation/integrity-pair-inventory.md §2`）の派生原則。
> Phase I で `references/03-implementation/canonicalization-checklist.md` として
...
- 横展開候補リスト: `references/03-implementation/integrity-pair-inventory.md §2.2`
- 共通 primitive: 同 §3
- Phase A 採用候補リスト: 同 §4
- Phase B 後の機械強制: `app-domain/integrity/` + Phase I の checklist

## P9: 撤退規律は「step 5 直接到達」を default とする

> 2026-04-28 追加。`canonicalization-domain-consolidation` Phase B〜E の判例蓄積
> (Phase B-6 / Phase C / Phase D-W1〜W3 / Phase E、合計 13 件中 12 件が step 5
> 直接到達で完遂) により institutionalize。撤退規律 5 step を本則とする旧設計を
> 「特殊条件下の手順」に格下げし、in-place migration を default に昇格する。

### default: step 5 直接到達 (in-place migration)

単一 guard 内で **inline 重複 logic → domain primitive 経由** に同 PR で同時切替し、既存 test を変更せず PASS させることで動作同一性を担保する。`legacy-retirement.md §7` に actual sunset 日付のみ記録する。

### 5 step (並行運用 → 観察期間 → @deprecated → caller 0 → 物理削除) を要する条件

以下のいずれかに該当する場合のみ 5 step フルコースを採用:

- **(a) 公開 API**: 旧 file が internal でなく外部 (npm package / cross-repo) から import されている
- **(b) 別 file への論理移動**: 同 file 内 logic 入れ替えではなく、新 file を立てて旧 file を sunset する case
- **(c) 多数 caller (≥ 5 file)**: 旧 file への import が 5 file 以上で、同時切替が PR 規模を超過する case

(a)〜(c) のいずれにも該当しない場合は **step 5 直接到達が正規の手順**。「観察期間が必要」という直感に従って 5 step を選んではならない (儀式コストが migration 価値を上回る)。

### 5 step の正式定義 (条件 (a)/(b)/(c) 該当時のみ)

| step | 旧 guard | 新 domain guard | 検証 |
| --- | --- | --- | --- |
...
各 PR で 1 step だけ進める (step 1〜4 を一気に進めない、観察期間を確保するため)。

### 適用対象

- **registry+guard ペアの旧経路撤退** — `canonicalization-domain-consolidation` Phase B〜E
- **将来の domain extraction project** — 同形 pattern (散在 logic → 共通 domain primitive 抽出) を踏襲する project 全般
