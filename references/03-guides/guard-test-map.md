...
| `app/src/test/guards/comparisonProjectionContextImportGuard.test.ts` | architecture | 3件 | `features/comparison/application/` 配下での `PeriodSelection` import を禁止し `buildComparisonProjectionContext.ts` の 1 ファイルのみ allowlist（phase-6-optional-comparison-projection Phase O2、migration baseline は Phase O4/O5 完了時に 0 到達） |
