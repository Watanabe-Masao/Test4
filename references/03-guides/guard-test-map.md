...
| `app/src/test/guards/unifiedWidgetContextNoPageLocalOptionalGuard.test.ts` | architecture | 4件 | U1/U2: `UnifiedWidgetContext` の `<Page> 固有` section 内 optional field を ratchet-down 管理。baseline=5 (insightData / costDetailData / selectedResults / storeNames / onCustomCategoryChange)。ADR-A-001 PR4 で page-specific ctx 型へ剥離後 baseline=0 固定モード移行（architecture-debt-recovery SP-A ADR-A-001 PR1） |
