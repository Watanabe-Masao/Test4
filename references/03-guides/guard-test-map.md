...
| `app/src/test/guards/shortcutPatternGuard.test.ts` | architecture | 2件 | S1: Dashboard widget registry の `isVisible: (ctx) => ctx.X?.` のような optional chaining gate（二重 null check pattern の起点）の出現数 ≤ baseline=6。ADR-B-001 PR2 で type narrowing 活用、PR3 で discriminated union 化後 gate 削除 → PR4 で baseline=0 fixed mode（widget-registry-simplification SP-B ADR-B-001 PR1） |
