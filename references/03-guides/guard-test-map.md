...
| `app/src/test/guards/registryInlineLogicGuard.test.ts` | architecture | 2件 | I1: Dashboard widget registry の `registry*.tsx` 配下 IIFE pattern `(() =>` の出現数 ≤ baseline=0 fixed mode。ADR-B-003 PR2 で 3 selector 抽出、PR3 で 4 call site 置換、PR4 (2026-04-26) で fixed mode 到達 + LEG-009 sunset。新規 IIFE は registry に書かず application/readModels/<source>/selectors.ts に pure selector として抽出する（widget-registry-simplification SP-B ADR-B-003 PR1-PR4） |
