...
| `app/src/test/guards/unifiedWidgetContextNoDashboardSpecificGuard.test.ts` | architecture | 2件 | DS1: `UnifiedWidgetContext` の `Dashboard 固有` section 内 optional field を ratchet-down 管理。baseline=20。ADR-A-002 PR3a-d で 4 registry を DashboardWidgetContext 経由に切替 → PR4 で field 削除して baseline=0 + fixed mode 移行予定（architecture-debt-recovery SP-A ADR-A-002 PR2） |
