...
| `app/src/test/guards/fullCtxPassthroughGuard.test.ts` | architecture | 2件 | F1: Dashboard widget registry の `registry*.tsx` 配下 full ctx passthrough（`ctx={ctx}` / `widgetCtx={ctx}`）の出現数 ≤ baseline=9。ADR-B-002 PR2 で pure delegation widget の helper signature を絞り込み props 化 → PR3 で重量級 widget (WID-001 / WID-002 / WID-018) の props 整理 → PR4 で baseline=0 fixed mode（widget-registry-simplification SP-B ADR-B-002 PR1） |
