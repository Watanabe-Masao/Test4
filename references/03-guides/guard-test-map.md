...
| `app/src/test/guards/phase6SummarySwapGuard.test.ts` | architecture | 5件 | Phase 6 Step A 完了状態の凍結: `ConditionSummaryEnhanced` / `ExecSummaryBarWidget` 内の旧 prev-year 命名 (`prevYear.totalSales` / `prevYearMonthlyKpi.monthlyTotal.sales` 等) のバラ参照を per-file count baseline (Cond=0 / Exec=2) で固定。共通 selector (`selectPrevYearSummaryFromFreePeriod` + `selectPrevYearSummaryFromLegacy` + `preferFreePeriodPrevYearSummary`) の存在 + import を強制（unify-period-analysis Phase 6 Step A regression freeze） |
