...
| `app/src/test/guards/registryInlineLogicGuard.test.ts` | architecture | 4件 | I1: Dashboard registry*.tsx の IIFE `(() =>` baseline=0 fixed mode (ADR-B-003 完遂) / I2: registry file 内 top-level function 宣言数 baseline=1 (ADR-B-004 ratchet-down、buildPrevYearCostMap WID-003) / I3: registry / page-widgets.tsx の `palette.X` 参照数 baseline=4 (ADR-B-004 ratchet-down、CostDetail KpiCard accent 4 色 WID-040) / scope 検証。ADR-B-004 PR2-4 で baseline=0 fixed mode 予定（widget-registry-simplification SP-B ADR-B-003 完遂 + ADR-B-004 PR1） |
