...
| `app/src/test/guards/projectizationPolicyGuard.test.ts` | documentation-steward | 4件 | AAG-COA (Change Operation Assessment): PZ-1 projectization metadata 欠落数の ratchet-down (baseline=9 of active projects, kind=collection 除外) / PZ-2 level 妥当性 / PZ-4〜PZ-12 Level 別 required+forbidden artifacts（過剰 project 化禁止 + 必要文書欠落検出）。詳細: `references/03-guides/projectization-policy.md` §12 |
