...
| `app/src/test/guards/deprecatedMetadataGuard.test.ts` | architecture | 2件 | DM1: production code (.ts/.tsx, test 除外) 内の `@deprecated` で 3 metadata（@expiresAt / @sunsetCondition / @reason）全て未記載のものを baseline=7 で ratchet-down 管理。ADR-D-004 PR2 で全 @deprecated に bulk 追記 → PR3 で baseline=0 fixed mode → PR4 で @expiresAt 超過を docs:check で fail（aag-temporal-governance-hardening SP-D ADR-D-004 PR1） |
