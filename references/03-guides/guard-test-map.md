...
| `app/src/test/guards/registryInlineLogicGuard.test.ts` | architecture | 2件 | I1: Dashboard widget registry の `registry*.tsx` 配下 IIFE pattern `(() =>` の出現数 ≤ baseline=3。ADR-B-003 PR2 で application/readModels/customerFact/selectors.ts に selector 抽出 → PR3 で 3 IIFE を selector call に置換 → PR4 で baseline=0 fixed mode（widget-registry-simplification SP-B ADR-B-003 PR1） |
