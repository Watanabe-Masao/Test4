...
| `app/src/test/guards/comparisonProjectionContextImportGuard.test.ts` | architecture | 3件 | `features/comparison/application/` 配下での `PeriodSelection` import を禁止。2 層 ratchet-down: ALLOWLIST = `buildComparisonProjectionContext.ts` 1 件 (最終形) + MIGRATION_BASELINE = `comparisonProjections.ts` / `hooks/useComparisonModule.ts` 2 件 (O4 で 1 に、O5 で 0 に縮退)（phase-6-optional-comparison-projection Phase O2） |
