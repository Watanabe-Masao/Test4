...
| `app/src/test/guards/chartInputBuilderGuard.test.ts` | architecture | 6件 | Chart Input Builder Pattern 強制: `presentation/components/charts/` 配下での `dateRangeToKeys` 直接呼び出しを禁止、query input 構築は application pure builder (`build<Name>Input.ts`) に集約（unify-period-analysis Phase 5、baseline 8 件 ratchet-down） |
