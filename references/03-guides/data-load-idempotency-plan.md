...
> 本格対応は別スコープで計画的に実施する。
>
> このスコープには 3 文書がある。役割は固定する:
>
> | 役割 | 文書 | 何が書かれているか |
> |---|---|---|
> | 正本（本書） | `data-load-idempotency-plan.md` | 根本問題・Phase 別履歴・推奨優先順序の唯一の正本 |
> | 要約 | `data-load-idempotency-handoff.md` | 次作業者の入口。本書の §7 と audit を要約したビュー |
> | 根拠資料 | `read-path-duplicate-audit.md` | FRAGILE / PARTIAL / SAFE 分類の機械的根拠 |
>
> 3 文書がズレた場合は本書を真とする。
...
- **FRAGILE → SAFE 昇格済み**: 3 クエリ（`purchaseComparison.ts` の UNION ALL 2 件 — PR D / `freePeriodFactQueries.ts` 1 件 — PR E）
- **FRAGILE（残・JSDoc only mitigation）**: 3 クエリ（`purchaseComparison.ts` の querySpecialSalesDaily / queryTransfersDaily / querySalesTotal — refactor 計画なし）
...
## 8. 残タスク（Phase 3.c の追加防御）の Done 定義

§7 末尾の 3 ステップは **追加防御** であり、本体スコープには含まれない。
ただし将来「2 件入れたから十分か？」とぶれないように、ここで Done を固定する。
要約版は handoff §2 にも転記してあるが、本書を正本とする。

1. **回帰テスト** — FRAGILE 6 件全てに、duplicate-injected mock conn による
   回帰テストが存在し、CI（vitest 全体）で green である。
2. **共有 helper** — 6 件のテストが個別に mock を組まず、
   `app/src/infrastructure/duckdb/__tests__/helpers/duplicateInjectedMockConn.ts`
   を経由している。検出粒度が 6 箇所で揃う。
3. **pre-aggregate refactor** — `purchaseComparison.ts` の FRAGILE 1/2 と
   ...
   置き換わっている。
   - ✅ PR D: FRAGILE 1/2 適用済み（`purchaseComparison.ts:107-130`, `:151-175`）。
   - ✅ PR E: `freePeriodFactQueries.ts` DAILY_SQL 適用済み（cs / p 双方 pre-aggregate）。
   - FRAGILE 3/4/5 (`querySpecialSalesDaily` / `queryTransfersDaily` / `querySalesTotal`)
     は audit 推奨事項上「JSDoc only mitigation」分類で refactor 計画なし。
4. **JSDoc 前提明示** — FRAGILE 6 件 + PARTIAL 3 件の関数 JSDoc に `@risk` タグと
   「ロード境界が壊れると silent に倍化する」前提が記載されている。
5. **文書同期** — 本書 §7 / handoff §2 / audit「推奨事項」が現実の実装と一致する。

**スコープ越境の禁止:**

- pre-aggregate を共通 SQL builder / DSL に寄せない。今回の目的は重複耐性で
  あって設計遊びではない。各クエリ内で局所的に subquery 集約を入れるだけに
  する。共通化が必要かどうかは 2 件 refactor 後の差分を見て判断する。
- audit FRAGILE 6 件をまとめて 1 PR で refactor しない。テスト → refactor の
  順序を破ると 2 次被害のリスクが高い。
