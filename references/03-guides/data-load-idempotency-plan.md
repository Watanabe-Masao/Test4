...
- **FRAGILE → SAFE 昇格済み**: 2 クエリ（`purchaseComparison.ts` の UNION ALL 2 件 — PR D で pre-aggregate refactor）
- **FRAGILE（残）**: 4 クエリ（`purchaseComparison.ts` 3 件 + `freePeriodFactQueries.ts` 1 件）
...
3. **pre-aggregate refactor** — `purchaseComparison.ts` の FRAGILE 1/2 と
   ...
   - PR E: `freePeriodFactQueries.ts` DAILY_SQL 予定。
