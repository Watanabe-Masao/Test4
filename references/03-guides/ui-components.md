...
全チャートは `presentation/components/charts/` に配置。ECharts ライブラリベースで実装。
...
統一パイプライン（ADR-003）によるチャート群。全て `presentation/components/charts/` に配置。
DuckDB Data Provider → Comparison/Alignment → JS Analysis → Chart VM → Chart の5段階で構成。

| コンポーネント | 説明 |
|--------------|------|
| `CumulativeChart` | 累計売上折れ線チャート |
| `TimeSlotChart` | 時間帯別売上チャート |
| `HeatmapChart` | 時間帯×曜日ヒートマップ |
| `StoreHourlyChart` | 店舗別時間帯分析 |
| `DeptHourlyChart` | 部門別時間帯パターン |
| `CategoryTrendChart` | カテゴリ日次トレンド |
| `CategoryHourlyChart` | カテゴリ×時間帯集計 |
| `CategoryMixChart` | カテゴリ構成比推移（週次） |
| `DeptTrendChart` | 部門別トレンド |
| `YoYChart` | 前年比較チャート |
| `FeatureChart` | 異常値・特徴量可視化（移動平均・Z スコア・スパイク検出） |
| `HourlyProfileChart` | 時間帯別売上分布 |
| `DowPatternChart` | 曜日別季節パターンヒートマップ |
| `StoreBenchmarkChart` | 店舗ランキング週次推移 |
| `DateRangePicker` | 日付範囲セレクタ（共有 UI） |
...
│   ├── 累計推移 → CumulativeChart / BudgetVsActualChart
│   ├── カテゴリ構成 → CategoryPieChart
│   ├── ヒートマップ → HeatmapChart / TimeSlotHeatmapChart
│   ├── 前年比較 → PrevYearComparisonChart / YoYChart
│   └── 要因分解 → WaterfallChart
