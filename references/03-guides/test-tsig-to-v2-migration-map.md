...
### 3.5. Phase 7 TSIG Deprecation 撤退期限（2026-04-27 確定）

| 項目                | 値                                                                                            |
| ------------------- | --------------------------------------------------------------------------------------------- |
| `@deprecated since` | **2026-04-27**（taxonomy-v2 Phase 7 着手日）                                                  |
| `@expiresAt`        | **2026-07-26**（90 日 cooling、Phase 8 retirement 着手目安）                                  |
| 撤退対象            | `app/src/test/guards/testSignalIntegrityGuard.test.ts` + 関連 architectureRules entry         |
| 撤退方法            | T:kind 認識化（per-tag obligation 化）または物理削除                                          |
| 残存 scope          | AR-G3-SUPPRESS-RATIONALE は §3.4 により恒久維持（撤退対象外）                                 |
| Phase 8 着手条件    | 全 test に v2 T:kind 付与済（Phase 6a-2 で達成済 V2-T-1 baseline=0）+ 90 日 cooling 完了      |
...
> v2 への置換マップは §1（4 rule × 移行先 T:kind）を canonical source とする。Phase 7 開始時点で全 4 rule の置換先が確定済み。
