...
| 3 | **pre-aggregate refactor** — `purchaseComparison.ts` の FRAGILE 1/2 と `freePeriodFactQueries.ts` DAILY_SQL の `cs` 側が、`store_day_summary` VIEW と同じ「source を subquery で `GROUP BY` してから UNION / JOIN する」パターンに置き換わっている。 | ✅ PR D (1/2) + PR E (#6) |
| 4 | **JSDoc 前提明示** — FRAGILE 6 件 + PARTIAL 3 件の関数 JSDoc に `@risk` タグと「ロード境界が壊れると silent に倍化する」前提が記載されている。 | ✅ PR A（refactor 済みクエリは @risk 除去） |
| 5 | **文書同期** — plan §7 / handoff §2 / audit「推奨事項」が現実の実装と一致し、どこにも「やる予定」と「実装済み」が混在していない。 | ✅ PR E |
...
### ~~中優先: `freePeriodFactQueries.DAILY_SQL` の `cs` 側も pre-aggregate する~~ ✅ PR E で完了

PR E で `cs` 側を `(store_id, date_key, day, is_prev_year)` 粒度に subquery
事前集約し、`p` 側と同じパターンに揃えた。outer GROUP BY は不要になり、
LEFT JOIN 後に SUM を再計算しない構造に変わった。
回帰テスト (`readPathDuplicateResistance.test.ts` の FRAGILE/6) は通常の
`it` に戻り、cs / p 双方の pre-aggregate 構造を構造的に検査する。
