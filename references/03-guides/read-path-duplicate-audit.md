...
で、UNION ALL と raw SUM の組み合わせが典型的な脆弱パターン。
`freePeriodFactQueries.ts` の daily 集計もフラジャイル。

### 一覧表

| # | File:line | Table | 集約 | Risk | 備考 |
|---|---|---|---|---|---|
| 1 | `purchaseComparison.ts:107-130` | purchase + special_sales + transfers | UNION ALL + SUM(cost/price) | FRAGILE | 3 テーブル UNION 後に SUM |
| 2 | `purchaseComparison.ts:151-175` | 同上 | UNION ALL + SUM(cost/price) GROUP BY store_id, day | FRAGILE | daily 版 |
