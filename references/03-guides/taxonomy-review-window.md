...
提案時に `references/02-status/taxonomy-review-journal.md` の **次回 window 提案リスト** に entry を追加:
...
- 人間 reviewer が AI の文案を承認 + 自身が提案者として journal に entry 追加
- AI が `R:unclassified` / `T:unclassified` で退避し、人間が後続 window で正規化提案
...
- 提案内容の妥当性確認（§4 判定基準）
- 自身が提案者として journal entry を update（AI 提案 → 人間提案へ昇格）
...
| 文書                                                  | 役割                                                                |
| ----------------------------------------------------- | ------------------------------------------------------------------- |
| `references/01-principles/taxonomy-constitution.md`   | 7 不可侵原則（本ガイドは原則 3 + 4 + 5 + 6 + 7 の運用）             |
| `references/01-principles/taxonomy-interlock.md`      | Interlock マトリクス（改訂手続き §3 と本ガイド §4.4 が対応）        |
| `references/01-principles/taxonomy-origin-journal.md` | Origin 形式（採択 entry が本 journal の Origin に転記される）       |
| `references/02-status/taxonomy-review-journal.md`     | 提案 / 採択 / 却下 / 例外の journal（本ガイドが定義する形式の運用） |
