...
5 WASM bridge が稼働中（`compat.bridge.count = 5`）。
4 engine（factorDecomposition, grossProfit, budgetAnalysis, forecast）は promotion-candidate（blocker なし）。
1 engine（timeSlot）は bridge-ready（compare 計画 + 観測テスト作成が必要）。
詳細は `engine-promotion-matrix.md` を参照。昇格基準は `promotion-criteria.md` を参照。
...
| 7 | Phase 6 | **進行中** — 4 engine が promotion-candidate、1 engine が bridge-ready |

**次の優先:** Phase 6（WASM bridge 収束）と Phase 3（Window 契約型）を並行。
