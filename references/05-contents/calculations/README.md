...
## 初期割当表（Phase D Step 4 完遂段階、16 件）
...
> Phase D Step 5+ で残 tier3 候補（`timeSlotCalculations` / `budgetSimulator` /
> `budgetSimulatorAggregations` / `algorithms/*` (4) / `temporal/*` (1) /
> `candidate/*` (10、物理化未))を spec 化予定。
> AR-CONTENT-SPEC-CANONICAL-REGISTRATION-SYNC が uncovered baseline=19 で
> ratchet-down 管理。
