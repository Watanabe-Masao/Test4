...
## 初期割当表（Phase D Step 5 完遂段階、21 件）
...
> Phase D Step 6+ で残 tier3 候補（`algorithms/sensitivity` / `algorithms/trendAnalysis` /
> `temporal/computeMovingAverage` (current 3 件) + `candidate/*` (11、物理化済 + sensitivity 含む))を
> spec 化予定。
> AR-CONTENT-SPEC-CANONICAL-REGISTRATION-SYNC が uncovered baseline=14 で
> ratchet-down 管理。
