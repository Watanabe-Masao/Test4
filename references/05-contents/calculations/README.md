...
> **current 全件 cover 完了** (24 件)。残 11 件は全て candidate（物理化済 file: piValue /
> inventoryCalc / pinIntervals / observationPeriod / remainingBudgetRate / customerGap /
> dowGapAnalysis / algorithms/sensitivity / algorithms/correlation / algorithms/trendAnalysis /
> temporal/computeMovingAverage）。Promote Ceremony 着手フェーズで `lifecycleStatus: proposed`
> 先行 spec 化 → 数値同等性検証 → current ↔ candidate 昇格 1 PR 5 同期で順次 cover 予定。
> AR-CONTENT-SPEC-CANONICAL-REGISTRATION-SYNC が uncovered baseline=11 で ratchet-down 管理。
