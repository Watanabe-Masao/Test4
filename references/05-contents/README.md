...
| `calculations/` | Domain calculation 仕様書（`CALC-NNN.md`） | Phase D Step 4 完遂段階で 16 件 (Step 1+2 tier1 6 件 + Step 3 tier2 5 件 + Step 4 tier3 第一波 5 件: dowGapAnalysis / discountImpact / prevYearCostApprox / costAggregation / markupRate)。Lifecycle State Machine + Promote Ceremony + canonicalRegistration sync + replacedBy/supersedes 双方向対称 + Behavior Claims (Phase J Evidence Level) を institutionalize、ratchet-down で残 19 件 cover 予定 |
