...
| **commit pin** | source が動いたのに spec.lastVerifiedCommit が追随していない | `AR-CONTENT-SPEC-LAST-VERIFIED-COMMIT` (full SHA 完全一致) + `AR-CONTENT-SPEC-OWNER` |
...
# 構造 sync + commit-pin sync（機械検証対象 — co-change guard + last-verified-commit guard で強制）
lastVerifiedCommit: <full-40-char-SHA>   # 直近に source と突合した commit (`git log -1 --format=%H -- <sourceRef>`)
...
> source ↔ spec の同期は `lastVerifiedCommit` (full SHA) と
> `AR-CONTENT-SPEC-LAST-VERIFIED-COMMIT` guard で構造的に保証する。
...
| Architecture Rule（`architectureRules.ts`） | `AR-CONTENT-SPEC-*` 11 件 (FRESHNESS は Phase K で撤退、LAST-VERIFIED-COMMIT に置換) |
