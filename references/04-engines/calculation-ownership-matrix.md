...
| ファイル | 関数 | 役割 | 実装責務 | 権威性 | 備考 |
|---|---|---|---|---|---|
| `rawAggregation/dailyAggregation.ts` | `aggregateByDay` | aggregate-source | TS（SQL 移管済み） | exploratory | 日別集計。SQL 版: `queries/aggregates/dailyAggregation.ts` |
| `rawAggregation/dailyAggregation.ts` | `cumulativeSum` | aggregate-source | TS（SQL 移管済み） | exploratory | 累計。SQL 版: `queries/aggregates/dailyAggregation.ts` |
...
| `rawAggregation/aggregationUtilities.ts` | `aggregatePeriodRates` | aggregate-source | TS（SQL 移管済み） | exploratory | 期間率集計。SQL 版: `queries/aggregates/storeAggregation.ts` |
...
### 8b. SQL 集約クエリ — aggregate-source / SQL（移管済み）

| ファイル | 関数 | 役割 | 実装責務 | 権威性 | 備考 |
|---|---|---|---|---|---|
| `queries/aggregates/dailyAggregation.ts` | `queryDailyCumulativeAggregation` | aggregate-source | SQL | exploratory | Issue-6: TS aggregateByDay + cumulativeSum の SQL 移管 |
| `queries/aggregates/storeAggregation.ts` | `queryStoreAggregation` | aggregate-source | SQL | exploratory | Issue-9: TS aggregatePeriodRates の SQL 移管（店舗別） |
| `queries/aggregates/storeAggregation.ts` | `queryStoreAggregationSummary` | aggregate-source | SQL | exploratory | Issue-9: 全店サマリーの SQL 移管 |
| `queries/aggregates/departmentAggregation.ts` | `queryDepartmentAggregation` | aggregate-source | SQL | exploratory | Issue-9: TS buildDepartmentKpiIndex の前段集約を SQL 移管 |
| `queries/aggregates/departmentAggregation.ts` | `queryDepartmentAggregationSummary` | aggregate-source | SQL | exploratory | Issue-9: 部門別売上加重平均の SQL 移管 |
