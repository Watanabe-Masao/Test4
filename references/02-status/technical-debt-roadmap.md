...
| legacy | 11 | 0 | **-11** | **全件解消** |
| bridge | 4 | **0** | **-4** | **全件解消** ✅ — presentationDuckdbHook 全件卒業、useUnifiedWidgetContext bridge 完了 |
| lifecycle | 1 | 1 | ±0 | ライフサイクル管理。正当 |

**削減可能:** 0 エントリ — migration/bridge/legacy 全件解消済み
...
| **完了** | P5 Query Access Architecture 再設計（Bridge 卒業） | **完了** ✅ — bridge 0件、presentationDuckdbHook 0件 |
...
5. ~~P5: Query Access Architecture 再設計~~ **完了** ✅ — bridge 0件、presentationDuckdbHook 0件
...
| bridge カテゴリ | 4 | **0**（-4） | 0 | **全件解消** ✅ |
| 凍結済み allowlist | 2 | **6**（+4） | 6 以上 | 達成 ✅ |
| presentationDuckdbHook | 27 | **0**（-27） | 0 | **全件解消** ✅ |
| Tier2 大型 component | 8 | **0**（-8、凍結） | 0 | **全件解消** ✅ |
| app→infra | 14 | **10**（-4） | 10 | **達成** ✅（2026-03-29: Port 移動 + adapter bridge 3件削除） |
| application/ports/ | 8 | **0**（全廃） | 0 | **全件解消** ✅（2026-03-29: domain/ports/ に移動） |
| Lint 警告 | 33 | **2**（-31） | 0 | 大幅削減 |
| テスト数 | 616 | **4,686**（+4,070） | — | 7.6倍 |
