...
| **前年予算比較** | `usecases/explanation/prevYearBudgetExplanation.ts` | useComparisonModule | `prevYearSameDowBudgetRatio`, `prevYearSameDateBudgetRatio` |

前年予算比較系は `aggregateKpiByAlignment` が生成する `storeContributions`（店舗×日別の計算根拠）を
...
| 粗利予算系 | `grossProfitBudget`, `grossProfitBudgetAchievement`, `grossProfitBudgetVariance`, `projectedGrossProfit`, `projectedGPAchievement` | 粗利予算, 粗利予算達成率, 粗利予算差異, 粗利着地見込, 粗利着地達成率 |
| 前年予算比較 | `prevYearSameDowBudgetRatio`, `prevYearSameDateBudgetRatio` | 前年同曜日予算比, 前年同日予算比 |
