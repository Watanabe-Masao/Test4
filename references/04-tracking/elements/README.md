# references/04-tracking/elements/ — 実装要素の現状把握台帳
...
**改善は「全体が壊れないよう充分注意しながら段階的に進める」**（4 ステップ pattern + guard 先行。詳細は `projects/completed/architecture-debt-recovery/plan.md` §10.5）。
...
| `calculations/` | Domain calculation 仕様書（`CALC-NNN.md`） | Phase D Step 6 完遂段階で 24 件 (Step 1+2 tier1 6 件 + Step 3 tier2 5 件 + Step 4 tier3 第一波 5 件 + Step 5 tier3 第二波 5 件 + Step 6 current 完遂 3 件: sensitivity / trendAnalysis / computeMovingAverage)。current 全件 cover 完了。Lifecycle State Machine + Promote Ceremony + canonicalRegistration sync + replacedBy/supersedes 双方向対称 + Behavior Claims (Phase J Evidence Level) を institutionalize、残 11 件 (candidate のみ) は Promote Ceremony 着手フェーズで spec 化 |
...
| `ui-components/` | Selected UI Component 仕様書（`UIC-NNN.md`） | Phase F 着手段階で 5 件 (ConditionSummaryEnhanced / KpiCard / KpiGrid / ChartCard / ChartLoading)。selection rule (複数 widget/page 参照 / props 重い / responsibility hotspot) で対象限定、props contract / hooks / sideEffects / state coverage を記録 |
...
| **commit pin** | source が動いたのに spec.lastSourceCommit が追随していない | `AR-CONTENT-SPEC-LAST-SOURCE-COMMIT` (full SHA 完全一致) + `AR-CONTENT-SPEC-OWNER` |

「存在」だけで終わらせず、**構造 sync と commit-pin sync の両方**を必須とするのが本カテゴリの運用要件。Phase K Option 1 (2026-04-29) で date-based cadence (`AR-CONTENT-SPEC-FRESHNESS`) は儀式と判定して撤退、commit-pin による構造的 mechanism に置換済。
...
# 構造 sync + commit-pin sync（機械検証対象 — co-change guard + last-source-commit guard で強制）
lastSourceCommit: <full-40-char-SHA>   # 直近に source と突合した commit (`git log -1 --format=%H -- <sourceRef>`)

# owner (責任 role)
owner: <role>                            # implementation / architecture / documentation-steward 等
...
> **Phase K Option 1 (2026-04-29) で撤退済**:
> `reviewCadenceDays` / `lastReviewedAt` field と `AR-CONTENT-SPEC-FRESHNESS` rule は
> date-based cadence の儀式性 (review = date 更新で構造的検証伴わず) のため撤退。
> source ↔ spec の同期は `lastSourceCommit` (full SHA) と
> `AR-CONTENT-SPEC-LAST-SOURCE-COMMIT` guard で構造的に保証する。
...
| obligation map（`obligation-collector.ts`） | source path 変更 → 対応 spec の `lastSourceCommit` 更新義務 |
| Architecture Rule（`architectureRules.ts`） | `AR-CONTENT-SPEC-*` 11 件 (FRESHNESS は Phase K で撤退、LAST-SOURCE-COMMIT に置換) |
...
詳細は `projects/completed/architecture-debt-recovery/inquiry/01a-widget-specs-bootstrap.md` を参照。
