...
現在25指標が登録済みだが、以下が欠落:
- 売上予算系: 経過予算率、進捗ギャップ、予算差異、必要日次売上、着地予測達成率、日平均売上
- 粗利予算系: 粗利予算、粗利率予算、粗利予算達成率、粗利予算差異、粗利進捗ギャップ、粗利着地予測、必要日次粗利
- 在庫差異: inventoryGap
- 客数: 客単価

→ 目標 41 指標（`metric-id-registry.md` 参照）
...
### Phase A: MetricMeta 型定義 + METRIC_DEFS 定数
...
### Phase B: MetricId 型拡張 + budgetAnalysis 計算追加
...
### Phase D: causalChain.ts ハードコード排除
