...
~~現在25指標が登録済みだが、以下が欠落~~ → **解決済み: 50 MetricId が実装済み**

~~→ 目標 41 指標~~ → **実績: 50 指標（コード定義済み・レジストリ文書化済み）**
...
**前提:** Phase A 完了
**状態:** 完了（2026-03）。50 MetricId が型定義済み。売上予算系 11 件は全て計算 + Explanation 実装済み。粗利予算系・仕入予算系の一部は MetricId 登録済み/計算未実装（`metric-id-registry.md` 参照）。
