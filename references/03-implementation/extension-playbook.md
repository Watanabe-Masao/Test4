...
- [ ] `references/03-implementation/metric-id-registry.md` に登録（ID・ラベル・計算式・ソース）

### 3. Explanation（説明責任 L1/L2/L3）

データソースに応じて2つの追加先がある:

**A. StoreResult ベースの指標（大多数）:**
...
**B. フックデータ依存の指標（前年予算比較等）:**
- [ ] `application/usecases/explanation/` に専用生成関数を追加
  - 例: `prevYearBudgetExplanation.ts`（usePrevYearMonthlyKpi のデータに依存）
- [ ] `useExplanation` フックでマージして ExplanationService の結果と統合
- [ ] 集約→ドリルダウン一致の不変条件テストを追加
  - `Σ(breakdown.value) = entry.合計値`（INV-EXPL-01 相当）
  - `evidenceRefs` が全計算寄与データを網羅（INV-EXPL-02 相当）
...
- [ ] 必要なら `references/03-implementation/calculation-engine.md` を更新
...
- [ ] `references/01-foundation/engine-responsibility.md` を確認
...
- [ ] チャートディレクトリを `presentation/components/charts/ChartName/` に作成
  - `ChartName.tsx` — 描画のみ（≤200行）
  - `ChartName.styles.ts` — styled-components（テーマトークン経由）
  - `ChartName.vm.ts` — ViewModel（WriteModel + ReadModel → 描画データの純粋変換）
  - `index.ts` — バレル re-export
- [ ] ViewModel は `build*ViewModel()` の命名規則に従う（設計原則 #14）
...
- [ ] `references/01-foundation/engine-responsibility.md` を更新
- [ ] `references/03-implementation/duckdb-architecture.md` に追記（必要な場合）
...
| 落とし穴 | 正しいやり方 |
|---|---|
| Explanation で計算を再実行する | StoreResult の既存値をそのまま参照する（StoreResult 外は専用生成関数） |
