...
inventorySource: "references/04-tracking/test-taxonomy-inventory.yaml (Phase 0 baseline 728 entry)"
status: draft
...
### 3.6. TSIG → v2 retirement coverage matrix (post-review fix #4、2026-04-27)

> **背景**: post-review で「TSIG 退役で false-green test 系の具体検出が薄くなっている可能性」が指摘されたため、 v1 TSIG が守っていた具体 harm を v2 でどう代替するかを **rule 単位で明示**する coverage matrix を本節に固定する。「v2 T:kind が代替する」という抽象表現に頼らず、**機械検証 / advisory / 退役** の 3 種類に分類して透明化。

| v1 rule          | v1 が検出した具体 harm                                                       | v2 での代替                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | 代替の種類                              |
| ---------------- | ---------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------------------------------|
| TSIG-TEST-01     | existence-only assertion（`expect(x).toBeDefined()` のみで終わる test）       | **退役 + advisory のみ**。v2 T:kind は **存在ベース vocabulary 検証**（@taxonomyKind の有無・registry 登録）であり、test 内 assertion 内容は検証しない。代わりに `test-signal-integrity-advisory.md` + pre-push hook の `[TSIG-ADVISORY]` 自己点検プロンプトで **作成者責任**として運用。**機械検証は弱まる**（False Green 再混入リスクあり）。代替に十分か未検証の forward-looking commitment。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | **回帰**（review discipline 依存）       |
| TSIG-TEST-04     | tautology assertion（`expect(true).toBe(true)` 等の常時 true assertion）     | **同上 — 退役 + advisory のみ**。Test 内容ベース検出は v2 で消滅。advisory 対象。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | **回帰**（review discipline 依存）       |
| TSIG-COMP-03     | multi-underscore unused suppress（`(_a, _b)` のような unused param 抑制）    | **AR-G3-SUPPRESS-RATIONALE で部分代替**。AR-G3-SUPPRESS-RATIONALE は scope 違いで恒久維持（§3.4）し、suppression rationale を必須化することで「黙らせる手段」を「制度化された例外」に格上げする。multi-underscore pattern 自体の検出は v2 で消滅したが、**より強い rationale enforcement** に置換されたため概念的には改善。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | **昇格**（より強い rationale 強制で代替）|
| AR-G3-SUPPRESS-RATIONALE | ts-suppress comment の rationale 必須化                              | **scope 違いで恒久維持**（§3.4）。T:kind 軸とは独立 layer として継続。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | **存続**（撤退対象外）                  |

#### 機械検証の回帰範囲（明示的に開示）

post-review で指摘された通り、**TSIG-TEST-01 / TSIG-TEST-04 が守っていた "実証的 assertion 強制" は v2 で機械検証されない**。具体的には:

- v2 T:kind は **存在 + vocabulary 整合**を検証する（registry 登録 / Interlock / Anchor Slice / Cognitive Load）
- v2 T:kind は **assertion 内容**を検証**しない**（`expect(x).toBeDefined()` のみの test も pass する）
- 結果として **False Green / Review Misleading の再混入経路**が広がっている

#### 補完経路（forward-looking commitment）

- **advisory**: pre-push hook の `[TSIG-ADVISORY]` で新規 test file 検出時に自己点検プロンプト表示（`test-signal-integrity-advisory.md`）
- **review discipline**: PR template の Taxonomy v2 チェックに「対応 test に substantive assertion を含む」確認項目を追加することを次回 review window で提案候補
- **将来代替**: T:kind 別の "substantive assertion" 定義を §1 §3 / per-T:kind operations.md に追記し、機械検証可能な範囲（特定 matcher 禁止リスト等）を将来 PR で復活させる候補
...
- 移行期限を CLAUDE.md / references/04-tracking/ で参照可能化
...
| 文書                                                   | 役割                                           |
| ------------------------------------------------------ | ---------------------------------------------- |
| `references/01-foundation/test-taxonomy-schema.md`     | v2 vocabulary 仕様正本 (Phase 1 deliverable)   |
| `references/01-foundation/taxonomy-constitution.md`    | 7 不可侵原則（原則 1 / 3 / 4 が本 map の根拠） |
| `references/01-foundation/taxonomy-interlock.md`       | R⇔T マトリクス（v2 T:kind ← 検証対象 R:tag）   |
| `references/01-foundation/test-signal-integrity.md`    | 現行 TSIG global rule の定義                   |
| `references/04-tracking/test-taxonomy-inventory.yaml`    | Phase 0 baseline (728 entry / Anchor 206)      |
...
| `projects/active/taxonomy-v2/plan.md` §OCS.4                  | Lifecycle State Machine 仕様正本               |
