...
>
> 上位原則: [`references/01-foundation/test-signal-integrity.md`](../01-principles/test-signal-integrity.md)
>
> 実装コンポーネント:
...
| **Advisory** | TSIG-ADV-01〜04 等の意味論依存パターン + 新規 test 追加時の自己点検 | `fixNow=review` で warning のみ | 作成者（user/AI）に再点検させる |
...
- 例外に該当する場合は references/01-foundation/test-signal-integrity.md
  の EX-01〜EX-03 を参照
- 不明確な場合は Discovery Review に持ち込む
  (references/03-implementation/discovery-review-checklist.md)
...
6. `references/03-implementation/architecture-rule-system.md` の手順に従い登録
...
- [`references/01-foundation/test-signal-integrity.md`](../01-principles/test-signal-integrity.md) — 上位原則（Protected Harm / Detection Boundary / Allowed Exceptions）
- [`references/03-implementation/discovery-review-checklist.md`](./discovery-review-checklist.md) — グレーパターンの逃がし先
- [`references/03-implementation/architecture-rule-system.md`](./architecture-rule-system.md) — Architecture Rule の運用手順
- [`references/03-implementation/coding-conventions.md`](./coding-conventions.md) — TypeScript / lint 規約
