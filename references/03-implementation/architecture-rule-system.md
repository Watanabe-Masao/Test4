...
| 改善が進まない | migrationRecipe が「次に何をすべきか」を常に提示する |
...
interface ArchitectureRule {
  ...
  // ── 修正手順（App Domain の安定知識） ──
  migrationRecipe?: {
    steps: string[]               // 順序付き修正手順
  }

  // ── 実行計画（Project Overlay の案件運用状態） ──
  executionPlan?: {
...
参照: references/01-foundation/design-principles.md
修正手順:
...
3. 推奨フィールド: `doc`, `migrationRecipe`, `executionPlan`, `decisionCriteria`
4. `architectureRuleGuard.test.ts` が自動で整合性を検証:
   - ID 一意性
   - guardTag の存在
   - baseline / thresholds の存在（gate + count 型）
   - migrationRecipe.steps が空でない
   - executionPlan.effort / priority が存在
...
現在値は `references/04-tracking/generated/architecture-health.json` を参照。
