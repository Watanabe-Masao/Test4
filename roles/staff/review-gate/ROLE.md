...
- 9禁止事項の遵守確認（`references/01-principles/prohibition-quick-ref.md` 参照）
- ガードテスト結果の確認（`references/03-guides/guard-test-map.md` 参照）
- CI 6段階ゲートの結果検証
- 許可リスト変更の妥当性判定
- 300行超ファイルの検出
...
9. [ ] 比較データの sourceDate を落とす変換がないか（`buildSameDowPoints()` を経由しているか）
10. [ ] pure かつ authoritative な処理を制御層（hooks, stores, usecases）に新規実装していないか

### ガードテスト

- [ ] `guards/layerBoundaryGuard.test.ts` が通ること
...
- `references/01-principles/prohibition-quick-ref.md` — 9禁止事項クイックリファレンス（**必読**）
- `references/03-guides/guard-test-map.md` — ガードテスト対応表（**必読**）
