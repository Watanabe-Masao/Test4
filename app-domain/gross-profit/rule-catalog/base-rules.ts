/**
 * Architecture Rules — BaseRule 定義データ（App Domain 物理正本）
 *
 * このファイルが **App Domain 側の物理正本**。ルールはデータ層であり、
 * 検出ロジックはテストファイル（app/src/test/guards/*.test.ts）に残る。
 *
 * 案件運用状態（fixNow / executionPlan / reviewPolicy / lifecyclePolicy）は
 * Project Overlay（projects/<active>/aag/execution-overlay.ts）側に分離され、
 * app/src/test/architectureRules/merged.ts で derived merge される。
 *
 * consumer は **必ず** `app/src/test/architectureRules` facade 経由で参照すること。
...
export const ARCHITECTURE_RULES: readonly BaseRule[] = [
...

  {
    ...
    decisionCriteria: {
      ...
      exceptions:
        "bridge 管理下の dual-run-compare（compareUtils.ts 経由、テスト側のみ）は許可",
      escalation:
        "getExecutionMode / recordCall / recordMismatch を発見したら即削除",
    },
    detection: {
      type: "regex",
      severity: "gate",
...

  {
    ...
    correctPattern: {
      description:
        "新規フィールドは feature slice のローカルコンテキストに寄せる",
    },
    outdatedPattern: {
    ...
    detection: {
      // 2026-04-14: unify-period-analysis Phase 1 で freePeriodLane
...

  {
    ...
    correctPattern: {
      description:
        "新規 plan は features/<feature>/application/plans/ に作成する",
    },
    outdatedPattern: {
...

  {
    ...
    outdatedPattern: {
      description:
        "domain/ から application/ / infrastructure/ / presentation/ を import する",
...

  {
    ...
    outdatedPattern: {
      description:
        "application/ から infrastructure/ を直接 import する（許可された経路以外）",
    ...
    protectedHarm: {
      prevents: [
        "application と infrastructure の密結合",
        "インフラ変更時の影響範囲拡大",
      ],
    },
  },
...

  {
    ...
    outdatedPattern: {
      description:
        "presentation/ から application/usecases/ を value import する",
...

  {
    ...
    protectedHarm: {
      prevents: [
        "infrastructure から application への逆方向依存",
        "循環依存の発生",
      ],
    },
  },
...

  {
    ...
    correctPattern: {
      description:
        "public API のみを export する。内部関数はファイル内に閉じる",
    },
    outdatedPattern: {
...

  {
    ...
    correctPattern: {
      description:
        "根本原因を修正する。どうしても必要な場合は allowlist に正当理由を記載",
    },
    outdatedPattern: {
      description:
        "eslint-disable / @ts-ignore / @ts-expect-error コメントを追加する",
      codeSignals: ["eslint-disable", "@ts-ignore", "@ts-expect-error"],
    },
    decisionCriteria: {
      when: "eslint-disable や @ts-ignore を追加しようとしたとき",
      exceptions:
        "ライブラリ制約（ECharts 等）や非標準 HTML 属性の場合は G3_ALLOWLIST に追加可能",
...

  {
    ...
    correctPattern: {
      description:
        "catch 内で最低でも console.warn を入れるか、エラーを伝播する",
    },
    outdatedPattern: {
...

  {
    ...
    decisionCriteria: {
      ...
      exceptions:
        "allowlists/size.ts の Tier 2 に登録可能（次回改修時に分割義務）",
...

  {
    ...
    decisionCriteria: {
      ...
      exceptions:
        "application 層の purchaseCostHandler / readPurchaseCost() の直接使用は許容",
...

  {
    ...
    correctPattern: {
      ...
      imports: [
        "@/application/readModels/freePeriodBudget/readFreePeriodBudgetFact",
      ],
    },
    outdatedPattern: {
...

  {
    ...
    correctPattern: {
      ...
      imports: [
        "@/application/readModels/freePeriodDeptKPI/readFreePeriodDeptKPI",
      ],
    },
    outdatedPattern: {
...

  {
    ...
    decisionCriteria: {
      ...
      escalation:
        "calculateGrossProfit → getEffectiveGrossProfit の 2 層構造を使用",
    },
    relationships: {
...

  {
    ...
    correctPattern: {
      description:
        "AnalysisFrame / CalculationFrame 経由でクエリ入力を構築する",
    },
    outdatedPattern: {
      description: "presentation/ で直接クエリ入力を組み立てる",
    },
    decisionCriteria: {
      when: "クエリ入力を構築するとき",
      exceptions: "例外なし",
...

  {
    ...
    correctPattern: {
      description:
        "新規ファイル追加時に calculationCanonRegistry.ts に分類を登録する",
    },
    outdatedPattern: {
      description:
        "domain/calculations/ にファイルを追加してレジストリに未登録",
    },
    decisionCriteria: {
...

  {
    ...
    correctPattern: {
      description:
        "z.object(...).parse() で入力を検証し、結果型を Zod schema で定義する",
    },
    outdatedPattern: {
      description: "required 分類なのに Zod parse がないファイル",
    },
    decisionCriteria: {
      when: "domain/calculations/ の required ファイルを変更するとき",
      exceptions:
        "例外なし。required は全て Zod 契約必須（現在 13/13 達成済み）",
...

  {
    ...
    correctPattern: {
      description:
        "review ファイルに Zod 契約を追加し zodAdded: true に更新する",
    },
    outdatedPattern: {
      description: "review 分類で zodAdded: false のまま放置する",
    },
    decisionCriteria: {
      when: "review 分類のファイルを変更するとき",
      exceptions:
        "出力型が外部定義（domain/models）に依存する場合は外部スキーマ整備後に対応",
...

  {
    ...
    correctPattern: {
      description: "canonical input builder を使用して正本関数に渡す",
    },
    outdatedPattern: {
      description: "presentation/ で独自に PI値や GAP を計算する",
    },
    decisionCriteria: {
...

  {
    ...
    outdatedPattern: {
      description: "正本化手順を省略して readModel を追加する",
    },
    decisionCriteria: {
...

  {
    ...
    correctPattern: {
      description: "buildComparisonScope() factory 経由で生成する",
    },
    outdatedPattern: {
      description: "presentation/ で直接 ComparisonScope を構築する",
    },
    decisionCriteria: {
      when: "比較スコープを生成するとき",
      exceptions: "例外なし",
...

  {
    ...
    correctPattern: {
      description: "定義書の集計ルールに従い、state リセットを忘れない",
    },
    outdatedPattern: {
      description: "二重計上、DuckDB is_prev_year の不整合、state リセット漏れ",
    },
    decisionCriteria: {
      when: "集計クエリや state リセットを書くとき",
      exceptions: "例外なし",
      escalation:
        "二重計上チェック、is_prev_year 整合確認、useEffect クリーンアップ確認",
    },
    relationships: {
      dependsOn: ["AR-STRUCT-CANONICALIZATION"],
    },
...

  {
    ...
    outdatedPattern: {
      description:
        "dualRunObserver / getExecutionMode / recordCall / recordMismatch の復活",
    },
    decisionCriteria: {
      when: "WASM 関連コードを変更するとき",
      exceptions:
        "bridge 管理下の dual-run-compare（Phase 5-8 の parity 検証）は許可",
...

  {
    ...
    correctPattern: {
      description:
        "readModel に usedFallback: boolean を含め、フォールバック時に true を設定する",
    },
...

  {
    ...
    outdatedPattern: {
      description:
        "旧パス（application/hooks/ 等）経由で移行済みモジュールを import する",
    },
    decisionCriteria: {
...

  {
    ...
    decisionCriteria: {
      ...
      exceptions: "例外なし",
      escalation:
        "PAGE_REGISTRY と routes.tsx の PAGE_COMPONENT_MAP を両方更新",
    },
...

  {
    ...
    outdatedPattern: {
      description: "presentation/ から DuckDB クエリを直接実行する",
    },
    decisionCriteria: {
...

  {
    ...
    decisionCriteria: {
      ...
      exceptions: "例外なし。domain/ は純粋でなければならない",
      escalation:
        "async は application/ に、外部 API は infrastructure/ に配置する",
    },
    migrationRecipe: {
      steps: [
...

  {
    ...
    correctPattern: {
      description:
        "QueryHandler + useQueryWithHandler 経由。input は正規化必須",
    },
    outdatedPattern: {
      description: "コンポーネント内で直接クエリを組み立て実行する",
    },
    decisionCriteria: {
      when: "DuckDB クエリを追加・変更するとき",
      exceptions:
        "例外なし。全クエリは QueryHandler + useQueryWithHandler 経由",
...

  {
    ...
    correctPattern: {
      description: "uiPersistenceAdapter 経由でストレージにアクセスする",
    },
    outdatedPattern: {
...

  {
    ...
    outdatedPattern: {
      description:
        "useDataStore.getState() / useUiStore.getState() の直接呼び出し",
    },
    decisionCriteria: {
      when: "presentation 層から store のデータや action が必要なとき",
      exceptions:
        "allowlists/responsibility.ts presentationGetStateLimits に登録可能",
...

  {
    ...
    outdatedPattern: {
      description: "let _cache = null; let _initialized = false;",
    },
    decisionCriteria: {
      when: "モジュールレベルでキャッシュやフラグが必要なとき",
      exceptions:
        "allowlists/responsibility.ts moduleScopeLetLimits に登録可能",
...

  {
    ...
    outdatedPattern: {
      description: "useMemo + useCallback が 12 を超えるコンポーネント/hook",
    },
    decisionCriteria: {
      when: "useMemo/useCallback を追加して合計が 12 に近づいたとき",
      exceptions:
        "allowlists/complexity.ts combinedHookComplexityLimits に登録可能",
      escalation:
        "useWeatherDaySelection のような sub-hook に関連 hooks を抽出",
    },
...

  {
    ...
    outdatedPattern: {
      description: "features/ 内で useMemo/useState が上限を超えるファイル",
    },
    decisionCriteria: {
      when: "features/ の hook に useMemo/useState を追加するとき",
      exceptions:
        "allowlists/complexity.ts featuresMemoLimits / featuresStateLimits に登録可能",
...

  {
    ...
    outdatedPattern: {
      description: "domain/models/ で export が 8 を超えるファイル",
    },
    decisionCriteria: {
      when: "domain/models/ に export を追加するとき",
      exceptions:
        "allowlists/responsibility.ts domainModelExportLimits に登録可能",
      escalation:
        "DiscountEntry.ts / AsyncStateFactories.ts のようにファイル分割",
    },
    detection: { type: "count", severity: "gate", baseline: 0 },
    migrationRecipe: {
      steps: [
...

  {
    ...
    outdatedPattern: {
      description: "DUMMY_/EMPTY_/ZERO_/IDLE_ が 7 を超えるファイル",
    },
    decisionCriteria: {
      when: "fallback 定数を追加するとき",
      exceptions:
        "allowlists/responsibility.ts fallbackConstantDensityLimits に登録可能",
      escalation:
        "ローカルエイリアス化（const zeroPair = ZERO_COST_PRICE_PAIR）",
    },
    detection: { type: "regex", severity: "gate", baseline: 0 },
    migrationRecipe: {
      steps: [
...

  {
    ...
    correctPattern: {
      description: "readCustomerFact() から CustomerFact を取得して分析に使う",
    },
    outdatedPattern: {
      description: "StoreResult.totalCustomers を分析・計算の入力に使用する",
    },
    decisionCriteria: {
      when: "分析で客数が必要なとき",
    ...
    migrationRecipe: {
      steps: [
        "1. StoreResult.totalCustomers の参照を削除",
        "2. readCustomerFact() に置き換え",
      ],
    },
...

  {
    ...
    outdatedPattern: {
      description:
        "ウィジェットが ctx 提供済みデータの hook を独自に呼び出している",
    },
    decisionCriteria: {
...

  {
    ...
    correctPattern: {
      description: "temporal 計算は一方向。結果は hook 経由で消費のみ",
    },
    outdatedPattern: {
      description: "temporal 計算結果を UI や比較ロジックに逆流させる",
    },
    decisionCriteria: {
...

  {
    ...
    outdatedPattern: {
      description: "sameDate と sameDow を同一コンテキストで混在使用する",
    },
    decisionCriteria: {
...

  {
    ...
    correctPattern: {
      description: "新機能は features/<feature>/ に配置。共通は既存 4 層に配置",
    },
...

  {
    ...
    correctPattern: {
      description:
        "旧 API を @deprecated にして新 API に一本化。併存期間は最短にする",
    },
    outdatedPattern: {
      description:
        "同じ目的の関数を 2 つ export する（例: getData と fetchData）",
    },
    decisionCriteria: {
...

  {
    ...
    outdatedPattern: {
      description: "分類のカバレッジを上げるために不正確なタグを付ける",
    },
    decisionCriteria: {
...

  {
    ...
    correctPattern: {
      description: "キャッシュ行数 ≤ 本体行数。超えたらキャッシュ戦略を見直す",
    },
    outdatedPattern: {
      description:
        "キャッシュ制御（invalidation, memo, dedup）が本体より大きい",
    },
    decisionCriteria: {
...

  {
    ...
    correctPattern: {
      description:
        "useWidgetQueryContext / useWidgetDataOrchestrator 経由でデータを受け取る",
    },
    outdatedPattern: {
      description:
        "Chart コンポーネントから useDuckDB* hook を直接 import する",
      imports: ["useDuckDB"],
    },
    decisionCriteria: {
      ...
      escalation:
        "plan hook または useWidgetQueryContext 経由でデータを受け取る",
    },
    relationships: {
      dependsOn: ["AR-A1-PRES-INFRA"],
    },
    ...
    migrationRecipe: {
      steps: [
        "1. DuckDB hook の import を削除",
        "2. plan hook 経由でデータを受け取るよう変更",
      ],
    },
    protectedHarm: {
...

  {
    ...
    correctPattern: {
      description: "QueryHandler または resolver 内でのみ alignment を処理する",
    },
    outdatedPattern: {
      description: "コンポーネントや hook で直接 alignment を判定する",
    },
    decisionCriteria: {
...

  {
    ...
    outdatedPattern: {
      description:
        "ファイルの特徴を確認せずに R:utility を付ける（最も多い誤分類パターン）",
    },
    decisionCriteria: {
...

  {
    ...
    outdatedPattern: {
      description:
        "チャートコンポーネント内にデータ取得・状態管理・計算ロジックを混在させる",
    },
...

  {
    ...
    outdatedPattern: {
      description:
        "オーケストレーション hook 内で useState や副作用を直接管理する",
    },
...

  {
    ...
    decisionCriteria: {
      ...
      escalation:
        "R:utility → R:orchestration にタグ変更するか hooks を別ファイルに抽出",
    },
    relationships: {
      dependsOn: ["AR-A1-DOMAIN"],
    },
...

  {
    ...
    correctPattern: {
      description:
        "useMemo = 0, useCallback = 0, useState = 0, 50 行以内。export 文のみ",
    },
    outdatedPattern: {
...

  {
    ...
    correctPattern: {
      description:
        "件数は generated section から自動埋め込み。例外は理由付きで EXCEPTIONS に登録",
    },
    outdatedPattern: {
...

  {
    ...
    correctPattern: {
      description:
        "catch 内で console.warn/error + エラー状態の伝播。または上位に re-throw",
      example: `.catch((err) => { console.warn('[context]:', err); setError(err) })`,
    },
    outdatedPattern: {
      description:
        "エラーを無視する catch: 空 catch、コメントのみ catch、default 値返却のみ",
    ...
    decisionCriteria: {
      ...
      escalation:
        "エラーが「何のデータに影響するか」を考え、影響があるなら UI に伝播",
    },
    protectedHarm: {
      prevents: [
        "エラー原因の追跡不能",
...

  {
    ...
    correctPattern: {
      description:
        "await で完了を待つ。失敗時は UI に通知。非クリティカルなら明示コメント",
    },
    outdatedPattern: {
      description:
        "repo.save().catch(console.warn) のように await なしで Promise を放置",
    ...
    decisionCriteria: {
      ...
      exceptions:
        "Parquet キャッシュ等の非クリティカル保存。ただしコメント必須",
...

  {
    ...
    correctPattern: {
      description:
        "ReadModelSlice の status チェック、または isLoading/error の明示的分岐",
      example:
    ...
    decisionCriteria: {
      ...
      exceptions:
        "domain/calculations/ 内の純粋計算（同期データのデフォルト値）",
      escalation: "「このデータが null のとき、0 を表示して正しいか」で判断",
    },
    protectedHarm: {
      prevents: [
        "KPIカードのサイレント消滅（今回のバグの根本原因）",
        "月遷移時の stale data 表示",
      ],
    },
  },

  {
    ...
    outdatedPattern: {
      description:
        "validateImportData() を呼ぶが結果を確認せずデータを無条件で保存",
    ...
    decisionCriteria: {
      ...
      escalation:
        "「このバリデーションエラーを無視してデータを使ったら何が壊れるか」で判断",
    },
    protectedHarm: {
      prevents: [
        "不正 CSV による誤った KPI 計算",
        "重複レコードによる二重計上",
      ],
    },
  },
...

  {
    ...
    correctPattern: {
      description:
        "queryToObjects の safeParse を本番でも first-row モードで実行",
    },
    outdatedPattern: {
      description:
        "import.meta.env.DEV でガードし本番では Zod バリデーションをスキップ",
...

  {
    ...
    correctPattern: {
      description:
        "Promise.race([operation, timeout(30_000)]) でタイムアウトを強制",
    },
    outdatedPattern: {
    ...
    decisionCriteria: {
      ...
      escalation:
        "「この Promise が resolve されなかったらアプリはどうなるか」で判断",
    },
    protectedHarm: {
      prevents: [
        "Worker ハングによるアプリ無応答",
...

  {
    ...
    migrationRecipe: {
      steps: [
        "1. setCurrentMonthData で storeResults: new Map() を同時にセット",
      ],
    },
    decisionCriteria: {
...

  {
    ...
    decisionCriteria: {
      ...
      escalation:
        "テストファイルを grep して同じメッセージ文字列を持つアサーションを更新",
    },
    migrationRecipe: {
      steps: [
  ...
  },
  {
    ...
    correctPattern: {
      description:
        "tag: 'required' のエントリには必ず semanticClass + authorityKind を設定する",
      example:
        "semanticClass: 'business', authorityKind: 'business-authoritative'",
    },
    outdatedPattern: {
...

  {
    ...
    correctPattern: {
      description:
        "semanticClass で business / analytic を区別し、derived view で運用分離する",
    },
    outdatedPattern: {
    ...
    decisionCriteria: {
      ...
      escalation:
        "semanticClass の判断に迷ったら review-needed にして inventory に記録",
    },
    migrationRecipe: {
      steps: [
        "1. semantic-inventory-procedure.md の Q1-Q8 で判定",
...

  {
    ...
    correctPattern: {
      description:
        "contractId を設定する前に必ず semanticClass + authorityKind を設定する",
      example:
...

  {
    ...
    correctPattern: {
      description:
        "contractId が 'BIZ-' で始まるエントリは reason に業務意味を記載する",
      example:
        "contractId: 'BIZ-001', reason: '在庫法粗利（calculateGrossProfit 経由）'",
    },
    outdatedPattern: {
      description:
        "BIZ-XXX の contractId があるのに reason が空または汎用的すぎる",
    },
    detection: { type: "custom", severity: "gate" },
    decisionCriteria: {
      ...
      escalation:
        "業務意味を書けないなら business ではなく analytic か utility を検討",
    },
    migrationRecipe: {
      steps: [
...

  {
    ...
    correctPattern: {
      description:
        "contractId が 'ANA-' で始まるエントリは methodFamily を設定する",
    ...
    decisionCriteria: {
      ...
      escalation:
        "methodFamily を書けないなら analytic ではなく utility を検討",
    },
    migrationRecipe: {
      steps: [
...

  {
    ...
    decisionCriteria: {
      ...
      escalation:
        "率の所有元が不明な場合は contract-definition-policy.md を参照",
    },
    migrationRecipe: {
      steps: [
        "1. 率を使用している箇所を特定",
...

  {
    ...
    correctPattern: {
      ...
      example:
        "import { decompose5 } from '@/application/services/factorDecompositionBridge'",
    },
    outdatedPattern: {
      description:
        "application / presentation 層から domain/calculations/ を直接 import する",
...

  {
    ...
    correctPattern: {
      description:
        "UI の既定経路は current-only。candidate は dual-run-compare で検証してから切替",
    },
    outdatedPattern: {
      description:
        "bridge モードを candidate-only に変更して UI の既定動作にする",
    },
    detection: { type: "custom", severity: "gate" },
    decisionCriteria: {
      ...
      escalation:
        "candidate を既定にしたい場合は Phase 8 の Promote Ceremony を経る",
    },
    migrationRecipe: {
      steps: [
...

  {
    ...
    outdatedPattern: {
      description:
        "current エントリに dual-run / promotion-ready 等の candidate 状態を付与する",
    },
    detection: { type: "custom", severity: "gate" },
    decisionCriteria: {
...

  {
    ...
    outdatedPattern: {
      description:
        "current エントリのコメントや文書で authoritative を修飾なしで使用する",
    },
    detection: { type: "custom", severity: "gate" },
    decisionCriteria: {
...

  {
    ...
    correctPattern: {
      description:
        "BUSINESS_SEMANTIC_VIEW と ANALYTIC_KERNEL_VIEW を分離して管理する",
    },
    outdatedPattern: {
...

  {
    ...
    outdatedPattern: {
      description:
        "current の WASM crate やブリッジに candidate 用の分岐やコードを追加する",
    },
    detection: { type: "custom", severity: "gate" },
    decisionCriteria: {
      when: "current 群のソースコードを変更するとき",
      exceptions:
        "バグ修正・保守は許容。新しい計算ロジックの追加は candidate で行う",
      escalation:
        "判断に迷ったら current-maintenance-policy.md §7 の分離ルールを参照",
    },
    migrationRecipe: {
      steps: [
...

  {
    ...
    outdatedPattern: {
      description:
        "WASM exports や domain/calculations を直接 import する箇所を増やす",
    },
    detection: { type: "import", severity: "gate", baseline: 0 },
    decisionCriteria: {
...

  {
    ...
    correctPattern: {
      description:
        "candidate/business エントリには必ず contractId: 'BIZ-XXX' を設定する",
...

  {
    ...
    correctPattern: {
      description:
        "candidate は runtimeStatus: 'candidate' で MIGRATION_CANDIDATE_VIEW に配置する",
    },
    outdatedPattern: {
    ...
    decisionCriteria: {
      ...
      exceptions:
        "Phase 8 の Promote Ceremony を経た場合のみ current に編入可能",
...

  {
    ...
    correctPattern: {
      description:
        "bridgeKind: 'business' の candidate は business bridge のみに接続する",
    },
    outdatedPattern: {
      description:
        "business candidate を forecastBridge / timeSlotBridge に接続する",
    },
    detection: { type: "custom", severity: "gate" },
    decisionCriteria: {
...

  {
    ...
    outdatedPattern: {
      description:
        "candidate の WASM exports や domain/calculations/ を直接 import する",
    },
    detection: { type: "import", severity: "gate", baseline: 0 },
    decisionCriteria: {
...

  {
    ...
    outdatedPattern: {
      description:
        "candidate 実装を追加しただけで dual-run なしに promotion-ready にする",
    },
    detection: { type: "custom", severity: "gate" },
    decisionCriteria: {
...

  {
    ...
    correctPattern: {
      ...
      example:
        "runtimeStatus: 'candidate', contractId: 'ANA-001', methodFamily: 'time_pattern'",
    },
    outdatedPattern: {
      description:
        "runtimeStatus: 'candidate', semanticClass: 'analytic' で contractId が未設定",
    },
    detection: { type: "custom", severity: "gate" },
    decisionCriteria: {
...

  {
    ...
    correctPattern: {
      description:
        "bridgeKind: 'analytics' の candidate は analytics bridge のみに接続する",
    },
    outdatedPattern: {
      description:
        "analytic candidate を grossProfitBridge / budgetAnalysisBridge に接続する",
    },
    detection: { type: "custom", severity: "gate" },
    decisionCriteria: {
...

  {
    ...
    correctPattern: {
      description:
        "candidate/analytics エントリには必ず methodFamily を設定する",
      example: "methodFamily: 'time_pattern'",
    },
    outdatedPattern: {
      description:
        "semanticClass: 'analytic', runtimeStatus: 'candidate' で methodFamily が未設定",
    },
    detection: { type: "custom", severity: "gate" },
    decisionCriteria: {
...

  {
    ...
    decisionCriteria: {
      ...
      exceptions:
        "なし — analytic candidate は candidate view + analytics bridge のみ",
...

  {
    ...
    outdatedPattern: {
      description:
        "factorDecomposition を semanticClass: 'analytic' の candidate として登録する",
    },
    detection: { type: "custom", severity: "gate" },
    decisionCriteria: {
...

  {
    ...
    correctPattern: {
      description:
        "新しい authoritative 計算は candidate として registry に登録してから実装する",
    },
    outdatedPattern: {
      description:
        "domain/calculations/ に新しい authoritative 関数を直接追加する",
    },
    detection: { type: "custom", severity: "gate" },
    decisionCriteria: {
      when: "domain/calculations/ に新しい pure 関数を追加するとき",
      exceptions:
        "utility / not-needed の関数追加は対象外。既存関数の修正も対象外",
      escalation:
        "新しい authoritative 計算が必要な場合は candidate として登録する",
    },
    migrationRecipe: {
      steps: [
...

  {
    ...
    correctPattern: {
      description:
        "JS reference はバグ修正のみ。新しいロジックは candidate として追加する",
    },
    outdatedPattern: {
      description:
        "既存の TS ファイルに新しい authoritative 計算関数を追加する",
    },
    detection: { type: "custom", severity: "gate" },
    decisionCriteria: {
...

  {
    ...
    correctPattern: {
      description:
        "presentation 層のヘルパーは semanticClass: 'presentation' のまま維持する",
    },
    outdatedPattern: {
      description:
        "presentation/ のヘルパーを semanticClass: 'business' / 'analytic' に変更する",
    },
    detection: { type: "custom", severity: "gate" },
    decisionCriteria: {
...

  {
    ...
    correctPattern: {
      description:
        "review-needed を解決してから current 編入・candidate 化する",
    },
    outdatedPattern: {
      description:
        "tag: 'review' のまま runtimeStatus を current や candidate に変更する",
    },
    detection: { type: "custom", severity: "gate" },
    decisionCriteria: {
      ...
      escalation:
        "分類に迷ったら semantic-inventory-procedure.md の Q1-Q8 で判定",
    },
    migrationRecipe: {
      steps: [
...

  {
    ...
    correctPattern: {
      description:
        "master registry からフィルタで derived view を生成し、CI で一致を検証する",
    },
    outdatedPattern: {
...

  {
    ...
    correctPattern: {
      description:
        "import { ARCHITECTURE_RULES, getRuleById, ... } from '../architectureRules' のように facade 経由で参照する",
    },
    outdatedPattern: {
      description:
        "'architectureRules/rules' や '@project-overlay/execution-overlay' を consumer から直接 import する",
    },
    detection: { type: "custom", severity: "gate", baseline: 0 },
    decisionCriteria: {
      when: "ルール配列 / ルール情報へアクセスするコードを書くとき",
      exceptions:
        "architectureRules/merged.ts と executionOverlayGuard.test.ts のみ（正本合成点と構造検証点）",
      escalation:
        "直接 import を発見したら architectureRules facade 経由に差し替える",
    },
    migrationRecipe: {
      steps: [
        "1. '../architectureRules/rules' / '@project-overlay/execution-overlay' の import 行を探す",
        "2. '../architectureRules' からの名前付き import に差し替える",
        "3. 必要な export が facade にない場合は index.ts に re-export を追加する",
      ],
    },
    relationships: {
      dependsOn: [
    ...
    sunsetCondition:
      "物理移動完了後も facade 入口が 1 本に保たれていることが長期的に観測された",
    protectedHarm: {
      prevents: [
  ...
  {
    id: "AR-AAG-NO-BASE-RULES-CONSUMER-IMPORT",
    principleRefs: ["F1"],
    guardTags: ["F1"],
    slice: "governance-ops",
    ruleClass: "invariant",
    ...
    correctPattern: {
      description:
        "consumer は '../architectureRules' 経由で ARCHITECTURE_RULES（merged 後）を参照する",
    },
    outdatedPattern: {
      description:
        "from '../architectureRules/rules' で BaseRule 配列を直接 import する",
      imports: ["architectureRules/rules"],
    },
    detection: { type: "custom", severity: "gate", baseline: 0 },
    decisionCriteria: {
      when: "ARCHITECTURE_RULES 配列にアクセスしたいとき",
      exceptions:
        "architectureRules/merged.ts（正本合成点）と executionOverlayGuard.test.ts（BaseRule 対 overlay の整合検証）のみ",
      escalation: "rules.ts 直 import を facade 経由に差し替える",
    },
    migrationRecipe: {
      steps: [
        "1. from '../architectureRules/rules' を探す",
        "2. from '../architectureRules' に変更する（merge 後の ARCHITECTURE_RULES を受け取る）",
      ],
    },
    sunsetCondition:
      "BaseRule と ArchitectureRule の差分が運用上意識されなくなった",
    protectedHarm: {
      prevents: [
  ...
  {
    id: "AR-AAG-NO-DIRECT-OVERLAY-IMPORT",
    principleRefs: ["F1", "C6"],
    guardTags: ["F1", "C6"],
    slice: "governance-ops",
    ruleClass: "invariant",
    ...
    correctPattern: {
      description:
        "consumer は '../architectureRules' 経由で fixNow / executionPlan を受け取る",
    },
    outdatedPattern: {
      description:
        "from '@project-overlay/execution-overlay' を consumer から直接 import する",
      imports: ["@project-overlay/execution-overlay"],
    },
    detection: { type: "custom", severity: "gate", baseline: 0 },
    decisionCriteria: {
      when: "fixNow / executionPlan / reviewPolicy / lifecyclePolicy を参照したいとき",
      exceptions:
        "architectureRules/merged.ts（正本合成点）と executionOverlayGuard.test.ts（整合検証）のみ",
      escalation:
        "発見したら facade 経由に差し替え、必要なら index.ts に re-export を追加する",
    },
    migrationRecipe: {
      steps: [
        "1. from '@project-overlay/execution-overlay' を探す",
        "2. 必要な情報（fixNow / executionPlan 等）は ARCHITECTURE_RULES[] 要素から取れるので facade 経由に差し替え",
      ],
    },
    sunsetCondition:
      "Project Overlay が物理移動後も consumer 境界が保たれていることが観測された",
    protectedHarm: {
      prevents: [
  ...
  // ── Scope-Aware Mutation ──
  {
    ...
    decisionCriteria: {
      ...
      exceptions:
        "resetTables() のような DROP + CREATE（行レベル変更ではない）は対象外",
      escalation:
        "is_prev_year = true または is_prev_year = false を WHERE 句に追加する",
    },
    relationships: {
...

  {
    ...
    correctPattern: {
      description:
        "全 file が R:* または R:unclassified、全 test が T:* または T:unclassified を持つ",
    },
    outdatedPattern: {
      description:
        "@responsibility / @taxonomyKind タグなしの production file / test",
    },
    decisionCriteria: {
      when: "新規 file / test を作成するとき / 既存 file の責務を再判断するとき",
      exceptions:
        "R:unclassified / T:unclassified を能動的に付与する場合のみ許容（review window で承認後の脱出を前提）",
      escalation:
        "タグなしを発見したら R:unclassified / T:unclassified を即付与（review window で具体タグへ昇格）",
    },
    detection: { type: "custom", severity: "gate", baseline: 0 },
    migrationRecipe: {
      steps: [
        "1. 該当 file / test の責務を判断",
        "2. registry V2 の vocabulary に存在する R:tag / T:kind を選択",
        "3. 不明なら R:unclassified / T:unclassified を能動付与",
        "4. review window で具体タグへ昇格（必要に応じて）",
      ],
    },
    sunsetCondition:
      "taxonomy-v2 制度成立 + 全 file / test が能動分類された状態が連続 2 四半期維持された場合",
    protectedHarm: {
      prevents: [
        "責務不明の放置（False Green: タグなしで CI 通過）",
  ...
  {
    slice: "governance-ops",
    id: "AR-TAXONOMY-KNOWN-VOCABULARY",
    principleRefs: ["G1"],
    ruleClass: "invariant",
    guardTags: ["G1"],
    epoch: 1,
    ...
    correctPattern: {
      description:
        "responsibilityTaxonomyRegistryV2 / testTaxonomyRegistryV2 に登録された vocabulary のみ使用",
    },
    outdatedPattern: {
      description: "registry に存在しない R:foo / T:bar の使用",
    },
    decisionCriteria: {
      when: "新規 file / test に v2 タグを付与するとき",
      exceptions: "なし。新 vocabulary は review window 経由のみ",
      escalation:
        "使用したい vocabulary が registry にない場合は review window に提案",
    },
    detection: { type: "custom", severity: "gate", baseline: 0 },
    migrationRecipe: {
      steps: [
        "1. registry V2 の vocabulary を確認",
        "2. 既存タグで対応可能か判断",
        "3. 不可能なら review window に提案 entry を追加",
        "4. 四半期 review window で人間判断 → 採択後に registry に追加",
      ],
    },
    sunsetCondition: "なし（vocabulary 制御は恒久的 mechanism）",
    protectedHarm: {
      prevents: [
        "vocabulary の無秩序な拡張（Cognitive Load Ceiling 違反へ繋がる）",
  ...
  {
    slice: "governance-ops",
    id: "AR-TAXONOMY-ONE-TAG-ONE-AXIS",
    principleRefs: ["C1", "G1"],
    ruleClass: "invariant",
    guardTags: ["G1", "C1"],
    epoch: 1,
    ...
    correctPattern: {
      description:
        "R: prefix は責務軸、T: prefix はテスト軸として分離。1 file は両軸を持てるが、各軸は 1 タグまで",
    },
    outdatedPattern: {
      description:
        "責務 + 純粋性 + 層を 1 タグに押し込む（v1 の R:utility / R:state-machine 等）",
    },
    decisionCriteria: {
      when: "タグ vocabulary 設計時 / 新タグ提案時",
      exceptions: "なし",
      escalation: "混在タグを発見したら review window で軸分離を裁定",
    },
    detection: { type: "custom", severity: "gate", baseline: 0 },
    migrationRecipe: {
      steps: [
        "1. 各タグの「何の軸を表すか」を明文化",
        "2. 複数軸を表すタグは review window で分離提案",
        "3. v2 vocabulary では責務軸（R:）のみを R registry に登録",
      ],
    },
    sunsetCondition: "なし（軸分離は恒久的 mechanism）",
    protectedHarm: {
      prevents: [
...

  {
    ...
    correctPattern: {
      description:
        "各 R:tag を持つ file に対し、interlock マトリクスの required T:kind を持つ test が最低 1 件存在",
    },
    outdatedPattern: {
      description:
        "R:calculation を持つ file に T:unit-numerical / T:boundary を持つ test がない（contract 破綻）",
    },
    decisionCriteria: {
      when: "新規 R:tag 付与時 / interlock マトリクス変更時",
      exceptions:
        "sentinel タグ（R:unclassified ↔ T:unclassified）は obligation: may-have",
      escalation:
        "required T:kind の test を作成、または review window で interlock 修正提案",
    },
    detection: { type: "custom", severity: "gate", baseline: 0 },
    migrationRecipe: {
      steps: [
        "1. interlock マトリクス（taxonomy-interlock.md §2.1）で required T:kind を確認",
        "2. 該当 R:tag を持つ file の test に required T:kind を付与",
        "3. test が存在しない場合は新規作成",
        "4. obligation 強度（must-have / should-have / may-have）に応じて hard fail / WARN を判定",
      ],
    },
    sunsetCondition: "なし（双方向契約は恒久的 mechanism）",
    protectedHarm: {
      prevents: [
        "tag contract の破綻（R:tag が test obligation を発行しても test がない状態）",
  ...
  {
    slice: "governance-ops",
    id: "AR-TAXONOMY-ORIGIN-REQUIRED",
    principleRefs: ["G1"],
    ruleClass: "invariant",
    guardTags: ["G1"],
    epoch: 1,
    ...
    correctPattern: {
      description:
        "registry V2 の各 entry が origin: { why, when, who, sunsetCondition } を持つ",
    },
    outdatedPattern: {
      description: 'タグ採択経緯不明（why = "unknown" / 全 field 欠落）',
    },
    decisionCriteria: {
      when: "新規 R:tag / T:kind 追加時",
      exceptions:
        'legacy タグは "legacy, origin unknown" を許容（Phase 0 Inventory で遡及記録済）',
      escalation: "review window で Origin を確定して registry に登録",
    },
    detection: { type: "custom", severity: "gate", baseline: 0 },
    migrationRecipe: {
      steps: [
        "1. 新タグ提案時に why / when / who / sunsetCondition を明記",
        "2. review window で人間裁定",
        "3. 採択後に registry V2 + Origin Journal に entry 追加",
      ],
    },
    sunsetCondition: "なし（Origin 記録は恒久的 mechanism）",
    protectedHarm: {
      prevents: [
...

  {
    ...
    correctPattern: {
      description:
        "責務軸 vocabulary 数 ≤ 15 / テスト軸 vocabulary 数 ≤ 15（軸ごと別カウント）",
    },
    outdatedPattern: {
      description: "15 を超える vocabulary 数（例: 責務軸 16 件以上）",
    },
    decisionCriteria: {
      when: "新タグ追加 review window / 既存タグ retirement review window",
      exceptions: "なし",
      escalation:
        "15 超過時は新タグ追加を保留し、既存 retirement とセットで審議",
    },
    detection: { type: "custom", severity: "gate", baseline: 0 },
    migrationRecipe: {
      steps: [
        "1. registry V2 の vocabulary 数を確認",
        "2. 15 超過なら統廃合候補を選定",
        "3. review window で retirement + 新タグ追加をセット審議",
      ],
    },
    sunsetCondition: "なし（Cognitive Load 制御は恒久的 mechanism）",
    protectedHarm: {
      prevents: [
        "vocabulary 過剰による認知負荷増大（責務分類が機能しなくなる）",
  ...
  {
    slice: "governance-ops",
    id: "AR-TAXONOMY-AI-VOCABULARY-BINDING",
    principleRefs: ["G1"],
    ruleClass: "invariant",
    guardTags: ["G1"],
    epoch: 1,
    doc: "CLAUDE.md",
    what: "AI による review window 外の新 vocabulary 追加を block する",
    why: "原則 3（語彙生成は高コスト儀式）+ 8 昇華メカニズム #7（AI Vocabulary Binding）の機械的強制。AI（claude）は registry に存在する vocabulary のみ使用可能、新タグ提案は review window 経由のみ",
    correctPattern: {
      description:
        "AI が registry V2 vocabulary のみ使用 / 新タグ追加は review-journal.md 経由",
    },
    outdatedPattern: {
      description: "AI が独自判断で git diff に新 R:tag / T:kind を導入",
    },
    decisionCriteria: {
      when: "AI が新タグを使用しようとするとき / git diff に新 vocabulary が出現したとき",
      exceptions: "人間承認 review window で landing 済の vocabulary は使用可",
      escalation: "review window で提案 → 採択後に AI が使用可能となる",
    },
    detection: { type: "custom", severity: "gate", baseline: 0 },
    migrationRecipe: {
      steps: [
        "1. AI は registry V2 vocabulary のみ使用",
        "2. 新タグが必要な場合は review-journal.md に提案 entry を追加（AI 起案 → 人間承認）",
        "3. 四半期 review window で人間裁定 → 採択後に registry V2 を更新",
      ],
    },
    sunsetCondition:
      "なし（AI による無秩序な vocabulary 追加防止は恒久的 mechanism）",
    protectedHarm: {
      prevents: [
...

  {
    ...
    correctPattern: {
      description:
        "spec.lastVerifiedCommit が git log -1 --format=%h -- <sourceRef> の出力と prefix 一致（どちらかが他方の前方一致）。git `%h` は最小 unique abbreviation を返すため repo 成長で同じ commit でも必要文字数が増える (例: 7 chars `50018d3` → 8 chars `50018d33`)。完全一致だと false-positive が出るため prefix 一致で『同じ commit』を判定する。前提: full git history (CI は actions/checkout に `fetch-depth: 0` 指定必須、shallow clone では guard が skip + warn)",
    },
    outdatedPattern: {
  ...
  {
    slice: "governance-ops",
    id: "AR-CONTENT-SPEC-EVIDENCE-LEVEL",
    principleRefs: ["G1"],
    ruleClass: "invariant",
    guardTags: ["G1"],
    epoch: 1,
    ...
    migrationRecipe: {
      steps: [
        "1. spec body に Behavior Claims table (markdown) を追加",
    ...
    sunsetCondition:
      "なし（Evidence Level の構造強制は spec 主張 = 実装 の C 層保証として恒久的）",
    protectedHarm: {
      prevents: [
        "spec prose に書いた振る舞い主張が test / guard なしで silent 化（C 層 drift）",
        "high-risk claim が「人間が書いただけ」状態で commit される (asserted bypass)",
