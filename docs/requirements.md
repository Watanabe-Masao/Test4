...
> **スコープ**: 在庫販売のみ（花・産直・売上納品を除外）が対象。
> 値入率・売変率から逆算した経営判断用の予測値。

#### 3.2.2a 在庫法と推定法の関係（重要）

**在庫法の粗利と推定法の粗利は一致しない。** これは設計上意図された仕様である。

| | 在庫法（実績粗利） | 推定法（推定粗利） |
|---|---|---|
| **対象スコープ** | 全売上・全仕入（花・産直・売上納品含む） | 在庫販売のみ（花・産直・売上納品除外） |
| **売上の範囲** | 総売上高 | コア売上 |
| **原価の算出** | 期首在庫 + 総仕入高 - 期末在庫 | 粗売上 ×(1-値入率) + 消耗品費 |
| **算出根拠** | 実際の在庫増減（実績値） | 値入率・売変率からの逆算（予測値） |
| **用途** | 会計的な月次損益確認 | 月次速報、棚卸前異常検知、売変影響分析 |
| **必要データ** | 期首在庫・期末在庫（実績）が必須 | 値入率・売変率があれば算出可能 |

**実務上の使い分け:**
- 期末在庫が判明している場合 → **在庫法**で確定粗利を表示
- 期末在庫が未確定の場合（月中など） → **推定法**で速報粗利を表示
- 両方算出できる場合 → 差異を分析して異常検知に活用（→ 3.2.5 差異分析）
