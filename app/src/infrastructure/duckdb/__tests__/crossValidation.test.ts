...
describe('SQL/JS 計算パス構造的等価性: total_cost の構成要素が一致する', () => {
  ...
  it('SQL total_cost に consumable が含まれていない（JS getDailyTotalCost と一致）', () => {
    ...
    expect(
      totalCostExpr.includes('costInclusion'),
...
describe('SQL/JS 計算パス構造的等価性: discountRate / coreMarkupRate / estMethodCogs', () => {
  ...
  describe('estMethodCogs の計算式', () => {
    ...
    it('SQL: est_method_cogs = est_gross_sales * (1 - core_markup_rate) + costInclusion パターンが存在する', () => {
      expect(sqlSource).toContain(
        'est_gross_sales * (1 - e.core_markup_rate) + e.total_cost_inclusion',
      )
