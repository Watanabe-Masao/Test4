...
export function CustomerScatterChart({ daily, daysInMonth, year, month, prevYearDaily }: Props) {
  ...
  const { yoyData, avgCustChange, avgTxChange, yoyQuadrants, yoyDowGroups } = useMemo(() => {
    if (!prevYearDaily) return { yoyData: [], avgCustChange: 0, avgTxChange: 0, yoyQuadrants: { q1: 0, q2: 0, q3: 0, q4: 0 }, yoyDowGroups: new Map<number, typeof yoyPts>() }
    const yoyPts: { day: number; custChange: number; txChange: number; sales: number; dow: number; dowLabel: string }[] = []
    let totalCustC = 0, totalTxC = 0, cnt = 0
    for (let d = 1; d <= daysInMonth; d++) {
      const rec = daily.get(d)
      const customers = rec?.customers ?? 0
      if (customers <= 0) continue
      ...
      const date = new Date(year, month - 1, d)
      const dow = date.getDay()
      yoyPts.push({ day: d, custChange, txChange, sales: rec?.sales ?? 0, dow, dowLabel: DOW_LABELS[dow] })
    ...
    return { yoyData: yoyPts, avgCustChange: avgCC, avgTxChange: avgTC, yoyQuadrants: { q1, q2, q3, q4 }, yoyDowGroups: grp }
  }, [daily, daysInMonth, year, month, prevYearDaily])
  ...
  return (
    <Wrapper>
      <HeaderRow>
        ...
        {hasPrev && (
          <ViewToggle>
            <ViewBtn $active={axisMode === 'absolute'} onClick={() => setAxisMode('absolute')}>実数</ViewBtn>
            <ViewBtn $active={axisMode === 'yoyChange'} onClick={() => setAxisMode('yoyChange')}>前年比変化率</ViewBtn>
          </ViewToggle>
        )}
      </HeaderRow>
      <QuadrantGrid>
        {isYoy ? (
          ...
          <>
            <QuadrantTag $color={ct.colors.success}>高客数+高単価: {quadrantCounts.q1}日</QuadrantTag>
            <QuadrantTag $color={ct.colors.info}>低客数+高単価: {quadrantCounts.q2}日</QuadrantTag>
            <QuadrantTag $color={ct.colors.warning}>高客数+低単価: {quadrantCounts.q4}日</QuadrantTag>
            <QuadrantTag $color={ct.colors.danger}>低客数+低単価: {quadrantCounts.q3}日</QuadrantTag>
          </>
        )}
      </QuadrantGrid>
        ...
        <ScatterChart margin={{ top: 8, right: 20, left: 8, bottom: 8 }}>
          ...
          {isYoy ? (
            <>
              <XAxis
                ...
                label={{ value: '客数 前年比変化率', position: 'insideBottomRight', offset: -5, fontSize: ct.fontSize.xs, fill: ct.textMuted }}
              />
              <YAxis
                dataKey="txChange"
                type="number"
                ...
                tickFormatter={(v: number) => toPct(v, 0)}
                width={55}
                label={{ value: '客単価 前年比変化率', angle: -90, position: 'insideLeft', offset: 10, fontSize: ct.fontSize.xs, fill: ct.textMuted }}
              />
            </>
          ) : (
            <>
          ...
          <Tooltip
            ...
            formatter={(value, name) => {
              ...
              if (name === '客数変化率' || name === '客単価変化率') return [toPct(value as number), name]
          ...
          {isYoy ? (
            <>
              <ReferenceLine x={0} stroke={ct.colors.slate} strokeDasharray="6 4" strokeOpacity={0.8} />
              <ReferenceLine y={0} stroke={ct.colors.slate} strokeDasharray="6 4" strokeOpacity={0.8} />
              <ReferenceLine x={avgCustChange} stroke={ct.colors.info} strokeDasharray="3 3" strokeOpacity={0.4} />
              <ReferenceLine y={avgTxChange} stroke={ct.colors.info} strokeDasharray="3 3" strokeOpacity={0.4} />
            </>
          ) : (
            <>
          ...
          {isYoy
            ? Array.from(yoyDowGroups.entries()).map(([dow, points]) => (
                <Scatter key={dow} name={DOW_LABELS[dow]} data={points} fill={DOW_COLORS[dow]} fillOpacity={0.75} shape="circle" />
              ))
