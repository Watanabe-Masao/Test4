...
/**
 * StoreAggregation行データからチャートデータを構築する
 */
function buildChartData(
  ...
  for (const row of rows) {
    ...
    const prev = hourMap.get(row.hour) ?? 0
    hourMap.set(row.hour, prev + row.amount)

    const totalPrev = storeTotals.get(row.storeId) ?? 0
    storeTotals.set(row.storeId, totalPrev + row.amount)
  }
  ...
  return { chartData, storeInfos }
}

// ── Component ──

export function DuckDBStoreHourlyChart({
  duckConn,
  duckDataVersion,
  currentDateRange,
  selectedStoreIds,
