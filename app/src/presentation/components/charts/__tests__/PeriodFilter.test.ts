...
describe('前年比較: 当年・前年の実データ日数に基づく個別除算', () => {
   ...
   */
  function computeAverage(totalAmount: number, actualDataDays: Set<number>): number {
...
describe('実データ駆動型除数 (computeDataDivisor)', () => {
  ...
  it('dailyAvg モードで distinct day 数を返す', () => {
    ...
    const records = [
      makeRecord(1),
      makeRecord(1, 20000, 'S2'), // day 1: 2レコード
  ...
  it('期間選択（1-7日）vs（1-28日）でデータ除数が正しく変わる', () => {
    // 全28日分のレコード
    const allRecords = Array.from({ length: 28 }, (_, i) => makeRecord(i + 1, 10000))
...
describe('曜日ごとの実データ駆動型除数 (computeDataDowDivisors)', () => {
  ...
  it('カレンダーベースのDOWカウントと実データベースの差異', () => {
    ...
    const records = [makeRecord(2), makeRecord(9), makeRecord(23)]
...
describe('【TR-DIV-002】countDistinctDays', () => {
  ...
  it('異なる日のレコードは個別にカウント', () => {
    const records = [makeRecord(1), makeRecord(5), makeRecord(10), makeRecord(20)]
...
describe('スライダー動的変更に対する除数追従', () => {
  // 全28日分のデータ
  const allRecords = Array.from({ length: 28 }, (_, i) => makeRecord(i + 1, (i + 1) * 10_000))
...
describe('平均計算 E2E: 一元管理関数経由の検証', () => {
  it('computeDivisor を使った平均と手動計算の結果が一致する', () => {
    const records = [makeRecord(2, 100_000), makeRecord(9, 120_000), makeRecord(16, 80_000)]
    const total = records.reduce((s, r) => s + r.totalAmount, 0) // 300,000
  ...
  it('当年・前年で独立した除数を computeDivisor で算出', () => {
    ...
    const prevRecords = [makeRecord(3, 100_000), makeRecord(10, 100_000), makeRecord(24, 100_000)]
...
describe('構造テスト: computeDivisor の一元管理', () => {
  ...
  it('computeDivisor と countDistinctDays の組合せで computeDataDivisor と同一結果', () => {
    const records = [makeRecord(1), makeRecord(1, 20000, 'S2'), makeRecord(5), makeRecord(10)]
