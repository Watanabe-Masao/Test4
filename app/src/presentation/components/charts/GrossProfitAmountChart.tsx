...
function buildGpData(
  ...
  let cumSales = 0, cumCost = 0, prevCumSales = 0, prevCumCost = 0
  ...
  for (let d = 1; d <= daysInMonth; d++) {
    ...
    if (rec) { cumSales += rec.sales; cumCost += rec.totalCost }
    ...
    if (hasPrevGp) {
      ...
      prevRate = prevCumSales > 0 ? calculateGrossProfitRate(prevCumSales - prevCumCost, prevCumSales) : null
    }
...
export const GrossProfitAmountChart = memo(function GrossProfitAmountChart({
  daily, daysInMonth, targetRate, warningRate, year, month,
  prevYearDaily, prevYearCostMap,
}: Props) {
  ...
  const data = useMemo(() => allData.filter((d) => d.day >= rangeStart && d.day <= rangeEnd), [allData, rangeStart, rangeEnd])
  ...
  const option = useMemo<EChartsOption>(() => {
    ...
    if (gpView === 'rateOnly') {
      const series: EChartsOption['series'] = [{
        ...
        markLine: {
          data: [
            { yAxis: targetRate, label: { formatter: `目標 ${toPct(targetRate)}`, position: 'end', fontSize: 9 }, lineStyle: { color: theme.chart.barPositive, type: 'dashed' } },
          ],
          symbol: 'none',
        },
      }]
      ...
      return {
        ...
        yAxis: { type: 'value', axisLabel: { formatter: (v: number) => toPct(v), color: theme.colors.text3, fontSize: 10 }, axisLine: { show: false }, splitLine: { lineStyle: { color: theme.colors.border, opacity: 0.3, type: 'dashed' } } },
        series,
      }
    }
    ...
    const series: EChartsOption['series'] = [
      {
        ...
        type: 'bar',
        yAxisIndex: 0,
        data: data.map((d) => ({ value: d.grossProfit, itemStyle: { color: getBarColor(d.rate), opacity: 0.7, borderRadius: [2, 2, 0, 0] } })),
        barMaxWidth: 14,
      },
      {
        name: '粗利率',
  ...
  const titleText = gpView === 'rateOnly' ? '粗利率推移（累計ベース）' : '粗利額累計推移（バー: 粗利額 / ライン: 粗利率）'
  const toolbar = <SegmentedControl options={VIEW_OPTIONS} value={gpView} onChange={setGpView} ariaLabel="表示モード" />
