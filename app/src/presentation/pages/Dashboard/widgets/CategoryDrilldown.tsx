...
export function CategoryDrilldown({
    ...
    const tooltipFn = (
      ...
      return (
        <>
          ...
          <div>販売構成比: {pct}</div>
          <div>
            販売金額: {fmtSen(isAmountMode ? val : isPrev ? (it.prevAmount ?? 0) : it.amount)}
          </div>
          {!isAmountMode && <div>数量: {val.toLocaleString()}点</div>}
          {!isPrev && diff != null && (
            <div>
              {compLabel}差: {diff >= 0 ? '+' : ''}
              {isAmountMode ? fmtSen(diff) : `${diff.toLocaleString()}点`}
            </div>
          )}
          {!isPrev && yoy && (
            <div>
  ...
  if (dayItemsYoY.length === 0 && cumItemsYoY.length === 0 && actual <= 0 && cumSales <= 0)
    return null
  ...
  return (
    <DrillSection>
      ...
      <ToggleBar>
        ...
        <ToggleGroup>
          <ToggleBtn
            $active={compare === 'daily'}
            onClick={() => {
              setCompare('daily')
              setDrillSourceRow('actual')
            }}
          >
            単日
          </ToggleBtn>
          <ToggleBtn
            $active={compare === 'cumulative'}
            onClick={() => {
              setCompare('cumulative')
              setDrillSourceRow('actual')
            }}
          >
            累計
        ...
        <ToggleGroup>
          <ToggleBtn
            $active={drillSourceRow === 'actual'}
            onClick={() => setDrillSourceRow('actual')}
          >
            実績
          </ToggleBtn>
          <ToggleBtn $active={drillSourceRow === 'prev'} onClick={() => setDrillSourceRow('prev')}>
            前年
          </ToggleBtn>
          {hasWoW && compare === 'daily' && (
            <ToggleBtn $active={drillSourceRow === 'wow'} onClick={() => setDrillSourceRow('wow')}>
              前週
            </ToggleBtn>
          )}
        </ToggleGroup>
      ...
      <SummaryRow>
        <SumItem>
          <SumLabel>{levelLabels[currentLevel]}数</SumLabel>
          <SumValue>{items.length}</SumValue>
        </SumItem>
        <SumItem>
          <SumLabel>合計（{drillSourceLabel}）</SumLabel>
          <SumValue>{fmtVal(displayTotal)}</SumValue>
        </SumItem>
        {summaryBudget > 0 && (
          <>
            <SumItem>
              <SumLabel>予算額</SumLabel>
              <SumValue>{fmtSen(summaryBudget)}</SumValue>
            </SumItem>
            <SumItem>
              <SumLabel>予算差異</SumLabel>
              <SumValue>
                <YoYVal $positive={budgetDiff >= 0}>{fmtSen(budgetDiff)}</YoYVal>
              </SumValue>
            </SumItem>
            <SumItem>
              <SumLabel>予算達成率</SumLabel>
              <SumValue>
                <YoYVal $positive={budgetAch >= 1}>{formatPercent(budgetAch, 2)}</YoYVal>
              </SumValue>
            </SumItem>
          </>
        )}
        {hasPrevYear && summaryPrevYear > 0 && (
          <>
            <SumItem>
              ...
              <SumValue>{fmtSen(summaryPrevYear)}</SumValue>
            </SumItem>
            <SumItem>
              <SumLabel>前年差異</SumLabel>
              <SumValue>
                <YoYVal $positive={pyDiff >= 0}>{fmtSen(pyDiff)}</YoYVal>
              </SumValue>
            </SumItem>
            <SumItem>
              <SumLabel>前年対比</SumLabel>
              <SumValue>
                <YoYVal $positive={pyRatio >= 1}>{formatPercent(pyRatio, 2)}</YoYVal>
              </SumValue>
            </SumItem>
          </>
        )}
        {hasWoW && compare === 'daily' && summaryWow > 0 && (
          <>
            <SumItem>
              ...
              <SumValue>{fmtSen(summaryWow)}</SumValue>
            </SumItem>
            <SumItem>
              <SumLabel>前週差異</SumLabel>
              <SumValue>
                <YoYVal $positive={wowDiff >= 0}>{fmtSen(wowDiff)}</YoYVal>
              </SumValue>
            </SumItem>
            <SumItem>
              <SumLabel>前週対比</SumLabel>
              <SumValue>
                <YoYVal $positive={wowRatio >= 1}>{formatPercent(wowRatio, 2)}</YoYVal>
              </SumValue>
            </SumItem>
          </>
        )}
        ...
        <DrillTable>
          ...
          <tbody>
            {sorted.map((it, i) => {
              ...
              // YoY comparison
              const yoyItem =
                effectiveSource === 'wow' ? dayItemsYoY.find((y) => y.code === it.code) : it
              const yoyCounterpart =
                isPrevSource && effectiveSource === 'prev'
                  ? isAmountMode
                    ? it.amount
                    : it.quantity
                  : isAmountMode
              ...
              // WoW comparison
              const wowItem = effectiveSource === 'wow' ? it : wowItemMap.get(it.code)
              const wowCounterpart =
                isPrevSource && effectiveSource === 'wow'
                  ? isAmountMode
                    ? it.amount
                    : it.quantity
                  : isAmountMode
                ...
                >
                  ...
                  <DTdAmt>
                    <AmtWrap>
                      <AmtTrack>
                        <AmtFill
                          $pct={maxVal > 0 ? (mainVal / maxVal) * 100 : 0}
                          $color={it.color}
                        />
                      </AmtTrack>
                  ...
                  <DTd $mono>
                    {isAmountMode ? `${subVal.toLocaleString()}点` : `${toComma(subVal)}円`}
                  </DTd>
                  {hasPrevYear && (
                    <>
