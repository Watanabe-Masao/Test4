...
export function RangeComparisonPanel({
  rangeAData,
  rangeBData,
  prevYear,
  ...
  if (singleRange) {
    return (
      <RangeSummaryPanel>
        <RangeSummaryTitle>
          期間集計: {singleRange.start}～{singleRange.end}日（{singleRange.salesDaysCount}営業日）
        </RangeSummaryTitle>
        <div style={{ padding: '16px' }}>
          <RangeSummaryGrid>
            ...
            <RangeSummaryItem>
              ...
              <RangeSummaryItemValue $color={sc.cond(singleRange.ach >= 1)}>
                {formatPercent(singleRange.ach)}
              </RangeSummaryItemValue>
            </RangeSummaryItem>
            {prevYear.hasPrevYear && singleRange.pySales > 0 && (
              <>
                ...
                <RangeSummaryItem>
                  ...
                  <RangeSummaryItemValue $color={sc.cond(singleRange.pyRatio >= 1)}>
                    {formatPercent(singleRange.pyRatio)}
                  </RangeSummaryItemValue>
                </RangeSummaryItem>
              </>
            )}
  ...
  // 3-column compare
  const cmpColor = (a: number, b: number) =>
    a > b ? sc.positive : a < b ? sc.negative : sc.neutral
  ...
  const pyA = rangeAData.pySales,
    pyB = rangeBData.pySales
      ...
      {prevYear.hasPrevYear && d.pySales > 0 && (
        <>
          ...
          <RangeMetricRow>
            ...
            <RangeMetricValue $color={sc.cond(d.pyRatio >= 1)}>
              {formatPercent(d.pyRatio)}
            </RangeMetricValue>
          </RangeMetricRow>
        </>
      )}
  ...
  return (
    <RangeSummaryPanel>
      <RangeSummaryTitle>
        期間比較分析: {rangeAData.start}～{rangeAData.end}日 vs {rangeBData.start}～{rangeBData.end}
        日
      </RangeSummaryTitle>
      <RangeCompareContainer>
        {/* Left: Period A */}
