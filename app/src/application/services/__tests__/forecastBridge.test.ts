...
/* ── 5. ForecastMismatchLog shape（WASM 接続済み） ── */

describe('forecast WASM モック経由の安全性', () => {
  it('WASM モック関数が正しい値を返す', () => {
...
/* ── 6. dual-run-compare + WASM 未接続: compare 不発、TS 結果を返す ── */

describe('dual-run-compare: WASM 未初期化時は TS 結果のみ（compare なし）', () => {
  it('calculateStdDev: TS 結果を返し warn なし', () => {
    setExecutionMode('dual-run-compare')
    // getForecastWasmExports() returns null by default → isWasmReady()=false
    const spy = vi.spyOn(console, 'warn')
    const values = [100, 200, 300, 400, 500]
    const result = calculateStdDev(values)
    expect(result).toEqual(calculateStdDevDirect(values))
...
/* ── 6b. dual-run-compare + WASM ready: mismatch 検出 ── */

describe('dual-run-compare + WASM ready: mismatch を検出して warn', () => {
  beforeEach(() => {
    setExecutionMode('dual-run-compare')
    // WASM ready に見せかける
  ...
  it('calculateStdDev: モック WASM とずれる → warn が出る', () => {
    const spy = vi.spyOn(console, 'warn')
    const values = [100, 200, 300, 400, 500]
    const result = calculateStdDev(values)
    // TS 結果が返される
    expect(result).toEqual(calculateStdDevDirect(values))
    // モック WASM (mean:99999, stdDev:99999) との差分で warn が出る
    expect(spy).toHaveBeenCalled()
  })

  it('linearRegression: モック WASM とずれる → warn が出る', () => {
    const spy = vi.spyOn(console, 'warn')
    const result = linearRegression(sampleDailySales)
    expect(result).toEqual(linearRegressionDirect(sampleDailySales))
    expect(spy).toHaveBeenCalled()
