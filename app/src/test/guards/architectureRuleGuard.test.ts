...
describe('AAG Self-Quality: ルール自身がルールの品質基準を満たす', () => {
  ...
  it('migrationRecipe/executionPlan と migrationPath の二重定義が一致する', () => {
    const drifted: string[] = []
    for (const rule of ARCHITECTURE_RULES) {
      if (!rule.migrationRecipe && !rule.executionPlan) continue
      if (!rule.migrationPath) {
        drifted.push(`${rule.id}: migrationRecipe/executionPlan があるが migrationPath がない`)
        continue
      }
      if (
        rule.migrationRecipe &&
        JSON.stringify(rule.migrationRecipe.steps) !== JSON.stringify(rule.migrationPath.steps)
      ) {
        drifted.push(`${rule.id}: migrationRecipe.steps ≠ migrationPath.steps`)
      }
      if (rule.executionPlan) {
        if (rule.executionPlan.effort !== rule.migrationPath.effort) {
          drifted.push(`${rule.id}: executionPlan.effort ≠ migrationPath.effort`)
        }
        if (rule.executionPlan.priority !== rule.migrationPath.priority) {
          drifted.push(`${rule.id}: executionPlan.priority ≠ migrationPath.priority`)
        }
      }
    }
    expect(
      drifted,
      '新形式（migrationRecipe/executionPlan）と旧形式（migrationPath）が一致しない。' +
        '移行期間中は両方を同期させる必要がある',
    ).toEqual([])
  })
