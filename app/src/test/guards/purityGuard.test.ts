...
describe('B1 拡張: domain/calculations の金銭計算関数は WASM bridge が存在すること', () => {
  ...
  const TS_ONLY_ALLOWLIST = new Set([
    // ユーティリティ（Rust 化候補 — Phase 3 で対応）
    'utils.ts',
    'aggregation.ts',
    'divisor.ts',
    'remainingBudgetRate.ts',
    'averageDivisor.ts',
    // forecast.ts は Date 依存関数を含む（Date 非依存部分は Rust 化済み）
    'forecast.ts',
    // ルールエンジン・判定ロジック（移動候補 → application/rules/）
    'observationPeriod.ts',
    'rules/alertSystem.ts',
    'rules/conditionResolver.ts',
    // データ検出・天気マッピング（移動候補 → infrastructure/）
    'dataDetection.ts',
    'forecastWeatherMapping.ts',
    'weatherAggregation.ts',
    // 集約・フィルタ（移動候補 → application/query-bridge/）
    'rawAggregation.ts',
    ...
    // フォーマッター・合成（移動候補 → application/analysis/ or domain/formatting/）
    'causalChain.ts',
