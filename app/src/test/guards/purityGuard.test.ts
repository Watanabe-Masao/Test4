...
describe('B1 拡張: domain/calculations の金銭計算関数は WASM bridge が存在すること', () => {
  ...
  const TS_ONLY_ALLOWLIST = new Set<string>(['piValue.ts', 'customerGap.ts'])
  ...
  it('domain/calculations/ に計算以外のファイルを新規追加していない', () => {
    ...
    expect(
      violations,
      `domain/calculations/ に計算以外のファイルが検出されました:\n${violations.join('\n')}\n` +
        '→ ルールエンジンは application/rules/、マッピングは infrastructure/、' +
        '集約は application/query-bridge/、フォーマッターは domain/formatting/ に配置してください。',
    ).toEqual([])
  })
})
...
describe('Platform 境界ガード', () => {
  ...
  it('platform/ が features/ を直接 import しない（逆依存禁止）', () => {
    ...
    expect(
      violations,
      `platform/ が features/ を import しています:\n${violations.join('\n')}\n` +
        '→ platform は features に依存してはなりません。Port 契約を domain/ に定義してください。',
    ).toEqual([])
  })
  ...
  it('features/ が platform/ を import する場合は Port 経由のみ', () => {
    ...
    expect(
      violations,
      `features/ が platform/ を直接 import しています:\n${violations.join('\n')}\n` +
        '→ features → platform の依存は domain/ports/ の Port 経由のみ許可されます。',
    ).toEqual([])
  })
})
