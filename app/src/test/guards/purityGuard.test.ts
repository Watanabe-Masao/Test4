...
describe('許可リスト増加防止ガード', () => {
  it('domain/ の 300行超除外ファイルは増やさない', () => {
    ...
    expect(
      largeFileCount,
      `[${rule.id}] domain/ の 300行超ファイルが ${largeFileCount} 件（上限: ${MAX_DOMAIN_EXCLUSIONS}）`,
  ...
  it('infrastructure/ の 400行超除外ファイルは増やさない', () => {
    ...
    expect(
      largeFileCount,
      `[${rule.id}] infrastructure/ の 400行超ファイルが ${largeFileCount} 件（上限: ${MAX_INFRA_EXCLUSIONS}）`,
...
describe('B1 拡張: domain/calculations の金銭計算関数は WASM bridge が存在すること', () => {
  ...
  // 全件移動完了（weather → domain/weather/、rawAggregation → application/query-bridge/）
  const NON_CALCULATION_FILES = new Set<string>([])

  // TS のみで許容するファイル（safeDivide の薄いラッパーで Rust 化は過剰）
  const TS_ONLY_ALLOWLIST = new Set<string>([
    'piValue.ts',
    'customerGap.ts',
    // budgetSimulator.ts: 既存 projectLinear / calculateYoYRatio /
    // calculateAchievementRate / prorateBudget の orchestration 層。
    // 新規 authoritative 数値計算は無く、WASM 化の意義は薄い。
    'budgetSimulator.ts',
    // budgetSimulatorAggregations.ts: budgetSimulator のサイズ制限対応で
    // 分離した集計関数群 (aggregateDowAverages / aggregateWeeks)。
    // 単純な合計・平均・週境界判定で WASM 化不要。
    'budgetSimulatorAggregations.ts',
    // prevYearCostApprox.ts: SP-B ADR-B-004 PR2 で抽出した近似計算
    // (sales - discount)。Map 走査と単純減算のみで WASM 化は過剰。
  ...
  it('NON_CALCULATION_FILES のサイズが増加していない（段階的移動）', () => {
    const MAX_NON_CALC = 0
    expect(
      NON_CALCULATION_FILES.size,
      `[${rule.id}] NON_CALCULATION_FILES が ${NON_CALCULATION_FILES.size} 件（上限: ${MAX_NON_CALC}）。` +
        'ファイル移動が完了したらエントリを削除してください。',
    ).toBeLessThanOrEqual(MAX_NON_CALC)
  })

  it('TS_ONLY_ALLOWLIST のサイズが増加していない（段階的 Rust 化）', () => {
    // 5 = 4 (既存) + 1 (prevYearCostApprox.ts: SP-B ADR-B-004 PR2 で
    // registryChartWidgets.tsx から抽出した近似計算。Map 走査と単純減算のみで
    // WASM 化は過剰。Rust 化対象から除外する trivial 関数として登録)。
    const MAX_TS_ONLY = 5
    expect(
      TS_ONLY_ALLOWLIST.size,
      `[${rule.id}] TS_ONLY_ALLOWLIST が ${TS_ONLY_ALLOWLIST.size} 件（上限: ${MAX_TS_ONLY}）。` +
        'Rust 化が進んだらエントリを削除してください。',
    ).toBeLessThanOrEqual(MAX_TS_ONLY)
  })

  it('domain/calculations/ に計算以外のファイルを新規追加していない', () => {
    // domain/calculations/ は Authoritative 金銭計算のみ。
    // 以下のパターンを含むファイルは別の層に配置すべき:
    // - ルールエンジン → application/rules/
    // - マッピングテーブル → infrastructure/
    // - 結果集約 → application/query-bridge/
    // - フォーマッター → domain/formatting/
    // - 比較モデル → domain/models/
    const PROHIBITED_PATTERNS = [
      /[Rr]ule/, // ルールエンジン
      /[Mm]apping/, // マッピングテーブル
      /[Ff]ormatter/, // フォーマッター
      /^raw[A-Z].*ggregat/, // rawAggregation 系（aggregation.ts は計算なので除外）
      /[Dd]etect/, // データ検出
      /[Ww]eather/, // 天気データ処理
      /[Cc]ausal/, // 因果連鎖オーケストレーション
    ]

    // 既存の NON_CALCULATION_FILES は移行完了まで許容
    const LEGACY_EXCEPTIONS = NON_CALCULATION_FILES
    ...
    for (const file of files) {
      ...
      if (relPath.startsWith('__tests__')) continue
      if (relPath === 'index.ts' || relPath.endsWith('/index.ts')) continue
      if (LEGACY_EXCEPTIONS.has(relPath)) continue

      const basename = path.basename(relPath, '.ts')
      for (const pattern of PROHIBITED_PATTERNS) {
        if (pattern.test(basename)) {
          violations.push(
            `${relPath} — ${pattern} にマッチ。domain/calculations/ に配置しないでください`,
          )
        }
      }
    }
...
// ─── Platform 境界ガード ─────────────────────────────────
// @guard platform → feature 逆依存禁止、feature → platform は Port 経由のみ

describe('Platform 境界ガード', () => {
  /**
   * platform/ が将来新設されたとき、feature → platform の直接依存と
   * platform → feature の逆依存を防止する。
   * 現時点では platform/ は未作成だが、作成時に即座に検出できるよう guard を先行配置する。
   */

  it('platform/ が features/ を直接 import しない（逆依存禁止）', () => {
    const platformDir = path.join(SRC_DIR, 'platform')
    if (!fs.existsSync(platformDir)) return // platform/ 未作成なら skip

    const files = collectTsFiles(platformDir)
    const violations: string[] = []
  ...
  it('features/ が platform/ を import する場合は Port 経由のみ', () => {
    const platformDir = path.join(SRC_DIR, 'platform')
    if (!fs.existsSync(platformDir)) return // platform/ 未作成なら skip
    ...
    for (const file of files) {
      ...
      for (const imp of imports) {
        if (imp.includes('platform/') || imp.includes('@/platform/')) {
          violations.push(
            `${rel(file)}: import '${imp}' — features → platform 直接依存。` +
              'domain/ports/ の Port 型を経由してください。',
          )
        }
      }
    }
