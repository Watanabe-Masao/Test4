...
describe('B1 拡張: domain/calculations の金銭計算関数は WASM bridge が存在すること', () => {
  ...
  const NON_CALCULATION_FILES = new Set([
    // 移動完了: yoyComparison, dataDetection, causalChainFormatters, causalChain, causalChainSteps, rules/*
    'forecastWeatherMapping.ts', // → infrastructure/weather/
    'weatherAggregation.ts', // → infrastructure/weather/
    'rawAggregation.ts', // → application/query-bridge/
