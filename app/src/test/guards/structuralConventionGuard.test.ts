...
describe('Structural Convention Guard', () => {
  ...
  for (const barrelRule of SUB_BARREL_RULES) {
    it(`${barrelRule.mainBarrel} のメインバレル直接 import を禁止（サブバレル経由に移行済み）`, () => {
      const files = collectTsFiles(SRC_DIR)
      const violations: string[] = []
      const mainBarrelPattern = new RegExp(
        `from '${barrelRule.mainBarrel.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}'`,
      )

      for (const file of files) {
        const rel = relativePath(file)
        // バレル定義ファイル自体は除外
        if (rel.startsWith(`${barrelRule.barrelDir}/`)) continue
        // テスト・stories は除外
      ...
      expect(
        violations.length,
        `[${rule.id}] メインバレル '${barrelRule.mainBarrel}' からの直接 import が検出されました。\n` +
          `サブバレル（record/storeTypes/calendar/analysis 等）を使用してください。\n` +
          `違反ファイル:\n${violations.join('\n')}`,
      ).toBeLessThanOrEqual(barrelRule.maxViolations)
    })
  }
...
describe('ImportedData Migration Guard', () => {
  ...
  it('ImportedData の direct import 数が上限以下（増加禁止）', () => {
    ...
    expect(
      count,
      `[${rule.id}] ImportedData direct import: ${count}/${MAX_IMPORTED_DATA_DIRECT_IMPORTS}。` +
...
describe('P0-2: 全許可リストエントリに lifecycle が付与されている', () => {
  ...
  it('未分類エントリが 0 件', () => {
    ...
    expect(unclassified, formatViolationMessage(rule, unclassified)).toEqual([])
  })
