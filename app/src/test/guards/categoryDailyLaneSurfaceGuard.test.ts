 ...
 *   - presentation-cts-surface-ratchetdown Phase 3 (2026-04-19): baseline 6 → 0
 *     - builders.ts: 3 → 0 (Shapley 5-factor leaf-grain 入力を
 *       `CategoryLeafDailyEntry` 参照に切替、raw `CategoryTimeSalesRecord`
 *       直接 import を排除)
 *     - data.ts: 3 → 0 (同上。buildFactorData の leaf-grain 入力を
 *       `CategoryLeafDailyEntry` 経由に)
 *   - 旧 "intentional permanent floor" (Shapley leaf-grain 要件) は解消済み:
 *     `CategoryLeafDailyEntry` (application/hooks/categoryLeafDaily/
 *     CategoryLeafDailyBundle.types) が `CategoryTimeSalesRecord` alias として
 *     leaf-grain 構造を保持するため、raw CTS 型を import せずに
 *     Shapley 5-factor (dept|line|klass) を計算可能。
 *     categoryLeafDailyLaneSurfaceGuard が presentation 全体の CTS surface を
 *     監視する後継 guard として本 guard の scope を覆う。
 */
const CATEGORY_DAILY_RAW_ACCESS_BASELINES: readonly BaselineEntry[] = [
  {
    ...
    maxOccurrences: 0,
    reason:
      'presentation-cts-surface-ratchetdown Phase 3: Shapley 5-factor leaf-grain 入力を CategoryLeafDailyEntry 参照に切替、raw CategoryTimeSalesRecord 直接 import を排除。完全 0 到達',
  },
  {
    path: 'presentation/pages/Dashboard/widgets/YoYWaterfallChart.data.ts',
    maxOccurrences: 0,
    reason:
      'presentation-cts-surface-ratchetdown Phase 3: buildFactorData の leaf-grain 入力を CategoryLeafDailyEntry 参照に切替、raw CategoryTimeSalesRecord 直接 import を排除。完全 0 到達',
  },
  {
