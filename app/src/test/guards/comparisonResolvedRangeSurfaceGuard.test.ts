 ...
 * allowlist。**Phase 3 完了時点で 1 件** (`YoYChart.tsx`)。
 *
 * 新規追加は原則禁止。Phase 5 (ViewModel / chart 薄化) で `YoYChart.tsx` を
 * 移行することでベースラインを 0 に縮退させる計画。
 */
const ALLOWLIST: readonly { readonly path: string; readonly reason: string }[] = [
  {
    path: 'presentation/components/charts/YoYChart.tsx',
    reason:
      'Phase 2b 導入前から scope?.effectivePeriod2 / effectivePeriod1 / alignmentMode を直接参照している chart。Phase 5 (ViewModel / chart 薄化) で ComparisonResolvedRange contract 経由に移行予定。それまでは既存動作を保つため allowlist で暫定許容',
  },
]
...
describe('ComparisonResolvedRange Surface Guard (unify-period-analysis Phase 3 封じ手)', () => {
  ...
  it('ALLOWLIST が baseline 1 件以下である (ratchet-down)', () => {
    // Phase 3 完了時点で YoYChart.tsx 1 件のみ。Phase 5 で 0 に縮退する計画。
