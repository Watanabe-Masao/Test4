 ...
 *
 * App Domain（rules.ts）と Project Overlay（execution-overlay.ts）の
 * 整合性を検証する。
 *
 * 検証項目:
 * 1. rules.ts の全 rule に overlay が存在する
 * 2. overlay に未知の ruleId がない
 * 3. 全 rule が executionPlan を持つ（merge 後）
 * 4. 全 rule が fixNow を持つ（merge 後）
 * 5. overlay の effort が有効値である
 * 6. overlay の priority が非負整数である
 *
...
describe('Execution Overlay Guard', () => {
  it('rules.ts の全 rule に overlay が存在する', () => {
    const missing: string[] = []
    for (const rule of BASE_RULES) {
      if (!EXECUTION_OVERLAY[rule.id]) {
        missing.push(rule.id)
      }
    }
    expect(
      missing,
      'overlay 欠損ルール（projects/pure-calculation-reorg/aag/execution-overlay.ts に追加してください）',
    ).toEqual([])
  })
