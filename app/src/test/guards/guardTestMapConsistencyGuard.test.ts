 ...
 *
 * `references/03-implementation/guard-test-map.md` が全 `app/src/test/guards/*.test.ts`
 ...
 *
 * @see references/03-implementation/guard-test-map.md
...
const GUARD_MAP_PATH = path.join(PROJECT_ROOT, 'references/03-implementation/guard-test-map.md')
...
function formatRatchetMessage(missing: readonly string[]): string {
  ...
  lines.push('')
  lines.push('references/03-implementation/guard-test-map.md に以下の guard を登録してください:')
