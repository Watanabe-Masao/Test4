...
describe('detection/checkBidirectionalReference', () => {
  ...
  it('全 entry が対称で違反 0', () => {
    const map: Record<string, string | null> = { a: 'b', b: 'a', c: null }
    expect(checkBidirectionalReference(['a', 'b', 'c'], (id) => map[id] ?? null, opts)).toEqual([])
  })

  it('片方向のみ定義で violation 1', () => {
    // a → b、b → null (非対称)
    const map: Record<string, string | null> = { a: 'b', b: null }
    const r = checkBidirectionalReference(['a', 'b'], (id) => map[id] ?? null, opts)
  ...
  it('reference 先が異なる対象を指す non-symmetric で violation', () => {
    // a → b、b → c (a と c の不整合)
    const map: Record<string, string | null> = { a: 'b', b: 'c', c: 'b' }
    const r = checkBidirectionalReference(['a', 'b', 'c'], (id) => map[id] ?? null, opts)
    expect(r.map((v) => v.location)).toContain('Antibody Pair: a → b')
  })

  it('null reference は対象外 (対概念がない archetype)', () => {
    const map: Record<string, string | null> = { a: null, b: null }
    expect(checkBidirectionalReference(['a', 'b'], (id) => map[id] ?? null, opts)).toEqual([])
  })
