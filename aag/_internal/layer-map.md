...
>
> **件数を書かない原則**: 件数 (`N rule` / `N file` / `N guard`) は **generated section に寄せる** (`AAG-REQ-NO-DATE-RITUAL` と同じ「現在値を prose に書かない」原則、`docStaticNumberGuard` 適用)。本 doc は file path + 役割のみ articulate、件数は `references/04-tracking/generated/architecture-health.generated.md` 参照。
...
| file | 役割 |
|---|---|
| [`meta.md`](./meta.md) §1 | AAG の why の正本 (機械検証不可、user 判断のみ) |
...
| Discovery Review | 意味品質補完 (user review) | 4.4 完備性監査 |
...
| `references/04-tracking/generated/architecture-health.generated.md` | 件数 / 現在値の正本 (本 doc では articulate しない) |
