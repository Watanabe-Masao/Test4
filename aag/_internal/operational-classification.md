...
> - 下位 (drill-down): `references/03-implementation/architecture-rule-system.md` (AR-rule 運用ガイド) / `references/03-implementation/allowlist-management.md` (allowlist 管理手順) / `app-domain/gross-profit/rule-catalog/base-rules.ts` (BaseRule 物理正本)
>
...
>
> **件数を書かない原則**: 各分類の件数 (`N rule` / `N invariant` / `N default`) は **generated section に寄せる** (`docStaticNumberGuard` 適用)。本 doc では rule ID + ruleClass + what (短文) のみ articulate、件数は `references/04-tracking/generated/architecture-health.generated.md` 参照。
...
詳細 inventory は `app-domain/gross-profit/rule-catalog/base-rules.ts` + `references/04-tracking/generated/architecture-health.generated.md` 参照 (件数は generated section)。
...
詳細は `references/03-implementation/architecture-rule-system.md` の review section 参照。
...
## §6 AAG Response フロー — 違反時に何が返るか

> 違反検出時に **どこで止まっても同じ情報構造で返る** ための統一 response mechanism。本 §は旧 `adaptive-architecture-governance.md` §AAG Response フローからの migrate (Project A Phase 2)。

### §6.1 共通 response renderer

```
コード変更
  │
  ├─ pre-commit ──┐
  ├─ guard test ──┤── renderAagResponse() ──→ 統一 format 出力
  ├─ obligation ──┤                              │
  └─ health ──────┘                              ├─ fixNow=now    → 修正手順を返す
...
| 入口 | タイミング | 共通 renderer |
|---|---|---|
| pre-commit | `git commit` 時 | 統一 format (shell 互換) |
| guard test | `npm run test:guards` 時 | `renderAagResponse()` 経由 |
| obligation | `docs:generate` 時 | 統一 format (`console.error`) |
...
| 情報 | 内容 |
|---|---|
| **何が** (what) | 違反した rule ID + ruleClass + 違反 file path + 違反 line |
| **なぜ** (why) | rule の `why` field articulate (= AR-rule の意図) |
| **方向** (slice 誘導) | 「向かう先」を 1 行で示す (例: 「正本経路に統合する」「責務を分離する」「allowlist で例外管理する」) |
| **手順** (`fixNow` 別の修正経路) | now → 修正手順、debt → allowlist 誘導、review → Discovery Review 誘導 |
| **例外** (`decisionCriteria`) | 例外として allowlist に含める判断基準 |
| **詳細** (canonical doc 参照) | rule の `canonicalDocRef.refs[].docPath` (Project B Phase 3 で landing 後) |

### §6.4 fixNow 分類と運用区分の関係

fixNow 分類 (`now` / `debt` / `review`) は本 doc §1〜§4 の運用区分と **同義**。`renderAagResponse()` は rule の `fixNow` field (= `defaults.ts` execution overlay の articulate) を読み、運用区分に応じた誘導文を生成する:

- **fixNow=now** → 本 doc §2 (即修正) の鉄則「今この diff で直す」が適用、修正手順を返す
- **fixNow=debt** → 本 doc §3 (構造負債) の鉄則「allowlist + ratchet-down で計画的に返す」が適用、allowlist 誘導を返す
- **fixNow=review** → 本 doc §4 (観測) の鉄則「Discovery Review の入力」が適用、即 fail させず観測値として記録

## §7 旧 運用区分 → 新 運用区分 mapping (`§1.5 archive 前 mapping 義務`)
...
- **rule 件数 / 現在値** → `references/04-tracking/generated/architecture-health.generated.md`
